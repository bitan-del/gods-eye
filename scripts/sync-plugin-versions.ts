...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core GodsEye release numbers.\n\n`;
...
function stripWorkspaceGodseyeDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.godseye !== "workspace:*") {
    return false;
  }
  delete devDeps.godseye;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceGodseyeDevDependency(pkg);
