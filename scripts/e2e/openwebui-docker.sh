...
IMAGE_NAME="godseye-openwebui-e2e"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
PROFILE_FILE="${GODSEYE_PROFILE_FILE:-$HOME/.profile}"
MODEL="${GODSEYE_OPENWEBUI_MODEL:-openai/gpt-5.4}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${GODSEYE_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${GODSEYE_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${GODSEYE_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${GODSEYE_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${GODSEYE_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="godseye-openwebui-e2e-$$"
GW_NAME="godseye-openwebui-gateway-$$"
OW_NAME="godseye-openwebui-$$"
...
docker run -d \
  ...
  -e "GODSEYE_GATEWAY_TOKEN=$TOKEN" \
  -e "GODSEYE_OPENWEBUI_MODEL=$MODEL" \
  -e "GODSEYE_SKIP_CHANNELS=1" \
  -e "GODSEYE_SKIP_GMAIL_WATCHER=1" \
  -e "GODSEYE_SKIP_CRON=1" \
  -e "GODSEYE_SKIP_CANVAS_HOST=1" \
  "${EXTERNAL_AUTH_MOUNTS[@]}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc '
    ...
    node "$entry" config set gateway.auth.token "$GODSEYE_GATEWAY_TOKEN" >/dev/null
    node "$entry" config set gateway.http.endpoints.chatCompletions.enabled true --strict-json >/dev/null
    node "$entry" config set agents.defaults.model.primary "$GODSEYE_OPENWEBUI_MODEL" >/dev/null
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="GodsEye E2E" \
  -e WEBUI_SECRET_KEY="godseye-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="GodsEye E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="godseye/default" \
...
echo "Running Open WebUI -> GodsEye smoke..."
docker exec \
