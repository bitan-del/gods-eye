...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_GODSEYE_BIN="/opt/homebrew/bin/godseye"
GUEST_GODSEYE_ENTRY="/opt/homebrew/lib/node_modules/godseye/godseye.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/godseye-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/godseye-parallels-build.lock"
...
Options:
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: godseye@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__GODSEYE_READY__\\n'\r"
expect "__GODSEYE_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/godseye-prl.sh <<'__GODSEYE_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__GODSEYE_SCRIPT__\r"
send -- "/bin/bash /tmp/godseye-prl.sh; rc=\$?; rm -f /tmp/godseye-prl.sh; printf '__GODSEYE_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__GODSEYE_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view godseye version --userconfig "$(mktemp)"
}
...
export GODSEYE_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/godseye-install.sh
bash /tmp/godseye-install.sh${version_arg_q}
$GUEST_GODSEYE_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_GODSEYE_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/godseye-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/godseye-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/godseye"; check_path "\$root/godseye/extensions"; if [ -d "\$root/godseye/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/godseye/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_GODSEYE_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_GODSEYE_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_GODSEYE_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_GODSEYE_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli \
    "$GUEST_GODSEYE_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_GODSEYE_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from godseye dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/godseye-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>GodsEye Control</title>' /tmp/godseye-dashboard-smoke.html >/dev/null; then
      if grep -F '<godseye-app></godseye-app>' /tmp/godseye-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>GodsEye Control</title>' /tmp/godseye-dashboard-smoke.html >/dev/null
grep -F '<godseye-app></godseye-app>' /tmp/godseye-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/godseye-discord-token <<'__GODSEYE_TOKEN__'
$DISCORD_TOKEN_VALUE
__GODSEYE_TOKEN__
cat >/tmp/godseye-discord-guilds.json <<'__GODSEYE_GUILDS__'
$guilds_json
__GODSEYE_GUILDS__
token="\$(tr -d '\n' </tmp/godseye-discord-token)"
guilds_json="\$(cat /tmp/godseye-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_GODSEYE_ENTRY channels status --probe --json
rm -f /tmp/godseye-discord-token /tmp/godseye-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "godseye-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "godseye-main-upgrade.tgz"
