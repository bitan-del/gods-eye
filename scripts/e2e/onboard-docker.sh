...
IMAGE_NAME="godseye-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/godseye-bin:$PATH"
  mkdir -p /tmp/godseye-bin
  cat > /tmp/godseye-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$GODSEYE_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_godseye_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/godseye-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/godseye-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$GODSEYE_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/godseye-e2e-$1.XXXXXX"
  }

  set_isolated_godseye_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export GODSEYE_HOME="$home_dir"
    export GODSEYE_STATE_DIR="$home_dir/.godseye"
    export GODSEYE_CONFIG_PATH="$GODSEYE_STATE_DIR/godseye.json"
    mkdir -p "$GODSEYE_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$GODSEYE_STATE_DIR/workspace"
    config_path="$GODSEYE_CONFIG_PATH"
    sessions_dir="$GODSEYE_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_godseye_env "$home_dir"
	    ...
	    node "$GODSEYE_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$GODSEYE_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$GODSEYE_ENTRY\" configure --section skills" send_skills_flow
