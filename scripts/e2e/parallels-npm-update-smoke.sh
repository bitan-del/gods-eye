...
PACKAGE_SPEC=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/godseye-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: godseye@latest
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/godseye-main-$CURRENT_HEAD_SHORT.tgz"
  cp "$MAIN_TGZ_DIR/$pkg" "$MAIN_TGZ_PATH"
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/godseye-main-update.ps1"
...
try {
  $env:PATH = "$env:LOCALAPPDATA\GodsEye\deps\portable-git\cmd;$env:LOCALAPPDATA\GodsEye\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\GodsEye\deps\portable-git\usr\bin;$env:PATH"
  $tgz = Join-Path $env:TEMP 'godseye-main-update.tgz'
  ...
  $version = Invoke-CaptureLogged 'godseye --version' { & $godseye --version }
  if ($version -notmatch [regex]::Escape($HeadShort)) {
    throw "version mismatch: expected substring $HeadShort"
  }
  Invoke-Logged 'godseye models set' { & $godseye models set openai/gpt-5.4 }
  ...
  Invoke-Logged 'godseye gateway restart' { & $godseye gateway restart }
  Start-Sleep -Seconds 5
  Invoke-Logged 'godseye gateway status' { & $godseye gateway status --deep --require-rpc }
  Invoke-CaptureLogged 'godseye agent' { & $godseye agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/godseye-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"GodsEye \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="godseye-update-$RANDOM-$RANDOM.ps1"
  log_name="godseye-update-$RANDOM-$RANDOM.log"
  done_name="godseye-update-$RANDOM-$RANDOM.done"
...
run_macos_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$MACOS_VM" --current-user /usr/bin/tee /tmp/godseye-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/godseye-main-update.tgz
/opt/homebrew/bin/npm install -g /tmp/godseye-main-update.tgz
version="\$(/opt/homebrew/bin/godseye --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
/opt/homebrew/bin/godseye models set openai/gpt-5.4
/opt/homebrew/bin/godseye gateway status --deep --require-rpc
/opt/homebrew/bin/godseye agent --agent main --session-id parallels-npm-update-macos-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$MACOS_VM" --current-user /bin/bash /tmp/godseye-main-update.sh
}
...
run_linux_update() {
  local tgz_url="$1"
  local head_short="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/godseye-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
curl -fsSL "$tgz_url" -o /tmp/godseye-main-update.tgz
npm install -g /tmp/godseye-main-update.tgz --no-fund --no-audit
version="\$(godseye --version)"
printf '%s\n' "\$version"
case "\$version" in
  *"$head_short"*) ;;
  *)
...
godseye models set openai/gpt-5.4
godseye agent --local --agent main --session-id parallels-npm-update-linux-$head_short --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" /bin/bash /tmp/godseye-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="godseye@$LATEST_VERSION"
fi
