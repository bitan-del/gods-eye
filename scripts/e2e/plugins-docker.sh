...
IMAGE_NAME="godseye-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  GODSEYE_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  GODSEYE_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export GODSEYE_ENTRY

home_dir=$(mktemp -d "/tmp/godseye-plugins-e2e.XXXXXX")
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$GODSEYE_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$GODSEYE_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$GODSEYE_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/godseye.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/godseye.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.godseye/extensions/demo-plugin"

cat > "$HOME/.godseye/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.godseye/extensions/demo-plugin/godseye.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/godseye-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@godseye/demo-plugin-tgz",
  "version": "0.0.1",
  "godseye": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/godseye.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$GODSEYE_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/godseye-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@godseye/demo-plugin-dir",
  "version": "0.0.1",
  "godseye": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/godseye.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$GODSEYE_ENTRY" plugins install "$dir_plugin"
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/godseye-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@godseye/demo-plugin-file",
  "version": "0.0.1",
  "godseye": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/godseye.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$GODSEYE_ENTRY" plugins install "file:$file_pack_dir/package"
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$HOME/.godseye/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".godseye", "godseye.json");
...
gateway_log="/tmp/godseye-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/godseye-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@godseye/slash-install-plugin",
  "version": "0.0.1",
  "godseye": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/godseye.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "godseye/fixture-marketplace"
    }
  }
}
JSON

node "$GODSEYE_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$GODSEYE_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$GODSEYE_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$GODSEYE_ENTRY" plugins update marketplace-shortcut --dry-run
node "$GODSEYE_ENTRY" plugins update marketplace-shortcut
node "$GODSEYE_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
