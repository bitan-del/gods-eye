...
const EXPECTED_REPOSITORY_URL = "https://github.com/bitan-del/gods-eye";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "godseye") {
    errors.push(`package.json name must be "godseye"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.godseye !== "godseye.mjs") {
    errors.push(
      `package.json bin.godseye must be "godseye.mjs"; found "${pkg.bin?.godseye ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`godseye-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `godseye-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
