#!/usr/bin/env bash
# ClawDock - Docker helpers for GodsEye
# Inspired by Simon Willison's "Running GodsEye in Docker"
# https://til.simonwillison.net/llms/godseye-docker
#
# Installation:
...
# Common paths to check for GodsEye
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/godseye"
  "${HOME}/workspace/godseye"
  "${HOME}/projects/godseye"
  "${HOME}/dev/godseye"
  "${HOME}/code/godseye"
  "${HOME}/src/godseye"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found GodsEye at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/godseye"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ GodsEye not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/bitan-del/gods-eye.git ~/godseye"
    echo "  cd ~/godseye && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^GODSEYE_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d godseye-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart godseye-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f godseye-gateway
}
...
clawdock-workspace() {
  cd ~/.godseye/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec godseye-gateway \
    bash -c 'echo "alias godseye=\"./godseye.mjs\"" > /tmp/.bashrc_godseye && bash --rcfile /tmp/.bashrc_godseye'
}
...
clawdock-cli() {
  _clawdock_compose run --rm godseye-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build godseye-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "GODSEYE_GATEWAY_TOKEN=$token" godseye-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" godseye-gateway \
    bash -c './godseye.mjs config set gateway.remote.token "$TOKEN" && ./godseye.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec godseye-gateway \
    bash -c "./godseye.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart godseye-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm godseye-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec godseye-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'godseye config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for GodsEye${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (godseye alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to godseye project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.godseye)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'godseye channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'godseye status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.gods-eye.org${_CLR_RESET}"
  echo ""
}
