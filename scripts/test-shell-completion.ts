 ...
 * `godseye update`. Use it to verify the completion installation flow
...
\`godseye update\`, \`godseye doctor\`, and \`godseye onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# GodsEye Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`godseye completion --install\` later to enable.`));
    return;
  }
