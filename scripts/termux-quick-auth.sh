...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/godseye/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "GodsEye Re-Auth" -c "After getting key, run: ssh $SERVER '~/godseye/scripts/mobile-reauth.sh'" --id godseye-auth
        ;;
    *)
