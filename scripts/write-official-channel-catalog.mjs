...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.godseye) ? packageJson.godseye : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.godseye?.channel?.id) || left.name;
    const rightId = trimString(right.godseye?.channel?.id) || right.name;
