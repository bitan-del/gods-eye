#!/bin/bash
# Setup GodsEye Auth Management System
...
echo "=== GodsEye Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'godseye-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  GodsEye message: Send warning via GodsEye itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/godseye-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/godseye-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/godseye-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now godseye-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status godseye-auth-monitor.timer"
echo ""
echo "Quick commands:"
