...
type DriverMode = "token" | "webhook" | "godseye";
...
function resolveStateDir(): string {
  const override = process.env.GODSEYE_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.GODSEYE_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".godseye");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver godseye] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) GodsEye spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|godseye> Driver transport mode (default: token)\n" +
    ...
    "  --godseye-bin <path>        GodsEye CLI binary for driver=godseye (default: godseye)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  GODSEYE_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  GODSEYE_DISCORD_SMOKE_DRIVER\n" +
    "  GODSEYE_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  GODSEYE_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  GODSEYE_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  GODSEYE_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  GODSEYE_DISCORD_SMOKE_AGENT\n" +
    "  GODSEYE_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  GODSEYE_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  GODSEYE_DISCORD_SMOKE_POLL_MS\n" +
    "  GODSEYE_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  GODSEYE_DISCORD_SMOKE_GODSEYE_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.GODSEYE_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.GODSEYE_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "godseye"
        ? "godseye"
  ...
  const botToken =
    ...
    process.env.GODSEYE_DISCORD_SMOKE_BOT_TOKEN ||
  ...
  const instruction =
    resolveArg("--instruction") || process.env.GODSEYE_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.GODSEYE_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.GODSEYE_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.GODSEYE_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const godseyeBin =
    resolveArg("--godseye-bin") || process.env.GODSEYE_DISCORD_SMOKE_GODSEYE_BIN || "godseye";
...
async function godseyeCliJson<T>(params: { godseyeBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.godseyeBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`godseye ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithGodseye(params: {
  godseyeBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await godseyeCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    godseyeBin: params.godseyeBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "godseye") {
    return await readMessagesWithGodseye({
      godseyeBin: params.args.godseyeBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `godseye-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await godseyeCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("godseye message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "godseye"
            ? await readMessagesWithGodseye({
                godseyeBin: args.godseyeBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from GodsEye.`,
        diagnostics: {
          bindingCandidates: [
            {
