...
const extensionRestartMetadataFiles = new Set(["godseye.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.GODSEYE_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.GODSEYE_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[godseye] ${message}\n`);
};

const runGodsEye = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["godseye.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
