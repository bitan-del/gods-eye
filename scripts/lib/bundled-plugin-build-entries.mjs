...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.godseye?.extensions)
    ? packageJson.godseye.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.godseye?.setupEntry === "string" &&
    packageJson.godseye.setupEntry.trim().length > 0
      ? packageJson.godseye.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/godseye.plugin.json`);
    if (hasPackageJson) {
