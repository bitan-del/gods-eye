...
# GodsEye Installer for macOS and Linux
...
GUM_VERSION="${GODSEYE_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://gods-eye.org/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$GODSEYE_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
cleanup_npm_godseye_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.godseye-* "$npm_root"/godseye 2>/dev/null || true
}

extract_godseye_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_godseye_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/godseye" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/godseye"|"/usr/local/bin/godseye")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/godseye/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale godseye symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing godseye binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing GodsEye package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_godseye_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "GodsEye npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*godseye" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_godseye_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_godseye_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "GodsEye npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an godseye binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${GODSEYE_TAGLINE_INDEX:-}" ]]; then
        if [[ "${GODSEYE_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((GODSEYE_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${GODSEYE_NO_ONBOARD:-0}
NO_PROMPT=${GODSEYE_NO_PROMPT:-0}
DRY_RUN=${GODSEYE_DRY_RUN:-0}
INSTALL_METHOD=${GODSEYE_INSTALL_METHOD:-}
GODSEYE_VERSION=${GODSEYE_VERSION:-latest}
USE_BETA=${GODSEYE_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/godseye"
GIT_DIR=${GODSEYE_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${GODSEYE_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${GODSEYE_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${GODSEYE_VERBOSE:-0}"
VERIFY_INSTALL="${GODSEYE_VERIFY_INSTALL:-0}"
GODSEYE_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
GodsEye installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://gods-eye.org/install.sh | bash -s -- [options]
...
Environment variables:
  GODSEYE_INSTALL_METHOD=git|npm
  GODSEYE_VERSION=latest|next|main|<semver>|<spec>
  GODSEYE_BETA=0|1
  GODSEYE_GIT_DIR=...
  GODSEYE_GIT_UPDATE=0|1
  GODSEYE_NO_PROMPT=1
  GODSEYE_VERIFY_INSTALL=1
  GODSEYE_DRY_RUN=1
  GODSEYE_NO_ONBOARD=1
  GODSEYE_VERBOSE=1
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://gods-eye.org/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://gods-eye.org/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://gods-eye.org/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://gods-eye.org/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected GodsEye checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a GodsEye source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_godseye_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"godseye"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://gods-eye.org/install.sh | bash"
}
...
ensure_godseye_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/godseye" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/godseye" ]]; then
        ln -sf "$npm_root/godseye/dist/entry.js" "${npm_bin}/godseye"
        ui_info "Created godseye bin link at ${npm_bin}/godseye"
    fi
    return 0
}

# Check for existing GodsEye installation
check_existing_godseye() {
    if [[ -n "$(type -P godseye 2>/dev/null || true)" ]]; then
        ui_info "Existing GodsEye installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make godseye show as \"command not found\" in new terminals."
...
warn_godseye_not_found() {
    ui_warn "Installed, but godseye is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t godseye 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named godseye; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_godseye_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P godseye 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/godseye" ]]; then
        echo "${npm_bin}/godseye"
        return 0
    fi
...
install_godseye_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/bitan-del/gods-eye.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing GodsEye from git checkout: ${repo_dir}"
    else
        ui_info "Installing GodsEye from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning GodsEye" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building GodsEye" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/godseye" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/godseye"
    ui_success "GodsEye wrapper installed to \$HOME/.local/bin/godseye"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view godseye dist-tags.beta 2>/dev/null || true)"
...
install_godseye() {
    local package_name="godseye"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            GODSEYE_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="godseye"
        else
            GODSEYE_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${GODSEYE_VERSION}" ]]; then
        GODSEYE_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${GODSEYE_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${GODSEYE_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing GodsEye v${resolved_version}"
    else
        ui_info "Installing GodsEye (${GODSEYE_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${GODSEYE_VERSION}")"

    if ! install_godseye_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_godseye_paths
        install_godseye_npm "${install_spec}"
    fi

    if [[ "${GODSEYE_VERSION}" == "latest" && "${package_name}" == "godseye" ]]; then
        if ! resolve_godseye_bin &> /dev/null; then
            ui_warn "npm install godseye@latest failed; retrying godseye@next"
            cleanup_npm_godseye_paths
            install_godseye_npm "godseye@next"
        fi
    fi

    ensure_godseye_bin_link || true
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (godseye not on PATH yet)"
        warn_godseye_not_found
        return 0
    fi
...
# ---------------------------------------------------------------------------
# Onboard flow — delegates to the built-in godseye onboard command
# (same proven TUI that OpenClaw uses — handles all providers, auth, etc.)
# ---------------------------------------------------------------------------
run_onboard_flow() {
    local claw="${GODSEYE_BIN:-}"
    if [[ -z "$claw" ]]; then
    ...
    echo ""
    ui_section "Gods Eye Setup"
    echo ""
    echo -e "${INFO}Starting the onboarding wizard...${NC}"
    echo -e "${INFO}This will walk you through configuring your AI providers.${NC}"
    echo ""

    # Ensure gateway.mode is set before onboard (prevents mode error)
    local current_mode
    ...
    # Run the built-in onboard command (handles all providers, auth, validation)
    if "$claw" onboard </dev/tty; then
        ui_success "Onboarding complete"
    else
        ui_warn "Onboarding exited early; you can run it again with: godseye onboard"
    fi
}
...
resolve_workspace_dir() {
    local profile="${GODSEYE_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.godseye/workspace-${profile}"
    else
        echo "${HOME}/.godseye/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run godseye onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but godseye not on PATH; skipping onboarding"
        warn_godseye_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run godseye onboard to retry"
        return
    }
}
...
if ! declare -F extract_godseye_semver >/dev/null 2>&1; then
...
resolve_godseye_version() {
    ...
    local claw="${GODSEYE_BIN:-}"
    if [[ -z "$claw" ]] && command -v godseye &> /dev/null; then
        claw="$(command -v godseye)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_godseye_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/godseye/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/godseye/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: godseye not on PATH yet"
        warn_godseye_not_found
        return 1
    fi

    run_quiet_step "Checking GodsEye version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: godseye gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_godseye_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found GodsEye checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set GODSEYE_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_godseye; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing GodsEye"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g godseye &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g godseye 2>/dev/null || true
    ...
    GODSEYE_BIN="$(resolve_godseye_bin || true)"
    ...
    installed_version=$(resolve_godseye_version)
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running godseye doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                GODSEYE_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                GODSEYE_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                GODSEYE_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run godseye doctor and godseye plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
    ...
    if command -v godseye &> /dev/null; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (godseye daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if GODSEYE_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: godseye daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${GODSEYE_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
