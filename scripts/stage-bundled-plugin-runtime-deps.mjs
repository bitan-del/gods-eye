...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.godseye?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.godseye) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.godseye;
  ...
  if (packageJson.peerDependenciesMeta?.godseye) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.godseye;
  ...
  if (packageJson.devDependencies?.godseye) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.godseye;
  ...
  if (platform === "win32") {
    ...
    throw new Error(
      `failed to resolve a toolchain-local npm next to ${execPath}. ` +
        ...
        "GodsEye refuses to shell out to bare npm on Windows; install a Node.js toolchain that bundles npm or run with a matching Node installation.",
    );
  }
