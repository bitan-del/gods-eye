...
const warningFilterKey = Symbol.for("godseye.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadGodsEyePlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadGodsEyePlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@godseye/build-smoke-plugin",
      type: "module",
      godseye: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'godseye/plugin-sdk';",
...
const registry = loadGodsEyePlugins({
  ...
  env: {
    ...process.env,
    GODSEYE_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
