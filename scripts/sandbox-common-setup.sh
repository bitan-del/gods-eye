...
BASE_IMAGE="${BASE_IMAGE:-godseye-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-godseye-sandbox-common:bookworm-slim}"
...
GODSEYE_DOCKER_BUILD_USE_BUILDX="${GODSEYE_DOCKER_BUILD_USE_BUILDX:-0}"
GODSEYE_DOCKER_BUILD_CACHE_FROM="${GODSEYE_DOCKER_BUILD_CACHE_FROM:-}"
GODSEYE_DOCKER_BUILD_CACHE_TO="${GODSEYE_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${GODSEYE_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${GODSEYE_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${GODSEYE_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${GODSEYE_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${GODSEYE_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=godseye.sandbox=1)
NOTE
