...
#
# Suggested cron: */30 * * * * /home/admin/godseye/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send GodsEye notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., godseye-alerts)
...
STATE_FILE="$HOME/.godseye/auth-monitor-state"
...
send_notification() {
    ...
    # Send via GodsEye if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use godseye
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via GodsEye to $NOTIFY_PHONE..."
            godseye send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! GodsEye is down. Run: ssh l36 '~/godseye/scripts/mobile-reauth.sh'" "urgent"
    exit 1
