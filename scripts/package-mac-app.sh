...
# Build and bundle GodsEye into a minimal .app we can open.
# Outputs to dist/GodsEye.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/GodsEye.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="GodsEye"
BUNDLE_ID="${BUNDLE_ID:-ai.godseye.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/godseye/godseye/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/GodsEye/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :GodsEyeBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :GodsEyeGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/GodsEye"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/GodsEye"
fi
chmod +x "$APP_ROOT/Contents/MacOS/GodsEye"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/GodsEye" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/GodsEye/Resources/GodsEye.icns" "$APP_ROOT/Contents/Resources/GodsEye.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/GodsEye/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying GodsEyeKit resources"
GODSEYEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/GodsEyeKit_GodsEyeKit.bundle"
if [ -d "$GODSEYEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/GodsEyeKit_GodsEyeKit.bundle"
  cp -R "$GODSEYEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/GodsEyeKit_GodsEyeKit.bundle"
else
  echo "WARN: GodsEyeKit resource bundle not found at $GODSEYEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running GodsEye"
killall -q GodsEye 2>/dev/null || true
