...
- regenerates apps/ios/GodsEye.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${GODSEYE_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid GODSEYE_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid GODSEYE_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid GODSEYE_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid GODSEYE_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing GODSEYE_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(GODSEYE_URL_SLASH)$(GODSEYE_URL_SLASH)#g'
)"
...
GODSEYE_CODE_SIGN_STYLE = Automatic
GODSEYE_DEVELOPMENT_TEAM = ${TEAM_ID}
GODSEYE_IOS_SELECTED_TEAM = ${TEAM_ID}
GODSEYE_APP_BUNDLE_ID = ai.godseye.client
GODSEYE_SHARE_BUNDLE_ID = ai.godseye.client.share
GODSEYE_ACTIVITY_WIDGET_BUNDLE_ID = ai.godseye.client.activitywidget
GODSEYE_WATCH_APP_BUNDLE_ID = ai.godseye.client.watchkitapp
GODSEYE_WATCH_EXTENSION_BUNDLE_ID = ai.godseye.client.watchkitapp.extension
GODSEYE_APP_PROFILE =
GODSEYE_SHARE_PROFILE =
GODSEYE_PUSH_TRANSPORT = relay
GODSEYE_PUSH_DISTRIBUTION = official
GODSEYE_URL_SLASH = /
GODSEYE_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
GODSEYE_PUSH_APNS_ENVIRONMENT = production
EOF
