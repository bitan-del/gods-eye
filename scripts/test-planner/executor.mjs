...
export function createExecutionArtifacts(env = process.env) {
  ...
  const ensureTempArtifactDir = () => {
    if (tempArtifactDir === null) {
      tempArtifactDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-test-parallel-"));
    }
  ...
  const cleanupTempArtifacts = () => {
    ...
    if (env.GODSEYE_TEST_KEEP_TEMP_ARTIFACTS === "1") {
...
export async function executePlan(plan, options = {}) {
  ...
  const windowsCiArgs = plan.runtimeCapabilities.isWindowsCi
    ...
    : [];
  const silentArgs = env.GODSEYE_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
  const rawMemoryTrace = env.GODSEYE_TEST_MEMORY_TRACE?.trim().toLowerCase();
  ...
  const heapSnapshotSignal = env.GODSEYE_TEST_HEAPSNAPSHOT_SIGNAL?.trim() || "SIGUSR2";
  const heapSnapshotBaseDir = heapSnapshotEnabled
    ? path.resolve(
        env.GODSEYE_TEST_HEAPSNAPSHOT_DIR?.trim() ||
          path.join(os.tmpdir(), `godseye-heapsnapshots-${Date.now()}`),
      )
    : null;
  const maxOldSpaceSizeMb = (() => {
    const raw = env.GODSEYE_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
