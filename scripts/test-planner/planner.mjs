...
const buildRequestedSurfaces = (request, env) => {
  ...
  const surfaces = [];
  const skipDefaultRuns = env.GODSEYE_TEST_SKIP_DEFAULT === "1";
  if (!skipDefaultRuns) {
    surfaces.push("unit");
  }
  if (env.GODSEYE_TEST_INCLUDE_EXTENSIONS === "1") {
    surfaces.push("extensions");
  }
  if (env.GODSEYE_TEST_INCLUDE_CHANNELS === "1") {
    surfaces.push("channels");
  }
  if (env.GODSEYE_TEST_INCLUDE_GATEWAY === "1") {
...
const resolveCIManifestScope = (scope = {}, env = process.env) => ({
  ...
  docsOnly: parseBooleanLike(scope.docsOnly ?? env.GODSEYE_CI_DOCS_ONLY, false),
  docsChanged: parseBooleanLike(scope.docsChanged ?? env.GODSEYE_CI_DOCS_CHANGED, false),
  runNode: parseBooleanLike(scope.runNode ?? env.GODSEYE_CI_RUN_NODE, true),
  runMacos: parseBooleanLike(scope.runMacos ?? env.GODSEYE_CI_RUN_MACOS, true),
  runAndroid: parseBooleanLike(scope.runAndroid ?? env.GODSEYE_CI_RUN_ANDROID, true),
  runWindows: parseBooleanLike(scope.runWindows ?? env.GODSEYE_CI_RUN_WINDOWS, true),
  runSkillsPython: parseBooleanLike(
    scope.runSkillsPython ?? env.GODSEYE_CI_RUN_SKILLS_PYTHON,
    true,
  ),
  hasChangedExtensions: parseBooleanLike(
    scope.hasChangedExtensions ?? env.GODSEYE_CI_HAS_CHANGED_EXTENSIONS,
    false,
  ),
  changedExtensionsMatrix: parseChangedExtensionsMatrix(
    scope.changedExtensionsMatrix ?? env.GODSEYE_CI_CHANGED_EXTENSIONS_MATRIX,
  ),
  runChangedSmoke: parseBooleanLike(
    scope.runChangedSmoke ?? env.GODSEYE_CI_RUN_CHANGED_SMOKE,
    true,
  ),
});
...
const resolveMaxWorkersForUnit = (unit, context) => {
  const overrideWorkers = Number.parseInt(context.env.GODSEYE_TEST_WORKERS ?? "", 10);
...
const withIncludeFileEnv = (context, unitId, files) => ({
  GODSEYE_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(unitId, files),
});
...
const resolveUnitHeavyFileGroups = (context) => {
  ...
  const heavyUnitFileLimit = parseEnvNumber(
    env,
    "GODSEYE_TEST_HEAVY_UNIT_FILE_LIMIT",
...
const buildDefaultUnits = (context, request) => {
  ...
  const extensionsBatchTargetMs = parseEnvNumber(
    env,
    "GODSEYE_TEST_EXTENSIONS_BATCH_TARGET_MS",
  ...
  const channelsBatchTargetMs = parseEnvNumber(
    env,
    "GODSEYE_TEST_CHANNELS_BATCH_TARGET_MS",
  ...
  const nextUnit = createExecutionUnit(context, {
    ...
    env:
      Array.isArray(unit.includeFiles) && unit.includeFiles.length > 0
        ? {
            ...unit.env,
            GODSEYE_VITEST_INCLUDE_FILE: context.writeTempJsonArtifact(
          ...
          ? [
              {
                ...
                command: [
                  "pnpm build",
                  "pnpm ui:build",
                  "node godseye.mjs --help",
                  "node godseye.mjs status --json --timeout 1",
...
export function explainExecutionTarget(request, options = {}) {
  ...
  context.noIsolateArgs =
    context.env.GODSEYE_TEST_ISOLATE === "1" || context.env.GODSEYE_TEST_ISOLATE === "true"
      ? []
      : context.env.GODSEYE_TEST_NO_ISOLATE !== "0" &&
          context.env.GODSEYE_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
...
export function buildExecutionPlan(request, options = {}) {
  ...
  context.noIsolateArgs =
    env.GODSEYE_TEST_ISOLATE === "1" || env.GODSEYE_TEST_ISOLATE === "true"
      ? []
      : env.GODSEYE_TEST_NO_ISOLATE !== "0" && env.GODSEYE_TEST_NO_ISOLATE !== "false"
        ? ["--isolate=false"]
        : [];
  ...
  const shardOverride = Number.parseInt(env.GODSEYE_TEST_SHARDS ?? "", 10);
  ...
  const shardIndexOverride = Number.parseInt(env.GODSEYE_TEST_SHARD_INDEX ?? "", 10);
  context.shardIndexOverride =
    Number.isFinite(shardIndexOverride) && shardIndexOverride > 0 ? shardIndexOverride : null;

  if (context.shardIndexOverride !== null && context.shardCount <= 1) {
    throw new Error(
      `GODSEYE_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} requires GODSEYE_TEST_SHARDS>1.`,
    );
  }
  if (context.shardIndexOverride !== null && context.shardIndexOverride > context.shardCount) {
    throw new Error(
      `GODSEYE_TEST_SHARD_INDEX=${String(context.shardIndexOverride)} exceeds GODSEYE_TEST_SHARDS=${String(context.shardCount)}.`,
    );
  }
  ...
  const topLevelParallelLimit = Math.max(
    1,
    parseEnvNumber(env, "GODSEYE_TEST_TOP_LEVEL_CONCURRENCY", defaultTopLevelParallelLimit),
  );
