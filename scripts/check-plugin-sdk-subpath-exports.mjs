...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("godseye/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("godseye/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced godseye/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced godseye/plugin-sdk/<subpath> must exist in the public package exports.",
  );
