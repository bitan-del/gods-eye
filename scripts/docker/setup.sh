...
IMAGE_NAME="${GODSEYE_IMAGE:-godseye:local}"
EXTRA_MOUNTS="${GODSEYE_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${GODSEYE_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${GODSEYE_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${GODSEYE_DOCKER_SOCKET:-}"
TIMEZONE="${GODSEYE_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$GODSEYE_CONFIG_DIR/godseye.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == GODSEYE_GATEWAY_TOKEN=* ]]; then
      token="${line#GODSEYE_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
ensure_control_ui_allowed_origins() {
  if [[ "${GODSEYE_GATEWAY_BIND}" == "loopback" ]]; then
    return 0
  fi

  local allowed_origin_json
  local current_allowed_origins
  allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$GODSEYE_GATEWAY_PORT" "$GODSEYE_GATEWAY_PORT")"
...
sync_gateway_mode_and_bind() {
  ...
  run_prestart_cli config set gateway.bind "$GODSEYE_GATEWAY_BIND" >/dev/null
  echo "Pinned gateway.mode=local and gateway.bind=$GODSEYE_GATEWAY_BIND for Docker setup."
}
...
run_prestart_cli() {
  # During setup, avoid the shared-network godseye-cli service because it
  ...
  run_prestart_gateway --entrypoint node godseye-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" godseye-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "GODSEYE_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "GODSEYE_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
GODSEYE_CONFIG_DIR="${GODSEYE_CONFIG_DIR:-$HOME/.godseye}"
GODSEYE_WORKSPACE_DIR="${GODSEYE_WORKSPACE_DIR:-$HOME/.godseye/workspace}"

validate_mount_path_value "GODSEYE_CONFIG_DIR" "$GODSEYE_CONFIG_DIR"
validate_mount_path_value "GODSEYE_WORKSPACE_DIR" "$GODSEYE_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "GODSEYE_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "GODSEYE_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "GODSEYE_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "GODSEYE_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "GODSEYE_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "GODSEYE_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$GODSEYE_CONFIG_DIR"
mkdir -p "$GODSEYE_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$GODSEYE_CONFIG_DIR/identity"
mkdir -p "$GODSEYE_CONFIG_DIR/agents/main/agent"
mkdir -p "$GODSEYE_CONFIG_DIR/agents/main/sessions"

export GODSEYE_CONFIG_DIR
export GODSEYE_WORKSPACE_DIR
export GODSEYE_GATEWAY_PORT="${GODSEYE_GATEWAY_PORT:-18789}"
export GODSEYE_BRIDGE_PORT="${GODSEYE_BRIDGE_PORT:-18790}"
export GODSEYE_GATEWAY_BIND="${GODSEYE_GATEWAY_BIND:-lan}"
export GODSEYE_IMAGE="$IMAGE_NAME"
export GODSEYE_DOCKER_APT_PACKAGES="${GODSEYE_DOCKER_APT_PACKAGES:-}"
export GODSEYE_EXTENSIONS="${GODSEYE_EXTENSIONS:-}"
export GODSEYE_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export GODSEYE_HOME_VOLUME="$HOME_VOLUME_NAME"
export GODSEYE_ALLOW_INSECURE_PRIVATE_WS="${GODSEYE_ALLOW_INSECURE_PRIVATE_WS:-}"
export GODSEYE_SANDBOX="$SANDBOX_ENABLED"
export GODSEYE_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export GODSEYE_TZ="$TIMEZONE"
...
if [[ -z "${GODSEYE_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    GODSEYE_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $GODSEYE_CONFIG_DIR/godseye.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      GODSEYE_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      GODSEYE_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      GODSEYE_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${GODSEYE_CONFIG_DIR}:/home/node/.godseye"
    gateway_workspace_mount="${GODSEYE_WORKSPACE_DIR}:/home/node/.godseye/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${GODSEYE_INSTALL_DOCKER_CLI:-}" ]]; then
    export GODSEYE_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  GODSEYE_CONFIG_DIR \
  GODSEYE_WORKSPACE_DIR \
  GODSEYE_GATEWAY_PORT \
  GODSEYE_BRIDGE_PORT \
  GODSEYE_GATEWAY_BIND \
  GODSEYE_GATEWAY_TOKEN \
  GODSEYE_IMAGE \
  GODSEYE_EXTRA_MOUNTS \
  GODSEYE_HOME_VOLUME \
  GODSEYE_DOCKER_APT_PACKAGES \
  GODSEYE_EXTENSIONS \
  GODSEYE_SANDBOX \
  GODSEYE_DOCKER_SOCKET \
  DOCKER_GID \
  GODSEYE_INSTALL_DOCKER_CLI \
  GODSEYE_ALLOW_INSECURE_PRIVATE_WS \
  GODSEYE_TZ

if [[ "$IMAGE_NAME" == "godseye:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  docker build \
    --build-arg "GODSEYE_DOCKER_APT_PACKAGES=${GODSEYE_DOCKER_APT_PACKAGES}" \
    --build-arg "GODSEYE_EXTENSIONS=${GODSEYE_EXTENSIONS}" \
    --build-arg "GODSEYE_INSTALL_DOCKER_CLI=${GODSEYE_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the GodsEye metadata subdirectory
# (.godseye/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh godseye-gateway -c \
  'find /home/node/.godseye -xdev -exec chown node:node {} +; \
   [ -d /home/node/.godseye/workspace/.godseye ] && chown -R node:node /home/node/.godseye/workspace/.godseye || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from GODSEYE_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $GODSEYE_GATEWAY_BIND"
echo "Gateway token: $GODSEYE_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm godseye-cli channels login"
...
# --- Sandbox setup (opt-in via GODSEYE_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: godseye-sandbox:bookworm-slim"
    docker build \
      -t "godseye-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker godseye-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg GODSEYE_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (GODSEYE_IMAGE=godseye:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: GODSEYE_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in GodsEye config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate godseye-gateway
  fi
else
...
echo "Config: $GODSEYE_CONFIG_DIR"
echo "Workspace: $GODSEYE_WORKSPACE_DIR"
echo "Token: $GODSEYE_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f godseye-gateway"
echo "  ${COMPOSE_HINT} exec godseye-gateway node dist/index.js health --token \"$GODSEYE_GATEWAY_TOKEN\""
