...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (GODSEYE_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${GODSEYE_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${GODSEYE_INSTALL_LATEST_OUT:-}"
fi
