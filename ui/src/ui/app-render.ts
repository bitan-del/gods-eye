...
const UPDATE_BANNER_DISMISS_KEY = "godseye:control-ui:update-banner-dismissed:v1";
              ...
              <div class="sidebar-brand">
                ${
                  navCollapsed
                    ? nothing
                    : html`
                        <img class="sidebar-brand__logo" src="${agentLogoUrl(basePath)}" alt="Gods Eye" />
                        <span class="sidebar-brand__copy">
                          <span class="sidebar-brand__eyebrow">${t("nav.control")}</span>
                          <span class="sidebar-brand__title">GodsEye</span>
                        </span>
                      `
                }
              </div>
            ...
            ? (() => {
                ...
                return lazyRender(lazySkills, (m) =>
                  m.renderSkills({
                    ...
                    storeDetailLoading: state.skillsStoreDetailLoading,
                    onStoreDetailOpen: (slug) => {
                      state.skillsStoreDetailSlug = slug;
                      void loadStoreSkillDetail(state, slug);
                    },
                    onStoreDetailClose: () => {
                      state.skillsStoreDetailSlug = null;
                      state.skillsStoreDetail = null;
                    },
                    // Sort & category
                    storeSort: state.skillsStoreSort,
                    onStoreSortChange: (sort) => (state.skillsStoreSort = sort),
                    storeCategory: state.skillsStoreCategory,
                    onStoreCategoryChange: (cat) => (state.skillsStoreCategory = cat),
                    // Create
        ...
        ${
          state.tab === "chat"
            ? renderChat({
                ...
                onNewSession: async () => {
                  // Create a new distinct chat session via the server
                  ...
                  try {
                    ...
                    const newKey = (result as { key?: string })?.key;
                    if (!newKey) {
                      return;
                    }
                    ...
                    void loadSessions(state, { activeMinutes: 10080 });
                  } catch {
                    // Fallback: use client-side key generation
