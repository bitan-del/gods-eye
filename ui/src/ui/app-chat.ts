...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as GodsEyeApp);
}
...
async function sendChatMessageNow(
  ...
  const runId = await sendChatMessage(host as unknown as GodsEyeApp, message, opts?.attachments);
...
async function clearChatHistory(host: ChatHost) {
  ...
  try {
    ...
    host.chatRunId = null;
    await loadChatHistory(host as unknown as GodsEyeApp);
  } catch (err) {
    host.lastError = String(err);
  }
  scheduleChatScroll(host as unknown as Parameters<typeof scheduleChatScroll>[0]);
...
export async function refreshChat(host: ChatHost, opts?: { scheduleScroll?: boolean }) {
  await Promise.all([
    loadChatHistory(host as unknown as GodsEyeApp),
    loadSessions(host as unknown as GodsEyeApp, {
      activeMinutes: 0,
      limit: 0,
      includeGlobal: true,
      includeUnknown: true,
