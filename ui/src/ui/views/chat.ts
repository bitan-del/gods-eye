...
export type ChatProps = {
  ...
  onSessionRename?: (key: string, newLabel: string) => void;
  onSessionDelete?: (key: string) => void;
                        ...
                        ${group.options.map(
                          (option) => html`
                            ...
                            >
                              ...
                              <span class="ge-sidebar__history-actions">
                                <button
                                  class="ge-sidebar__history-action"
                                  type="button"
                                  title="Rename"
                                  @click=${(e: Event) => {
                                    e.stopPropagation();
                                    const newName = prompt("Rename chat:", option.label);
                                    if (newName?.trim()) {
                                      props.onSessionRename?.(option.key, newName.trim());
                                    }
                                  }}
                                >
                                ...
                                <button
                                  class="ge-sidebar__history-action ge-sidebar__history-action--delete"
                                  type="button"
                                  title="Delete"
                                  @click=${(e: Event) => {
                                    e.stopPropagation();
                                    props.onSessionDelete?.(option.key);
                                  }}
                                >
      ...
      ${catalog.map(
        (model) => html`
          ...
          >
            ...
            ${
              model.provider === "claude-cli" || model.provider === "codex-cli"
                ? html`
...
function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    const marker = raw.__godseye as Record<string, unknown> | undefined;
