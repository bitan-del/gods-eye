...
export async function handleWhatsAppStart(host: GodsEyeApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: GodsEyeApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: GodsEyeApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: GodsEyeApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: GodsEyeApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: GodsEyeApp): string {
...
function resolveGatewayHttpAuthHeader(host: GodsEyeApp): string | null {
...
function buildGatewayHttpHeaders(host: GodsEyeApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: GodsEyeApp) {
...
export function handleNostrProfileToggleAdvanced(host: GodsEyeApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: GodsEyeApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: GodsEyeApp) {
  const state = host.nostrProfileFormState;
