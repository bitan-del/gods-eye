...
import type { GodsEyeApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as GodsEyeApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as GodsEyeApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as GodsEyeApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as GodsEyeApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as GodsEyeApp);
    await loadConfig(host as unknown as GodsEyeApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as GodsEyeApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as GodsEyeApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as GodsEyeApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as GodsEyeApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as GodsEyeApp);
    await loadDevices(host as unknown as GodsEyeApp);
    await loadConfig(host as unknown as GodsEyeApp);
    await loadExecApprovals(host as unknown as GodsEyeApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as GodsEyeApp);
    await loadConfig(host as unknown as GodsEyeApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as GodsEyeApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as GodsEyeApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__GODSEYE_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as GodsEyeApp;
...
async function loadOverviewLogs(host: GodsEyeApp) {
...
function buildAttentionItems(host: GodsEyeApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as GodsEyeApp, true),
    loadConfigSchema(host as unknown as GodsEyeApp),
    loadConfig(host as unknown as GodsEyeApp),
  ]);
}
