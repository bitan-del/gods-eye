...
This document describes how the Gods Eye ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`godseye acp` exposes an ACP agent over stdio and forwards prompts to a running
Gods Eye Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Gods Eye Gateway
...
- ACP client spawns `godseye acp` and speaks ACP messages over stdio.
...
`godseye acp` resolves the Gateway URL and auth from CLI flags or config:
