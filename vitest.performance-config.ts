...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.GODSEYE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.GODSEYE_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (isEnabled(env.GODSEYE_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.GODSEYE_VITEST_PRINT_IMPORT_BREAKDOWN)) {
