---
name: node-connect
description: Diagnose Gods Eye node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Gods Eye is advertising that route, then fix pairing/auth.
...
- whether `godseye devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `godseye qr --json`. It uses the same setup-code payload Android scans.

```bash
godseye config get gateway.mode
godseye config get gateway.bind
godseye config get gateway.tailscale.mode
godseye config get gateway.remote.url
godseye config get gateway.auth.mode
godseye config get gateway.auth.allowTailscale
godseye config get plugins.entries.device-pair.config.publicUrl
godseye qr --json
godseye devices list
godseye nodes status
```

If this Gods Eye instance is pointed at a remote gateway, also run:

```bash
godseye qr --remote --json
```
...
If `godseye qr --json` says `Gateway is only bound to loopback`:
...
```bash
godseye devices list
godseye devices approve --latest
```
...
- `godseye devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run godseye qr again, rescan, then approve the pending device pairing.`
