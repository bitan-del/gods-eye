---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Gods Eye deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Gods Eye cron scheduling for periodic checks, or version status checks on a machine running Gods Eye (laptop, workstation, Pi, VPS).
---

# Gods Eye Host Hardening
...
- Never claim Gods Eye changes the host firewall, SSH, or OS updates; it does not.
...
5. Gods Eye gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Gods Eye can access sensitive data.
...
### 2) Run Gods Eye security audits (read-only)

As part of the default read-only checks, run `godseye security audit --deep`. Only offer alternatives if the user requests them:

1. `godseye security audit` (faster, non-probing)
2. `godseye security audit --json` (structured output)

Offer to apply Gods Eye safe defaults (numbered):

1. `godseye security audit --fix`

Be explicit that `--fix` only tightens Gods Eye defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Gods Eye version/update status (read-only)

As part of the default read-only checks, run `godseye update status`.
...
- Credential hygiene notes (location of Gods Eye creds, prefer disk encryption)
...
- Gods Eye security audit (re-run)
...
After Gods Eye install or first hardening pass, run at least one baseline audit and version check:

- `godseye security audit`
- `godseye security audit --deep`
- `godseye update status`

Ongoing monitoring is recommended. Use the Gods Eye cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `godseye cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `godseye update status`
...
Before creating, `godseye cron list` and match on exact `name`. If found, `godseye cron edit <id> ...`.
If not found, `godseye cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `godseye update status` (preferred for source checkouts and channels)
2. `npm view godseye version` (published npm version)

## Gods Eye command accuracy

Use only supported commands and flags:

- `godseye security audit [--deep] [--fix] [--json]`
- `godseye status` / `godseye status --deep`
- `godseye health --json`
- `godseye update status`
- `godseye cron add|list|runs|run`

Do not invent CLI flags or imply Gods Eye enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Gods Eye compaction:
