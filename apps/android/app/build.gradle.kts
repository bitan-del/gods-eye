...
val androidStoreFile = providers.gradleProperty("GODSEYE_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("GODSEYE_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("GODSEYE_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("GODSEYE_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set GODSEYE_ANDROID_STORE_FILE, " +
            "GODSEYE_ANDROID_STORE_PASSWORD, GODSEYE_ANDROID_KEY_ALIAS, and " +
            "GODSEYE_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/GodsEyeKit/Sources/GodsEyeKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.godseye.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "GODSEYE_ENABLE_SMS", "false")
            buildConfigField("boolean", "GODSEYE_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "GODSEYE_ENABLE_SMS", "true")
            buildConfigField("boolean", "GODSEYE_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "godseye-$versionName-$buildType.apk"
                    } else {
                        "godseye-$versionName-$flavorName-$buildType.apk"
                    }
