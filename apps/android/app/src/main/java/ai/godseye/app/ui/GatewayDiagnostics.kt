...
internal fun godsEyeAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this GodsEye Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `godseye devices list` should show a pending pairing request
    - if more signal is needed, ask for `godseye qr --json`, `godseye devices list`, and `godseye nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${godsEyeAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("GodsEye gateway diagnostics", report))
