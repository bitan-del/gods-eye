...
private const val NOTIFICATION_CHANNEL_BASE_ID = "godseye.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "GodsEye Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "GodsEye Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "GodsEye Active")
      }
