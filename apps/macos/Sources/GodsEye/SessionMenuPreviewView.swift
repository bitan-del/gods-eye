...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.godseye", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> GodsEyeSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: GodsEyeSessionPreviewEntry,
    ...
    private static func cache(payload: GodsEyeSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: GodsEyeChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [GodsEyeKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [GodsEyeKit.AnyCodable]) -> [GodsEyeChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(GodsEyeChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: GodsEyeChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: GodsEyeChatMessage) -> Bool {
    ...
    private static func toolNames(for message: GodsEyeChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: GodsEyeChatMessage) -> String? {
