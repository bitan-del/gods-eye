...
enum DebugActions {
    private static let verboseDefaultsKey = "godseye.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "godseye.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = GodsEyePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Gods Eye", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = GodsEyePaths.configURL
        guard
