...
enum GodsEyeLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = GodsEyeOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = GodsEyeFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        GodsEyeLogging.bootstrapIfNeeded()
        let label = GodsEyeLogging.makeLabel(subsystem: subsystem, category: category)
...
struct GodsEyeOSLogHandler: AppLogLevelBackedHandler {
...
struct GodsEyeFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = GodsEyeLogging.parseLabel(self.label)
