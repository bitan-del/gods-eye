    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("Allow these so GodsEye can notify and capture when needed.")
                    .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = GodsEyeLocationMode.off.rawValue
    ...
    private var locationMode: GodsEyeLocationMode {
        GodsEyeLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: GodsEyeLocationMode) async -> Bool {
