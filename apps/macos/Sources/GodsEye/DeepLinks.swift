...
private let deepLinkLogger = Logger(subsystem: "ai.godseye", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "GodsEye is paused", message: "Unpause GodsEye to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run GodsEye agent?", message: body) else { return }
        }
