...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.godseye/godseye.json. Prefer GODSEYE_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.godseye/godseye.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.godseye/godseye.json. Restart the gateway to apply."
        }
