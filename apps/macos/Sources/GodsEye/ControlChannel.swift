...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: GodsEyeProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.godseye", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: GodsEyeKit.AnyCodable]()) {
                $0[$1.key] = GodsEyeKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: GodsEyeProtocol.AnyCodable?) -> [String: GodsEyeProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: GodsEyeProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: GodsEyeKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: GodsEyeProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("godseye.control.heartbeat")
    static let controlAgentEvent = Notification.Name("godseye.control.agent")
}
