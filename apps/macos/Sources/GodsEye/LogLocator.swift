...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["GODSEYE_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/godseye")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("godseye-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("godseye-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/godseye/ (rolling or stdout), or nil if none exist.
