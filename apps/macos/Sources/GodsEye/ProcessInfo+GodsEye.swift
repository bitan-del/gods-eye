...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.godseye.mac`) even if the shipped
    ...
    {
        if environment["GODSEYE_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "godseye.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "godseye.nixMode") { return true }
