...
private let webChatSwiftLogger = Logger(subsystem: "ai.godseye", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "godseye.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: GodsEyeChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> GodsEyeChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(GodsEyeChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            GodsEyeChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return GodsEyeChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> GodsEyeChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                GodsEyeGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: GodsEyeProtocol.ModelChoice) -> GodsEyeChatModelChoice {
        GodsEyeChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<GodsEyeChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any GodsEyeChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = GodsEyeChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: GodsEyeChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<GodsEyeChatView>) -> NSViewController
    {
