...
enum CommandResolver {
    private static let projectRootDefaultsKey = "godseye.gatewayProjectRootPath"
    private static let helperName = "godseye"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let godseyeEntry = root.appendingPathComponent("godseye.mjs").path
        if FileManager().isReadableFile(atPath: godseyeEntry) { return godseyeEntry }
        let binEntry = root.appendingPathComponent("bin/godseye.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/godseye")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let godseyePaths = self.godseyeManagedPaths(home: home)
        if !godseyePaths.isEmpty {
            extras.insert(contentsOf: godseyePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + godseyePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func godseyeManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".godseye"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func godseyeExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectGodsEyeExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("godseye.mjs").path,
            root.appendingPathComponent("bin/godseye.js").path,
        ]
    ...
    static func hasAnyGodsEyeInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.godseyeExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func godseyeNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let godseyePath = self.projectGodsEyeExecutable(projectRoot: root) {
            return [godseyePath, subcommand] + extraArgs
        }
        if let godseyePath = self.godseyeExecutable(searchPaths: searchPaths) {
            return [godseyePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "godseye", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            godseye entrypoint missing (looked for dist/index.js or godseye.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real godseye CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/godseye"
        ...
        \(cliSection)
        \(projectSection)
        if command -v godseye >/dev/null 2>&1; then
          CLI="$(command -v godseye)"
          godseye \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/godseye.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/godseye.mjs"
            node "$PRJ/godseye.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/godseye.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/godseye.js"
            node "$PRJ/bin/godseye.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent godseye"
          pnpm --silent godseye \(quotedArgs);
        else
          echo "godseye CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? GodsEyeConfigFile.loadDict()
