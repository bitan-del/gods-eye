...
struct GodsEyeMacCLI {
    static func main() async {
        ...
        default:
            fputs("godseye-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      godseye-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      godseye-mac discover [--timeout <ms>] [--json] [--include-local]
      godseye-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      godseye-mac connect
      godseye-mac connect --url ws://127.0.0.1:18789 --json
      godseye-mac discover --timeout 3000 --json
      godseye-mac wizard --mode local
    """)
}
