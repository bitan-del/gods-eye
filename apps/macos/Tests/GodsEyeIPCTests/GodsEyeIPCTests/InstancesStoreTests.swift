...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: GodsEyeProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: GodsEyeProtocol.AnyCodable] = [
            "presence": .init([GodsEyeProtocol.AnyCodable(entry)]),
        ]
        let payload = GodsEyeProtocol.AnyCodable(payloadMap)
