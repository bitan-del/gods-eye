...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["godseye"]
        let text = "hey godseye"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey godseye do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["godseye"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "godseye", start: 0.2, duration: 0.1, range: transcript.range(of: "godseye")),
