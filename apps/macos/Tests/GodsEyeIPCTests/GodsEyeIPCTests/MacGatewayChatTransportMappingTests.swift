...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: GodsEyeProtocol.AnyCodable(["ok": GodsEyeProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: GodsEyeProtocol.AnyCodable(["ok": GodsEyeProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = GodsEyeProtocol.AnyCodable([
            "runId": GodsEyeProtocol.AnyCodable("run-1"),
            "sessionKey": GodsEyeProtocol.AnyCodable("main"),
            "state": GodsEyeProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: GodsEyeProtocol.AnyCodable(["a": GodsEyeProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
