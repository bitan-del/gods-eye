...
@MainActor
public struct GodsEyeChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: GodsEyeChatViewModel
    ...
    public init(
        viewModel: GodsEyeChatViewModel,
    ...
    private var visibleMessages: [GodsEyeChatMessage] {
        let base: [GodsEyeChatMessage]
    ...
    private func mergeToolResults(in messages: [GodsEyeChatMessage]) -> [GodsEyeChatMessage] {
    ...
    private func isToolResultMessage(_ message: GodsEyeChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: GodsEyeChatMessage) -> Bool {
    ...
    private func primaryText(in message: GodsEyeChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: GodsEyeChatMessage) -> Bool {
    ...
    private func toolCalls(in message: GodsEyeChatMessage) -> [GodsEyeChatMessageContent] {
    ...
    private func inlineToolResults(in message: GodsEyeChatMessage) -> [GodsEyeChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: GodsEyeChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: GodsEyeChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(GodsEyeChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
