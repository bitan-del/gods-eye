...
struct ChatMessageBubble: View {
    let message: GodsEyeChatMessage
    let style: GodsEyeChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? GodsEyeChatTheme.userText : GodsEyeChatTheme.assistantText
    ...
    private var inlineAttachments: [GodsEyeChatMessageContent] {
    ...
    private var toolCalls: [GodsEyeChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [GodsEyeChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? GodsEyeChatTheme.userBubble
        }
        if self.style == .onboarding {
            return GodsEyeChatTheme.onboardingAssistantBubble
        }
        return GodsEyeChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return GodsEyeChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: GodsEyeChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? GodsEyeChatTheme.userText : GodsEyeChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: GodsEyeChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(GodsEyeChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [GodsEyeChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: GodsEyeChatTheme.assistantText)
            }
        }
    }
}
