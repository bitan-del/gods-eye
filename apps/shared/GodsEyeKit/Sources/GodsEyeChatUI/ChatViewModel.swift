...
private let chatUILogger = Logger(subsystem: "ai.godseye", category: "GodsEyeChatUI")

@MainActor
@Observable
public final class GodsEyeChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [GodsEyeChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [GodsEyeChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [GodsEyePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [GodsEyeChatPendingToolCall] = []
    public private(set) var sessions: [GodsEyeChatSessionEntry] = []
    private let transport: any GodsEyeChatTransport
    private var sessionDefaults: GodsEyeChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: GodsEyeChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any GodsEyeChatTransport,
    ...
    public var sessionChoices: [GodsEyeChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [GodsEyeChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: GodsEyePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [GodsEyeChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: GodsEyeChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: GodsEyeChatMessage) -> GodsEyeChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> GodsEyeChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return GodsEyeChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: GodsEyeChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: GodsEyeChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: GodsEyeChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [GodsEyeChatMessage],
        incoming: [GodsEyeChatMessage]) -> [GodsEyeChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [GodsEyeChatMessage]) -> [GodsEyeChatMessage] {
        var result: [GodsEyeChatMessage] = []
    ...
    private static func dedupeKey(for message: GodsEyeChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [GodsEyeChatMessageContent] = [
            GodsEyeChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> GodsEyeChatAttachmentPayload in
            GodsEyeChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> GodsEyeChatSessionEntry {
        GodsEyeChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = GodsEyeChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: GodsEyeChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: GodsEyeChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: GodsEyeAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = GodsEyeChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> GodsEyePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
