...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> GodsEyeChatHistoryPayload
{
    GodsEyeChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> GodsEyeChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> GodsEyeChatSessionEntry
{
    GodsEyeChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> GodsEyeChatModelChoice {
    GodsEyeChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [GodsEyeChatHistoryPayload],
    sessionsResponses: [GodsEyeChatSessionsListResponse] = [],
    modelResponses: [[GodsEyeChatModelChoice]] = [],
    ...
    -> (TestChatTransport, GodsEyeChatViewModel)
{
...
private func sendUserMessage(_ vm: GodsEyeChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, GodsEyeChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [GodsEyeChatHistoryPayload]
    private let sessionsResponses: [GodsEyeChatSessionsListResponse]
    private let modelResponses: [[GodsEyeChatModelChoice]]
    ...
    private let stream: AsyncStream<GodsEyeChatTransportEvent>
    private let continuation: AsyncStream<GodsEyeChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<GodsEyeChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> GodsEyeChatHistoryPayload {
        ...
        return self.historyResponses.last ?? GodsEyeChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [GodsEyeChatAttachmentPayload]) async throws -> GodsEyeChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return GodsEyeChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> GodsEyeChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? GodsEyeChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: GodsEyeChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = GodsEyeChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = GodsEyeChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: GodsEyeChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = GodsEyeChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: GodsEyeChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(GodsEyeChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == GodsEyeChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = GodsEyeChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: GodsEyeChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = GodsEyeChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = GodsEyeChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = GodsEyeChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { GodsEyeChatViewModel(sessionKey: "main", transport: transport) }
