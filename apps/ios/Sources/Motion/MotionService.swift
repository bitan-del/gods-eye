...
final class MotionService: MotionServicing {
    func activities(params: GodsEyeMotionActivityParams) async throws -> GodsEyeMotionActivityPayload {
        ...
        let mapped: [GodsEyeMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return GodsEyeMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: GodsEyePedometerParams) async throws -> GodsEyePedometerPayload {
        ...
        let payload: GodsEyePedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = GodsEyePedometerPayload(
