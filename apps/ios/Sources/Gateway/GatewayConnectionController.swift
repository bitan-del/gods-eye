...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [GodsEyeCapability.canvas.rawValue, GodsEyeCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(GodsEyeCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(GodsEyeCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = GodsEyeLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(GodsEyeCapability.location.rawValue) }

        caps.append(GodsEyeCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(GodsEyeCapability.watch.rawValue)
        }
        caps.append(GodsEyeCapability.photos.rawValue)
        caps.append(GodsEyeCapability.contacts.rawValue)
        caps.append(GodsEyeCapability.calendar.rawValue)
        caps.append(GodsEyeCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(GodsEyeCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            GodsEyeScreenCommand.record.rawValue,
            GodsEyeSystemCommand.notify.rawValue,
            GodsEyeChatCommand.push.rawValue,
        ...
        if caps.contains(GodsEyeCapability.camera.rawValue) {
            commands.append(GodsEyeCameraCommand.list.rawValue)
            commands.append(GodsEyeCameraCommand.snap.rawValue)
            commands.append(GodsEyeCameraCommand.clip.rawValue)
        }
        if caps.contains(GodsEyeCapability.location.rawValue) {
            commands.append(GodsEyeLocationCommand.get.rawValue)
        }
        if caps.contains(GodsEyeCapability.device.rawValue) {
            commands.append(GodsEyeDeviceCommand.status.rawValue)
            commands.append(GodsEyeDeviceCommand.info.rawValue)
        }
        if caps.contains(GodsEyeCapability.watch.rawValue) {
            commands.append(GodsEyeWatchCommand.status.rawValue)
            commands.append(GodsEyeWatchCommand.notify.rawValue)
        }
        if caps.contains(GodsEyeCapability.photos.rawValue) {
            commands.append(GodsEyePhotosCommand.latest.rawValue)
        }
        if caps.contains(GodsEyeCapability.contacts.rawValue) {
            commands.append(GodsEyeContactsCommand.search.rawValue)
            commands.append(GodsEyeContactsCommand.add.rawValue)
        }
        if caps.contains(GodsEyeCapability.calendar.rawValue) {
            commands.append(GodsEyeCalendarCommand.events.rawValue)
            commands.append(GodsEyeCalendarCommand.add.rawValue)
        }
        if caps.contains(GodsEyeCapability.reminders.rawValue) {
            commands.append(GodsEyeRemindersCommand.list.rawValue)
            commands.append(GodsEyeRemindersCommand.add.rawValue)
        }
        if caps.contains(GodsEyeCapability.motion.rawValue) {
            commands.append(GodsEyeMotionCommand.activity.rawValue)
            commands.append(GodsEyeMotionCommand.pedometer.rawValue)
        }
