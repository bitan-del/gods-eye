...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: GodsEyeWatchNotifyParams) -> GodsEyeWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [GodsEyeWatchAction]?,
        kind: String?,
        promptId: String?) -> [GodsEyeWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> GodsEyeWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return GodsEyeWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                GodsEyeWatchAction(id: "approve", label: "Approve"),
                GodsEyeWatchAction(id: "decline", label: "Decline", style: "destructive"),
                GodsEyeWatchAction(id: "open_phone", label: "Open iPhone"),
                GodsEyeWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            GodsEyeWatchAction(id: "done", label: "Done"),
            GodsEyeWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            GodsEyeWatchAction(id: "open_phone", label: "Open iPhone"),
            GodsEyeWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: GodsEyeWatchRisk?,
        priority: GodsEyeNotificationPriority?) -> GodsEyeWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: GodsEyeNotificationPriority?,
        risk: GodsEyeWatchRisk?) -> GodsEyeNotificationPriority?
    {
