...
final class ContactsService: ContactsServicing {
    ...
    func search(params: GodsEyeContactsSearchParams) async throws -> GodsEyeContactsSearchPayload {
        ...
        return GodsEyeContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: GodsEyeContactsAddParams) async throws -> GodsEyeContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return GodsEyeContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return GodsEyeContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> GodsEyeContactPayload {
        GodsEyeContactPayload(
