...
struct IOSGatewayChatTransport: GodsEyeChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.godseye", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> GodsEyeChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(GodsEyeChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> GodsEyeChatHistoryPayload {
        ...
        return try JSONDecoder().decode(GodsEyeChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [GodsEyeChatAttachmentPayload]) async throws -> GodsEyeChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [GodsEyeChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(GodsEyeChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(GodsEyeGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GodsEyeGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: GodsEyeChatEventPayload.self)
                        {
