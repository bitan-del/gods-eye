...
@MainActor
final class GodsEyeAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.godseye.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.godseye.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.godseye.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "godseye.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "godseye.watch.promptId"
    static let sessionKeyKey = "godseye.watch.sessionKey"
    static let actionPrimaryIDKey = "godseye.watch.action.primary.id"
    static let actionPrimaryLabelKey = "godseye.watch.action.primary.label"
    static let actionSecondaryIDKey = "godseye.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "godseye.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "godseye.watch.action.primary"
    static let actionSecondaryIdentifier = "godseye.watch.action.secondary"
    static let actionIdentifierPrefix = "godseye.watch.action."
    static let actionIDKeyPrefix = "godseye.watch.action.id."
    static let actionLabelKeyPrefix = "godseye.watch.action.label."
    static let categoryPrefix = "godseye.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: GodsEyeWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> GodsEyeWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return GodsEyeWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Gods Eye" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [GodsEyeWatchAction]) -> [UNNotificationAction] {
...
struct GodsEyeApp: App {
    ...
    @UIApplicationDelegateAdaptor(GodsEyeAppDelegate.self) private var appDelegate
...
extension GodsEyeApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("GodsEye: installing uncaught exception handler")
