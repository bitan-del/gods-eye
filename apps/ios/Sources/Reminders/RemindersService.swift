...
final class RemindersService: RemindersServicing {
    func list(params: GodsEyeRemindersListParams) async throws -> GodsEyeRemindersListPayload {
        ...
        let payload: [GodsEyeReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return GodsEyeRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: GodsEyeRemindersAddParams) async throws -> GodsEyeRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = GodsEyeReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return GodsEyeRemindersAddPayload(reminder: payload)
    }
