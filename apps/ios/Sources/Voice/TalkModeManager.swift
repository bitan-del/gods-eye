...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__GODSEYE_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<GodsEyeTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.godseye", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = GodsEyeTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> GodsEyeTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return GodsEyeTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> GodsEyeTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> GodsEyeTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return GodsEyeTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> GodsEyeTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = GodsEyeTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: GodsEyeTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: GodsEyeAgentEventPayload.self
            ) else {
                continue
            }
