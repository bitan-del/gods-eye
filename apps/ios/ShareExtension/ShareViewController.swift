...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.godseye.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to GodsEye", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to GodsEye gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to GodsEye.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "GodsEyeShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "GodsEye is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "GodsEye Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("godseye-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
