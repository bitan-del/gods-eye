...
const clearPlannerShardEnv = (env) => {
  const nextEnv = { ...env };
  delete nextEnv.GODSEYE_TEST_SHARDS;
  delete nextEnv.GODSEYE_TEST_SHARD_INDEX;
  delete nextEnv.GODSEYE_TEST_FORCE_THREADS;
  delete nextEnv.GODSEYE_TEST_FORCE_FORKS;
  delete nextEnv.GODSEYE_TEST_DISABLE_THREAD_EXPANSION;
  delete nextEnv.GODSEYE_TEST_SHOW_POOL_DECISION;
  return nextEnv;
};
...
describe("scripts/test-parallel lane planning", () => {
  ...
  it("recycles default local unit-fast runs into bounded batches", () => {
    ...
    const output = execFileSync("node", ["scripts/test-parallel.mjs", "--plan"], {
      ...
      env: {
        ...clearPlannerShardEnv(process.env),
        CI: "",
        GODSEYE_TEST_UNIT_FAST_LANES: "1",
        GODSEYE_TEST_UNIT_FAST_BATCH_TARGET_MS: "1",
      },
      encoding: "utf8",
    });
    ...
    const output = execFileSync(
      ...
      ["scripts/test-parallel.mjs", "--plan", "--surface", "unit", "--surface", "extensions"],
      {
        ...
        env: {
          ...
          GITHUB_ACTIONS: "",
          RUNNER_OS: "macOS",
          GODSEYE_TEST_HOST_CPU_COUNT: "10",
          GODSEYE_TEST_HOST_MEMORY_GIB: "64",
          GODSEYE_TEST_LOAD_AWARE: "0",
        },
        encoding: "utf8",
      },
    );
  ...
  it("prints the planner-backed CI manifest as JSON", () => {
    ...
    const output = execFileSync("node", ["scripts/test-parallel.mjs", "--ci-manifest"], {
      ...
      env: {
        ...
        GODSEYE_CI_DOCS_ONLY: "false",
        GODSEYE_CI_DOCS_CHANGED: "false",
        GODSEYE_CI_RUN_NODE: "true",
        GODSEYE_CI_RUN_MACOS: "true",
        GODSEYE_CI_RUN_ANDROID: "false",
        GODSEYE_CI_RUN_WINDOWS: "true",
        GODSEYE_CI_RUN_SKILLS_PYTHON: "false",
        GODSEYE_CI_HAS_CHANGED_EXTENSIONS: "false",
        GODSEYE_CI_CHANGED_EXTENSIONS_MATRIX: '{"include":[]}',
      },
      encoding: "utf8",
    });
  ...
  it("writes CI workflow outputs in ci mode", () => {
    const repoRoot = path.resolve(import.meta.dirname, "../..");
    const outputPath = path.join(os.tmpdir(), `godseye-ci-output-${Date.now()}.txt`);
  ...
  it("writes install-smoke outputs in install-smoke mode", () => {
    const repoRoot = path.resolve(import.meta.dirname, "../..");
    const outputPath = path.join(os.tmpdir(), `godseye-install-output-${Date.now()}.txt`);
    ...
    execFileSync("node", ["scripts/ci-write-manifest-outputs.mjs", "--workflow", "install-smoke"], {
      ...
      env: {
        ...
        GITHUB_OUTPUT: outputPath,
        GODSEYE_CI_DOCS_ONLY: "false",
        GODSEYE_CI_RUN_CHANGED_SMOKE: "true",
      },
      encoding: "utf8",
    });
  ...
  it("writes bun outputs in ci-bun mode", () => {
    const repoRoot = path.resolve(import.meta.dirname, "../..");
    const outputPath = path.join(os.tmpdir(), `godseye-bun-output-${Date.now()}.txt`);
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...
          GITHUB_ACTIONS: "",
          RUNNER_OS: "Linux",
          GODSEYE_TEST_HOST_CPU_COUNT: "16",
          GODSEYE_TEST_HOST_MEMORY_GIB: "128",
        },
        encoding: "utf8",
      },
    );
  ...
  it("rejects explicit existing files that are not known test files", () => {
    const repoRoot = path.resolve(import.meta.dirname, "../..");
    const tempFilePath = path.join(os.tmpdir(), `godseye-non-test-${Date.now()}.ts`);
