...
process.env.GODSEYE_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime GodsEye modules.
const testEnv = withIsolatedTestHome();

installProcessWarningFilter();

const REGISTRY_STATE = Symbol.for("godseye.pluginRegistryState");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: GodsEyeConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: GodsEyeConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
