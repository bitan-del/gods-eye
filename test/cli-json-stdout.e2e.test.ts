  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.GODSEYE_HOME;
        delete env.GODSEYE_STATE_DIR;
        delete env.GODSEYE_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "godseye.mjs");
