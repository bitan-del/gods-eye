...
describe("resolveVitestIsolation", () => {
  ...
  it("restores isolate mode when explicitly requested", () => {
    expect(resolveVitestIsolation({ GODSEYE_TEST_ISOLATE: "1" })).toBe(true);
    expect(resolveVitestIsolation({ GODSEYE_TEST_NO_ISOLATE: "0" })).toBe(true);
    expect(resolveVitestIsolation({ GODSEYE_TEST_NO_ISOLATE: "false" })).toBe(true);
  });
});
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from GODSEYE_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-vitest-channels-"));
    try {
      ...
      const config = createChannelsVitestConfig({
        GODSEYE_VITEST_INCLUDE_FILE: includeFile,
      });
