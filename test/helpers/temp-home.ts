...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked GODSEYE_HOME.
  delete process.env.GODSEYE_HOME;
  process.env.GODSEYE_STATE_DIR = path.join(base, ".godseye");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "godseye-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".godseye", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
