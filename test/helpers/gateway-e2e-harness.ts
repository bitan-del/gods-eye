...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `godseye-e2e-${name}-`));
  const configDir = path.join(homeDir, ".godseye");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          GODSEYE_SKIP_PROVIDERS: "1",
          GODSEYE_SKIP_GMAIL_WATCHER: "1",
          GODSEYE_SKIP_CRON: "1",
          GODSEYE_SKIP_BROWSER_CONTROL_SERVER: "1",
          GODSEYE_SKIP_CANVAS_HOST: "1",
          GODSEYE_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
