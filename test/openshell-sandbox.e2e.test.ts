...
const GODSEYE_OPENSHELL_E2E = process.env.GODSEYE_E2E_OPENSHELL === "1";
const GODSEYE_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const GODSEYE_OPENSHELL_COMMAND = process.env.GODSEYE_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "godseye-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && GODSEYE_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: GODSEYE_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(GODSEYE_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-openshell-e2e-"));
      ...
      const gatewayName = `godseye-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `godseye-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: GODSEYE_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("godseye-openshell-e2e");
