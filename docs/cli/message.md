---
summary: "CLI reference for `godseye message` (send + channel actions)"
read_when:
...
```
godseye message <subcommand> [flags]
```
...
- On cache miss, Gods Eye will attempt a live directory lookup when the provider supports it.
...
- `godseye message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Gods Eye does not force a `default` account SecretRef scope
...
```
godseye message send --channel discord \
...
```
godseye message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
godseye message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
godseye message poll --channel msteams \
...
```
godseye message react --channel slack \
...
```
godseye message react --channel signal \
...
```
godseye message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
godseye message send --channel telegram --target @mychat \
