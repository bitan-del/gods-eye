---
summary: "CLI reference for `godseye plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
godseye plugins inspect <id>
godseye plugins enable <id>
godseye plugins disable <id>
godseye plugins uninstall <id>
godseye plugins doctor
godseye plugins update <id>
godseye plugins update --all
godseye plugins marketplace list <marketplace>
```

Bundled plugins ship with Gods Eye but start disabled. Use `plugins enable` to
activate them.

Native Gods Eye plugins must ship `godseye.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: godseye` or `Format: bundle`. Verbose list/info
...
godseye plugins install clawhub:<package>              # ClawHub only
godseye plugins install <package> --pin                # pin version
godseye plugins install <path>                         # local path
godseye plugins install <plugin>@<marketplace>         # marketplace
godseye plugins install <plugin> --marketplace <name>  # marketplace (explicit)
```
...
`godseye.hooks` in `package.json`. Use `godseye hooks` for filtered hook
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Gods Eye
...
```bash
godseye plugins install clawhub:godseye-codex-app-server
godseye plugins install clawhub:godseye-codex-app-server@1.2.3
```

Gods Eye now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
godseye plugins install godseye-codex-app-server
```

Gods Eye downloads the package archive from ClawHub, checks the advertised
...
```bash
godseye plugins marketplace list <marketplace-name>
godseye plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
godseye plugins install <plugin-name> --marketplace <marketplace-name>
godseye plugins install <plugin-name> --marketplace <owner/repo>
godseye plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Gods Eye accepts relative path sources from
that repo and rejects external git, GitHub, URL/archive, and absolute-path
plugin sources from remote manifests.

For local paths and archives, Gods Eye auto-detects:

- native Gods Eye plugins (`godseye.plugin.json`)
...
```bash
godseye plugins install -l ./my-plugin
```
...
```bash
godseye plugins uninstall <id>
godseye plugins uninstall <id> --dry-run
godseye plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$GODSEYE_STATE_DIR/extensions/<id>`). Use
...
```bash
godseye plugins update <id-or-npm-spec>
godseye plugins update --all
godseye plugins update <id-or-npm-spec> --dry-run
godseye plugins update @godseye/voice-call@beta
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Gods Eye reuses the recorded install spec for that
...
or exact version. Gods Eye resolves that package name back to the tracked plugin
...
```bash
godseye plugins inspect <id>
godseye plugins inspect <id> --json
```
