---
summary: "CLI reference for `godseye onboard` (interactive onboarding)"
read_when:
...
```bash
godseye onboard
godseye onboard --flow quickstart
godseye onboard --flow manual
godseye onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`GODSEYE_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `godseye gateway run`.
...
- Fastest first chat: `godseye dashboard` (Control UI, no channel setup).
