...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Gods Eye Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`godseye acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Gods Eye gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
godseye acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
godseye acp --url wss://gateway-host:18789 --token-file ~/.godseye/gateway.token

# Attach to an existing session key
godseye acp --session agent:main:main

# Attach by label (must already exist)
godseye acp --session-label "support inbox"

# Reset the session key before the first prompt
godseye acp --session agent:main:main --reset-session
```
...
godseye acp client --server-args --url wss://gateway-host:18789 --token-file ~/.godseye/gateway.token

# Override the server command (default: godseye)
godseye acp client --server "node" --server-args godseye.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Gods Eye Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `godseye acp` over stdio.

Example config (persisted):

```bash
godseye config set gateway.remote.url wss://gateway-host:18789
godseye config set gateway.remote.token <token>
```
...
```bash
godseye acp --session agent:main:main
godseye acp --session agent:design:main
godseye acp --session agent:qa:bug-123
```
...
Gods Eye bot over ACP, use `acpx` with its built-in `godseye` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx godseye` at `godseye acp`.
3. Target the Gods Eye session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Gods Eye ACP session
acpx godseye exec "Summarize the active Gods Eye session state."

# Persistent named session for follow-up turns
acpx godseye sessions ensure --name codex-bridge
acpx godseye -s codex-bridge --cwd /path/to/repo \
  "Ask my Gods Eye work agent for recent context relevant to this repo."
```

If you want `acpx godseye` to target a specific Gateway and session key every
time, override the `godseye` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "godseye": {
      "command": "env GODSEYE_HIDE_BANNER=1 GODSEYE_SUPPRESS_NOTES=1 godseye acp --url ws://127.0.0.1:18789 --token-file ~/.godseye/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Gods Eye checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env GODSEYE_HIDE_BANNER=1 GODSEYE_SUPPRESS_NOTES=1 node godseye.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Gods Eye agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Gods Eye ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`GODSEYE_GATEWAY_TOKEN`, `GODSEYE_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`GODSEYE_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `GODSEYE_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `godseye acp client` sets `GODSEYE_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `godseye`).
