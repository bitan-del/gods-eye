---
summary: "CLI reference for `godseye configure` (interactive configuration prompts)"
read_when:
...
Tip: `godseye config` without a subcommand opens the same wizard. Use
`godseye config get|set|unset` for non-interactive edits.
...
```bash
godseye configure
godseye configure --section model --section channels
```
