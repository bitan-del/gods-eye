---
summary: "CLI reference for `godseye secrets` (reload, audit, configure, apply)"
read_when:
...
Use `godseye secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan Gods Eye state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `godseye.json` refs)
...
```bash
godseye secrets audit
godseye secrets audit --check
godseye secrets audit --json
godseye secrets audit --allow-exec
```
...
```bash
godseye secrets configure
godseye secrets configure --plan-out /tmp/godseye-secrets-plan.json
godseye secrets configure --apply --yes
godseye secrets configure --providers-only
godseye secrets configure --skip-provider-setup
godseye secrets configure --agent ops
godseye secrets configure --json
```
...
- `configure` targets secret-bearing fields in `godseye.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Gods Eye fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
godseye secrets apply --from /tmp/godseye-secrets-plan.json --json
```
...
- `godseye.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.godseye/.env` known secret keys whose values were migrated
