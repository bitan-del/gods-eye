...
read_when:
  - You want to use Qwen (Alibaba Cloud Model Studio) with Gods Eye
...
```bash
# China endpoint
godseye onboard --auth-choice modelstudio-standard-api-key-cn

# Global/Intl endpoint
godseye onboard --auth-choice modelstudio-standard-api-key
```
...
```bash
# China endpoint
godseye onboard --auth-choice modelstudio-api-key-cn

# Global/Intl endpoint
godseye onboard --auth-choice modelstudio-api-key
```
...
`~/.godseye/.env` or via `env.shellEnv`).
