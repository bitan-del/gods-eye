---
summary: "Use OpenAI via API keys or Codex subscription in Gods Eye"
read_when:
  - You want to use OpenAI models in Gods Eye
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Gods Eye.
...
```bash
godseye onboard --auth-choice openai-api-key
# or non-interactive
godseye onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Gods Eye forwards both through the `openai/*` Responses path.
Gods Eye intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Gods Eye does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Gods Eye.
...
godseye onboard --auth-choice openai-codex

# Or run OAuth directly
godseye models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Gods Eye
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Gods Eye also supports:

- `openai-codex/gpt-5.3-codex-spark`

Gods Eye treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Gods Eye also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Gods Eye uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Gods Eye also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Gods Eye enables it by default for
...
Gods Eye, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Gods Eye exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Gods Eye applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Gods Eye now auto-enables OpenAI server-side
