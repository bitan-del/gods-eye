---
summary: "Use Venice AI privacy-focused models in Gods Eye"
read_when:
  - You want privacy-focused inference in Gods Eye
...
```bash
godseye onboard --auth-choice venice-api-key
```
...
```bash
godseye agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Gods Eye shows all available Venice models. Pick based on your needs:
...
```bash
godseye models set venice/kimi-k2-5
godseye models set venice/claude-opus-4-6
```

List all available models:

```bash
godseye models list | grep venice
```

## Configure via `godseye configure`

1. Run `godseye configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Gods Eye automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
godseye agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
godseye agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
godseye agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
godseye agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
godseye agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `godseye models list` to see currently available models. Some models may be temporarily offline.
