---
summary: "Use OpenCode Zen and Go catalogs with Gods Eye"
read_when:
...
OpenCode exposes two hosted catalogs in Gods Eye:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Gods Eye keeps the runtime provider ids
...
godseye onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
godseye onboard --auth-choice opencode-go
godseye onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
