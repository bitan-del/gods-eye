---
summary: "Use Synthetic's Anthropic-compatible API in Gods Eye"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Gods Eye registers it as the
...
```bash
godseye onboard --auth-choice synthetic-api-key
```
...
Note: Gods Eye's Anthropic client appends `/v1` to the base URL, so use
