...
read_when:
  - You want to use Groq with Gods Eye
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Gods Eye connects
...
media-understanding provider, Gods Eye uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.godseye/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `godseye models list | grep groq`
