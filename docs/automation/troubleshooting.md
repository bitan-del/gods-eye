...
godseye system heartbeat last
godseye logs --follow
godseye config get agents.defaults.heartbeat
godseye channels status --probe
```
...
```bash
godseye config get agents.defaults.heartbeat.activeHours
godseye config get agents.defaults.heartbeat.activeHours.timezone
godseye config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
godseye cron list
godseye logs --follow
```
