...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `godseye hooks`, while hook-pack installation and updates now go through `godseye plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Gods Eye. See [Webhook Hooks](/automation/webhook) or use `godseye webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `godseye hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Gods Eye keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Gods Eye's behavior without modifying core code

## Getting Started

### Bundled Hooks

Gods Eye ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.godseye/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.godseye/logs/commands.log`
...
During onboarding (`godseye onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Gods Eye requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Gods Eye; located at `<godseye>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.godseye/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `godseye.hooks` in
`package.json`. Install them with:

```bash
godseye plugins install <path-or-spec>
```
...
Hook packs can ship dependencies; they will be installed under `~/.godseye/hooks/<id>`.
Each `godseye.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `godseye plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
The `metadata.godseye` object supports:
...
```typescript
{
  ...
  cfg: Gods EyeConfig            // Current gateway config
}
```
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Gods Eye persists them.
...
- **Managed hooks** (`~/.godseye/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.godseye/hooks/my-hook
cd ~/.godseye/hooks/my-hook
```
...
metadata: { "godseye": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
godseye hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.godseye/workspace`)
...
```yaml
metadata: { "godseye": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "godseye": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.godseye/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.godseye/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.godseye/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/bitan-del/gods-eye/tree/main/src/hooks/bundled)
