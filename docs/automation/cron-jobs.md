...
- Jobs persist under `~/.godseye/cron/` so restarts don’t lose schedules.
...
- For upgrades, `godseye doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
godseye cron list
godseye cron run <job-id>
godseye cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.godseye/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `godseye cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Gods Eye applies a
...
If `delivery` is omitted for isolated jobs, Gods Eye defaults to `announce`.
...
- CLI equivalents: `godseye cron add --light-context ...` and `godseye cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `godseye cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.godseye/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.godseye/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Gods Eye classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Gods Eye archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `godseye cron runs` and adjust retention before logs become large
...
godseye cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
godseye cron edit <jobId> --agent ops
godseye cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
godseye cron run <jobId>
godseye cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `godseye cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
godseye cron edit <jobId> \
...
```bash
godseye system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`godseye system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `GODSEYE_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Gods Eye applies exponential retry backoff for recurring jobs after consecutive errors:
