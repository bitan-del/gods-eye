...
read_when:
  ...
  - Wiring external systems into Gods Eye
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Gods Eye config directory (typically `~/.godseye/hooks/transforms`).
...
- `godseye webhooks gmail setup` writes `hooks.gmail` config for `godseye webhooks gmail run`.
