...
你可以在 `~/.godseye/godseye.json` 中覆盖此设置：
...
所有日志配置都在 `~/.godseye/godseye.json` 的 `logging` 下。
...
- Gods Eye 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `godseye plugins enable diagnostics-otel` 启用插件。
...
- `godseye.tokens`（计数器，属性：`godseye.token`、`godseye.channel`、`godseye.provider`、`godseye.model`）
- `godseye.cost.usd`（计数器，属性：`godseye.channel`、`godseye.provider`、`godseye.model`）
- `godseye.run.duration_ms`（直方图，属性：`godseye.channel`、`godseye.provider`、`godseye.model`）
- `godseye.context.tokens`（直方图，属性：`godseye.context`、`godseye.channel`、`godseye.provider`、`godseye.model`）

消息流：

- `godseye.webhook.received`（计数器，属性：`godseye.channel`、`godseye.webhook`）
- `godseye.webhook.error`（计数器，属性：`godseye.channel`、`godseye.webhook`）
- `godseye.webhook.duration_ms`（直方图，属性：`godseye.channel`、`godseye.webhook`）
- `godseye.message.queued`（计数器，属性：`godseye.channel`、`godseye.source`）
- `godseye.message.processed`（计数器，属性：`godseye.channel`、`godseye.outcome`）
- `godseye.message.duration_ms`（直方图，属性：`godseye.channel`、`godseye.outcome`）

队列 + 会话：

- `godseye.queue.lane.enqueue`（计数器，属性：`godseye.lane`）
- `godseye.queue.lane.dequeue`（计数器，属性：`godseye.lane`）
- `godseye.queue.depth`（直方图，属性：`godseye.lane` 或 `godseye.channel=heartbeat`）
- `godseye.queue.wait_ms`（直方图，属性：`godseye.lane`）
- `godseye.session.state`（计数器，属性：`godseye.state`、`godseye.reason`）
- `godseye.session.stuck`（计数器，属性：`godseye.state`）
- `godseye.session.stuck_age_ms`（直方图，属性：`godseye.state`）
- `godseye.run.attempt`（计数器，属性：`godseye.attempt`）
...
- `godseye.webhook.error`
  - `godseye.channel`、`godseye.webhook`、`godseye.chatId`、`godseye.error`
- `godseye.message.processed`
  - `godseye.channel`、`godseye.outcome`、`godseye.chatId`、`godseye.messageId`、`godseye.sessionKey`、`godseye.sessionId`、`godseye.reason`
- `godseye.session.stuck`
  - `godseye.state`、`godseye.ageMs`、`godseye.queueDepth`、`godseye.sessionKey`、`godseye.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `godseye doctor`。
