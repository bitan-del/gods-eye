...
read_when:
  ...
  - 你想部署到云平台
  - 你需要更新、迁移或卸载
summary: 安装 Gods Eye —— 安装脚本、npm/pnpm、从源码、Docker 等
title: 安装
x-i18n:
...
<Note>
在 Windows 上，我们强烈建议你在 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 下运行 Gods Eye。
</Note>

## 安装方法

<Tip>
**安装脚本** 是安装 Gods Eye 的推荐方式。它会一步完成 Node 检测、安装和新手引导。
</Tip>

<Warning>
对于 VPS/云主机，尽量避免使用第三方“一键式”市场镜像。优先选择干净的基础 OS 镜像（例如 Ubuntu LTS），然后使用安装脚本自行安装 Gods Eye。
</Warning>
...
<AccordionGroup>
  <Accordion title="安装脚本" icon="rocket" defaultOpen>
    ...
    <Tabs>
      <Tab title="macOS / Linux / WSL2">
        ```bash
        curl -fsSL https://gods-eye.org/install.sh | bash -s -- --no-onboard
        ```
      </Tab>
      <Tab title="Windows (PowerShell)">
        ```powershell
  ...
  <Accordion title="npm / pnpm" icon="package">
    如果你已经自行管理 Node，我们推荐使用 Node 24。出于兼容性考虑，Gods Eye 仍支持 Node 22 LTS，目前为 `22.16+`：
    ...
    <Tabs>
      <Tab title="npm">
        ...
        <Accordion title="sharp 构建错误？">
          ...
          ```bash
          SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g godseye@latest
          ```
      ...
      <Tab title="pnpm">
        ...
        pnpm approve-builds -g        # 批准 godseye、node-llama-cpp、sharp 等
        godseye onboard --install-daemon
        ```
    ...
    ```bash
    npm install -g github:godseye/godseye#main
    ```

    ```bash
    pnpm add -g github:godseye/godseye#main
    ```
  ...
  <Accordion title="从源码" icon="github">
    ...
    <Steps>
      <Step title="克隆并构建">
        克隆 [Gods Eye 仓库](https://github.com/bitan-del/gods-eye) 并构建：
      ...
      <Step title="链接 CLI">
        ...
        或者，你也可以跳过链接，直接在仓库内通过 `pnpm godseye ...` 运行命令。
      </Step>
      <Step title="运行新手引导">
        ```bash
        godseye onboard --install-daemon
...
```bash
godseye doctor         # 检查配置问题
godseye status         # Gateway 网关状态
godseye dashboard      # 打开浏览器 UI
```

如果你需要自定义运行时路径，请使用：

- `GODSEYE_HOME` 用于基于主目录的内部路径
- `GODSEYE_STATE_DIR` 用于可变状态位置
- `GODSEYE_CONFIG_PATH` 用于配置文件位置
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不在**你的 `$PATH` 中，那么你的 shell 就找不到全局 npm 二进制文件（包括 `godseye`）。
