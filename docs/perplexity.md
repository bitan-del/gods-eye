...
Gods Eye supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Gods Eye also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `godseye configure --section web`. It stores the key in
`~/.godseye/godseye.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.godseye/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
