...
- During heartbeat runs, Gods Eye treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `godseye sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Gods Eye skips the heartbeat run entirely (no model call).
...
```bash
godseye system event --text "Check for urgent follow-ups" --mode now
```
