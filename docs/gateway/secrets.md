...
Gods Eye supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `GODSEYE_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Gods Eye runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Gods Eye validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Gods Eye QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/godseye"],
...
- Gods Eye resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `godseye.json` refs)
...
When reload-time activation fails after a healthy state, Gods Eye enters degraded secrets state.
...
- Strict command paths (for example `godseye memory` remote-memory paths and `godseye qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `godseye status`, `godseye status --all`, `godseye channels status`, `godseye channels resolve`, `godseye security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `godseye secrets reload`.
...
```bash
godseye secrets audit --check
godseye secrets configure
godseye secrets audit --check
```
...
- plaintext values at rest (`godseye.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `godseye.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `godseye secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `godseye.json` plus `auth-profiles.json` for one agent scope
...
- `godseye secrets configure --providers-only`
- `godseye secrets configure --skip-provider-setup`
- `godseye secrets configure --agent <id>`
...
```bash
godseye secrets apply --from /tmp/godseye-secrets-plan.json
godseye secrets apply --from /tmp/godseye-secrets-plan.json --allow-exec
godseye secrets apply --from /tmp/godseye-secrets-plan.json --dry-run
godseye secrets apply --from /tmp/godseye-secrets-plan.json --dry-run --allow-exec
```
...
Gods Eye intentionally does not write rollback backups containing historical plaintext secret values.
