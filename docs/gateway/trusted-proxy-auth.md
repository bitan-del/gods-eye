...
read_when:
  - Running Gods Eye behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Gods Eye
...
- You run Gods Eye behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Gods Eye checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Gods Eye extracts the user identity from the configured header
...
- Gods Eye can stay on loopback HTTP behind the proxy.
...
If Gods Eye itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://godseye.example.com
    to: http://godseye-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`godseye security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Gods Eye config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `godseye security audit` and review findings
