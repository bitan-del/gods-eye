...
read_when:
  - Setting up Gods Eye for the first time
...
Gods Eye reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.godseye/godseye.json`.

If the file is missing, Gods Eye uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `godseye onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    godseye onboard       # full onboarding flow
    godseye configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    godseye config get agents.defaults.workspace
    godseye config set agents.defaults.heartbeat.every "2h"
    godseye config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.godseye/godseye.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Gods Eye only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`godseye doctor`, `godseye logs`, `godseye health`, `godseye status`)
- Run `godseye doctor` to see exact issues
- Run `godseye doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `godseye.json`.
    ...
    ```bash
    godseye config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `GODSEYE_APNS_RELAY_BASE_URL` and `GODSEYE_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `GODSEYE_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.godseye/godseye.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `godseye config set` for single keys.
    </Warning>
    ...
    ```bash
    godseye gateway call config.get --params '{}'  # capture payload.hash
    godseye gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.godseye/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    godseye gateway call config.patch --params '{
...
Gods Eye reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.godseye/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Gods Eye runs your login shell and imports only the missing keys:
...
Env var equivalent: `GODSEYE_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${GODSEYE_GATEWAY_TOKEN}" } },
