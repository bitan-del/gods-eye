...
- `godseye gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `godseye doctor` reports no blocking config/service issues.
- `godseye channels status --probe` shows connected/ready channels.
...
godseye config get agents.defaults.models
```
...
godseye status
godseye channels status --probe
godseye pairing list --channel <channel> [--account <id>]
godseye config get channels
godseye logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `godseye config get gateway.auth.token` then paste into Control UI settings.                           |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `godseye devices list` then `godseye devices approve <requestId>`.                                                                          |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `godseye configure`). If you are running Gods Eye via Podman using the dedicated `godseye` user, the config lives at `~godseye/.godseye/godseye.json`.
...
godseye cron status
godseye cron list
godseye cron runs --id <jobId> --limit 20
godseye system heartbeat last
godseye logs --follow
```
...
godseye approvals get --node <idOrNameOrIp>
godseye logs --follow
godseye status
```
...
godseye browser start --browser-profile godseye
...
```bash
godseye gateway status
godseye config get gateway.mode
godseye config get gateway.remote.url
godseye config get gateway.auth.mode
```
...
```bash
godseye config get gateway.bind
godseye config get gateway.auth.token
godseye gateway status
godseye logs --follow
```
