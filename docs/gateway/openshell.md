---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Gods Eye agents"
read_when:
...
OpenShell is a managed sandbox backend for Gods Eye. Instead of running Docker
containers locally, Gods Eye delegates sandbox lifecycle to the `openshell` CLI,
...
- Gods Eye Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Gods Eye creates an OpenShell
...
- You edit files locally outside Gods Eye and want those changes visible in the
...
- When the sandbox is first created, Gods Eye seeds the remote workspace from
...
- Gods Eye does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Gods Eye after the initial seed,
the remote sandbox does **not** see those changes. Use
`godseye sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"godseye"`   | Sandbox source for first-time create                  |
...
1. Gods Eye calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Gods Eye calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `godseye sandbox` commands
