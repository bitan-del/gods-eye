...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Gods Eye runs”).
...
- `godseye health` and `godseye status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `godseye gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`GODSEYE_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`GODSEYE_GATEWAY_URL`) may use env credentials only (`GODSEYE_GATEWAY_TOKEN` / `GODSEYE_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `GODSEYE_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `GODSEYE_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `GODSEYE_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `GODSEYE_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `GODSEYE_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `GODSEYE_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
