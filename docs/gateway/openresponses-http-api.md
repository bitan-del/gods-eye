...
Gods Eye’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`godseye agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "godseye"`, `model: "godseye/default"`, `model: "godseye/<agentId>"`, or `x-godseye-agent-id`
- use `x-godseye-model` when you want to override the selected agent's backend model
- use `x-godseye-session-key` for explicit session routing
- use `x-godseye-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Gods Eye reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
