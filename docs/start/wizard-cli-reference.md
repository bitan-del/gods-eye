...
read_when:
  - You need detailed behavior for godseye onboard
...
This page is the full reference for `godseye onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.godseye/godseye.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `godseye doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `godseye pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `godseye health`.
    - `godseye status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.godseye/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.godseye/agents/<agentId>/agent/auth-profiles.json`
...
`~/.godseye/credentials/oauth.json` (or `$GODSEYE_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`godseye onboard`](/cli/onboard)
