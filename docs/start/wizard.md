...
CLI onboarding is the **recommended** way to set up Gods Eye on macOS,
...
`godseye dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`godseye configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.godseye/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `godseye doctor` first.
</Note>
...
Use `godseye agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.godseye/workspace-<agentId>`.
...
- CLI command reference: [`godseye onboard`](/cli/onboard)
