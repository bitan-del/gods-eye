...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Gods Eye
...
Running Gods Eye via Node with `tsx` fails at startup with:

```
[godseye] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `godseye status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch godseye.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node godseye.mjs status` works on Node 25.
