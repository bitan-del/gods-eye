---
title: Lobster
summary: "Typed workflow runtime for Gods Eye with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets Gods Eye run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Gods Eye runs one Lobster tool call and gets a structured result.
...
Gods Eye launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | godseye.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
godseye.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Gods Eye tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Gods Eye Gateway (see the [Lobster repo](https://github.com/bitan-del/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Gods Eye keeps core tools enabled. To restrict core
...
→ godseye calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ godseye calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Gods Eye tools that do.
