...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Gods Eye run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Gods Eye in plain language to "run this in Codex" or "start Claude Code in a thread", Gods Eye should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Gods Eye-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Gods Eye native sub-agent runtime  |
...
- Gods Eye binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Gods Eye returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Gods Eye agent id.
...
- Gods Eye ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, Gods Eye uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Gods Eye may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Gods Eye session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Gods Eye sandbox.
...
If no target resolves, Gods Eye returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Gods Eye returns a clear unsupported-control error.
...
When Gods Eye uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Gods Eye `agentId` path).
...
```bash
godseye plugins install acpx
godseye config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
godseye plugins install ./extensions/acpx
```
...
- Relative paths resolve from Gods Eye workspace directory.
...
- Gods Eye startup remains non-blocking while the backend health check runs.
...
```bash
godseye config set plugins.entries.acpx.config.permissionMode approve-all
godseye config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Gods Eye currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
