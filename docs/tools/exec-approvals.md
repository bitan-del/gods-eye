...
- **gateway host** → `godseye` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Gods Eye also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.godseye/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Gods Eye treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.godseye/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `godseye approvals allowlist ...`).
- `godseye security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `godseye doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Gods Eye still rejects the `env` builtin in safe-bin
...
`~/.godseye/exec-approvals.json` directly.

CLI: `godseye approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Gods Eye does not treat
...
topics, Gods Eye preserves the topic for the approval prompt and the post-approval follow-up.
