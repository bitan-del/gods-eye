...
read_when:
  - Running Gods Eye Gateway in WSL2 while Chrome lives on Windows
...
- Gods Eye Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/bitan-del/gods-eye/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Gods Eye and Chrome are on the same machine
...
- Gods Eye points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Gods Eye is not the problem yet.
...
Fix that before touching Gods Eye config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Gods Eye at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Gods Eye to succeed
...
```bash
godseye browser open https://example.com --browser-profile remote
godseye browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `godseye browser tabs` returns the target
...
3. Gods Eye config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Gods Eye config or Control UI auth
