---
summary: "Install, configure, and manage Gods Eye plugins"
read_when:
...
Plugins extend Gods Eye with new capabilities: channels, model providers, tools,
skills, speech, image generation, and more. Some plugins are **core** (shipped
with Gods Eye), others are **external** (published on npm by the community).
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    godseye plugins install ./my-plugin
    godseye plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@godseye/voice-call
...
Gods Eye recognizes two plugin formats:
...
| Voice Call      | `@godseye/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@godseye/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@godseye/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with Gods Eye)
...
watch + in-process restart enabled (the default `godseye gateway` path), that
...
Gods Eye scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.godseye/extensions/*.ts` and `\<workspace\>/.godseye/extensions/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.godseye/extensions/*.ts` and `~/.godseye/extensions/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Gods Eye. Many are enabled by default (model providers, speech).
...
```bash
godseye plugins list                    # compact inventory
godseye plugins inspect <id>            # deep detail
godseye plugins inspect <id> --json     # machine-readable
godseye plugins status                  # operational summary
godseye plugins doctor                  # diagnostics

godseye plugins install <package>        # install (ClawHub first, then npm)
godseye plugins install clawhub:<pkg>   # install from ClawHub only
godseye plugins install <path>          # install from local path
godseye plugins install -l <path>       # link (no copy) for dev
godseye plugins update <id>             # update one plugin
godseye plugins update --all            # update all

godseye plugins enable <id>
godseye plugins disable <id>
```

See [`godseye plugins` CLI reference](/cli/plugins) for full details.
