...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Gods Eye plugin SDK" });
    ```
...
If no `provider` is set, Gods Eye checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Gods Eye resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `godseye configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.godseye/.env`.
