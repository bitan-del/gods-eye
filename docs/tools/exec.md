...
- `gateway`/`node` approvals are controlled by `~/.godseye/exec-approvals.json`.
...
- Gods Eye sets `GODSEYE_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Gods Eye prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`godseye security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `godseye doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`godseye security audit` and `godseye doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
