...
Gods Eye can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `godseye configure --section web` enables the bundled Firecrawl plugin automatically.
...
Gods Eye always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
