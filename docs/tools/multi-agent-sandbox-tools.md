...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `godseye sandbox explain`.
...
Legacy `agent.*` configs are migrated by `godseye doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=godseye-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${GODSEYE_STATE_DIR:-$HOME/.godseye}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
