---
summary: "ClawHub guide: public registry, native Gods Eye install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Gods Eye skills and plugins**.

- Use native `godseye` commands to search/install/update skills and install
...
```bash
godseye plugins install clawhub:<package>
...
Native `godseye` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Gods Eye skills.
...
4. Users browse, download, and install skills in Gods Eye.
...
If you want to add new capabilities to your Gods Eye agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `godseye skills install <skill-slug>`
3. Start a new Gods Eye session so it picks up the new skill.
...
## How it fits into Gods Eye

Native `godseye skills install` installs into the active workspace `skills/`
directory. `godseye plugins install clawhub:...` records a normal managed
...
current working directory. If an Gods Eye workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Gods Eye loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.godseye/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Gods Eye how to perform a
...
Interested in becoming a moderator? Ask in the Gods Eye Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Gods Eye workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/godseye/skills` and `~/.godseye/skills`). This is designed to find older skill installs without extra flags.
