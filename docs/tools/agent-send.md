...
`godseye agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    godseye agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    godseye agent --agent ops --message "Summarize logs"

    # Target a phone number (derives session key)
    godseye agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    godseye agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    godseye agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    godseye agent --agent ops --message "Generate report" \
...
godseye agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
