---
summary: "How Gods Eye builds prompt context and reports token usage + costs"
read_when:
...
Gods Eye tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Gods Eye assembles its own system prompt on every run. It includes:
...
For images, Gods Eye downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Gods Eye session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `godseye status --usage` and `godseye channels list` show
...
`cacheWrite`. If pricing is missing, Gods Eye shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Gods Eye can
...
Anthropic's 1M context window is currently beta-gated. Gods Eye can inject the
...
Gods Eye skips the `context-1m-*` beta header because Anthropic currently
