...
Gods Eye integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Gods Eye owns lifecycle when `channels.signal.autoStart=true`.
...
- Gods Eye spawns `imsg rpc` as a child process (legacy iMessage integration).
