...
This document explains how Gods Eye manages sessions end-to-end:
...
Gods Eye is designed around a single **Gateway process** that owns session state.
...
Gods Eye persists sessions in two layers:
...
- Transcripts: `~/.godseye/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Gods Eye resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Gods Eye reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.godseye/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Gods Eye intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Gods Eye consumes the events, but Pi decides when to compact).
...
Gods Eye also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Gods Eye bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Gods Eye leaves it alone.
...
- `godseye status` (CLI)
- `godseye sessions` / `sessions --json`
...
Gods Eye supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Gods Eye strips/suppresses this in the delivery layer.

As of `2026.1.10`, Gods Eye also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Gods Eye uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Gods Eye’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `godseye status`.
