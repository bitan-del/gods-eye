...
Gods Eye features that can generate provider usage or paid API calls.
...
- `godseye status --usage` and `godseye channels list` show provider **usage windows**
...
Gods Eye can pick up credentials from:
...
- `godseye status --usage`
- `godseye models status --json`
...
`godseye models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
