...
This is the full reference for `godseye onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.godseye/godseye.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `godseye doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.godseye/credentials/oauth.json`; auth profiles live in `~/.godseye/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.godseye/credentials/oauth.json` (or `$GODSEYE_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.godseye/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `godseye pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `godseye configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `godseye health`.
    - Tip: `godseye status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export GODSEYE_GATEWAY_TOKEN="your-token"
godseye onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env GODSEYE_GATEWAY_TOKEN
```
...
- Stores it under `~/.godseye/tools/signal-cli/<version>/`.
...
Typical fields in `~/.godseye/godseye.json`:
...
`godseye agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.godseye/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.godseye/agents/<agentId>/sessions/`.
