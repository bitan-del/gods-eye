...
- With Anthropic API-key auth profiles, Gods Eye seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Gods Eye injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Gods Eye exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.godseye/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$GODSEYE_STATE_DIR/logs/cache-trace.jsonl`
...
- `GODSEYE_CACHE_TRACE=1` enables cache tracing.
- `GODSEYE_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `GODSEYE_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `GODSEYE_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `GODSEYE_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
