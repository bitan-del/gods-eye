...
Gods Eye supports **camera capture** for agent workflows:
...
```bash
godseye nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
godseye nodes camera snap --node <id> --facing front
godseye nodes camera clip --node <id> --duration 3000
godseye nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`godseye.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `godseye` CLI to invoke camera commands on the macOS node.

Examples:

```bash
godseye nodes camera list --node <id>            # list camera ids
godseye nodes camera snap --node <id>            # prints MEDIA:<path>
godseye nodes camera snap --node <id> --max-width 1280
godseye nodes camera snap --node <id> --delay-ms 2000
godseye nodes camera snap --node <id> --device-id <id>
godseye nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
godseye nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
godseye nodes camera clip --node <id> --device-id <id>
godseye nodes camera clip --node <id> --no-audio
```

Notes:

- `godseye nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
godseye nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
