...
read_when:
  ...
  - You want a reference for all registration methods on Gods EyePluginApi
...
The `register(api)` callback receives an `Gods EyePluginApi` object with these
methods:
...
| `api.config`             | `Gods EyeConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `godseye/plugin-sdk/<your-plugin>`
