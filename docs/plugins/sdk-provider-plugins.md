...
summary: "Step-by-step guide to building a model provider plugin for Gods Eye"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Gods Eye
...
(LLM) to Gods Eye. By the end you will have a provider with a model catalog,
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Gods Eye can detect
  ...
  <Step title="Register the provider">
    ...
    `godseye onboard --acme-ai-api-key <key>` and select
    ...
    `godseye/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Gods Eye calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Gods Eye classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # godseye.providers metadata
├── godseye.plugin.json      # Manifest with providerAuthEnvVars
