...
read_when:
  ...
  - You are defining plugin config schemas or package.json godseye metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`godseye.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `godseye` field that tells the plugin system what
...
```json
{
  "name": "@myorg/godseye-my-provider",
  "version": "1.0.0",
  "type": "module",
  "godseye": {
    "extensions": ["./index.ts"],
...
When enabled, Gods Eye loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `godseye.plugin.json` in the package root.
Gods Eye uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Gods Eye",
  "configSchema": {
    "type": "object",
...
Gods Eye loads when it only needs setup surfaces (onboarding, config repair,
...
**When Gods Eye uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `godseye/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Gods Eye validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "godseye/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `godseye onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "godseye/plugin-sdk/channel-setup";
...
helpers from `godseye/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`godseye/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `godseye/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "godseye/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/godseye-my-channel",
...
```bash
godseye plugins install @myorg/godseye-my-plugin
```

Gods Eye tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
godseye plugins install clawhub:@myorg/godseye-my-plugin   # ClawHub only
godseye plugins install npm:@myorg/godseye-my-plugin       # npm only
```
...
```bash
godseye plugins search <query>
godseye plugins install <package-name>
```

<Info>
  For npm-sourced installs, `godseye plugins install` runs
