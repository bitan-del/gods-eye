...
read_when:
  - You are building an Gods Eye plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (godseye.plugin.json)

This page is for the **native Gods Eye plugin manifest** only.
...
Gods Eye auto-detects those bundle layouts too, but they are not validated
against the `godseye.plugin.json` schema described here.

For compatible bundles, Gods Eye currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Gods Eye runtime expectations.

Every native Gods Eye plugin **must** ship a `godseye.plugin.json` file in the
**plugin root**. Gods Eye uses this manifest to validate configuration
...
`godseye.plugin.json` is the metadata Gods Eye reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Gods Eye can inspect without loading plugin code.                                      |
...
Gods Eye reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Gods Eye falls back to `choiceId`.                                        |
...
- if Gods Eye must know it before loading plugin code, put it in `godseye.plugin.json`
...
- The manifest is **required for native Gods Eye plugins**, including local filesystem loads.
