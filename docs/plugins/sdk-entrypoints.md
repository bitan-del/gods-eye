...
**Import:** `godseye/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `Gods EyePluginConfigSchema \| () => Gods EyePluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: Gods EyePluginApi) => void`                               | Yes      | —                   |

- `id` must match your `godseye.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `godseye/plugin-sdk/core`
...
| `registerFull` | `(api: Gods EyePluginApi) => void`                               | No       | —                   |
...
Gods Eye loads this instead of the full entry when a channel is disabled,
...
Gods Eye classifies loaded plugins by their registration behavior:
...
Use `godseye plugins inspect <id>` to see a plugin's shape.
