...
read_when:
  - You see the GODSEYE_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the GODSEYE_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Gods Eye plugin
---

# Plugin SDK Migration

Gods Eye has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`godseye/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`godseye/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`godseye/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "godseye/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "godseye/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "godseye/extension-api";
...
```bash
GODSEYE_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 godseye gateway run
GODSEYE_SUPPRESS_EXTENSION_API_WARNING=1 godseye gateway run
```
