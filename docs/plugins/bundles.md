---
summary: "Install and use Codex, Claude, and Cursor bundles as Gods Eye plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Gods Eye maps bundle content into native features
...
Gods Eye can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Gods Eye maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Gods Eye plugins. Native plugins run
...
of requiring authors to rewrite them as native Gods Eye plugins, Gods Eye
...
## What Gods Eye maps from bundles

Not every bundle feature runs in Gods Eye today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Gods Eye skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | Gods Eye-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but Gods Eye does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Gods Eye best when they use skill roots and Gods Eye-style
...
Gods Eye checks for native plugin format first:

1. `godseye.plugin.json` or valid `package.json` with `godseye.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Gods Eye uses the native path. This prevents
...
- Gods Eye does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `godseye plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Gods Eye does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Gods Eye hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
