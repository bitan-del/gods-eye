...
read_when:
  - Building or debugging native Gods Eye plugins
...
This page covers the internal architecture of the Gods Eye plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Gods Eye. Every
native Gods Eye plugin registers against one or more capability types:
...
Gods Eye classifies every loaded plugin into a shape based on its actual
...
Use `godseye plugins inspect <id>` to see a plugin's shape and capability
...
When you run `godseye doctor` or `godseye plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `godseye status --all` and `godseye plugins doctor`.

## Architecture overview

Gods Eye's plugin system has four layers:

1. **Manifest + discovery**
   Gods Eye finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `godseye.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Gods Eye plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Gods Eye reads the registry to expose tools, channels, provider
...
That split lets Gods Eye validate config, explain missing/disabled plugins, and
...
normal chat actions. Gods Eye keeps one shared `message` tool in core, and
...
Gods Eye treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Gods Eye-facing
  surfaces
...
So if Gods Eye adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Gods Eye has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { Gods EyePluginDefinition } from "godseye/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "godseye/plugin-sdk";

const plugin: Gods EyePluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Gods Eye already treats image/audio/video understanding as one shared
...
If Gods Eye adds a new domain later, such as video generation, use the same
...
`Gods EyePluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Gods Eye can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Gods Eye knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `Gods EyePluginApi` or
  `api.runtime`
...
Native Gods Eye plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Gods Eye process

Compatible bundles are safer by default because Gods Eye currently treats them
...
name: `@godseye/<id>` by default, or an approved typed suffix such as
...
Gods Eye exports capabilities, not implementation convenience.
...
At startup, Gods Eye does roughly this:
...
The manifest is the control-plane source of truth. Gods Eye uses it to:
...
Gods Eye keeps short in-process caches for:
...
- Set `GODSEYE_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `GODSEYE_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `GODSEYE_PLUGIN_DISCOVERY_CACHE_MS` and
  `GODSEYE_PLUGIN_MANIFEST_CACHE_MS`.
...
Gods Eye still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Gods Eye calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Gods Eye tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Gods Eye's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Gods Eye's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Gods Eye adds those
...
- If Gods Eye adds a new capability such as video generation later, define the
...
- Gods Eye only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Gods Eye plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `godseye/plugin-sdk` import when
authoring plugins:

- `godseye/plugin-sdk/plugin-entry` for plugin registration primitives.
- `godseye/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `godseye/plugin-sdk/channel-setup`,
  `godseye/plugin-sdk/channel-pairing`,
  `godseye/plugin-sdk/channel-contract`,
  `godseye/plugin-sdk/channel-feedback`,
  `godseye/plugin-sdk/channel-inbound`,
  `godseye/plugin-sdk/channel-lifecycle`,
  `godseye/plugin-sdk/channel-reply-pipeline`,
  `godseye/plugin-sdk/command-auth`,
  `godseye/plugin-sdk/secret-input`, and
  `godseye/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `godseye/plugin-sdk/channel-config-helpers`,
  `godseye/plugin-sdk/allow-from`,
  `godseye/plugin-sdk/channel-config-schema`,
  `godseye/plugin-sdk/channel-policy`,
  `godseye/plugin-sdk/config-runtime`,
  `godseye/plugin-sdk/infra-runtime`,
  `godseye/plugin-sdk/agent-runtime`,
  `godseye/plugin-sdk/lazy-runtime`,
  `godseye/plugin-sdk/reply-history`,
  `godseye/plugin-sdk/routing`,
  `godseye/plugin-sdk/status-helpers`,
  `godseye/plugin-sdk/runtime-store`, and
  `godseye/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `godseye/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `godseye/plugin-sdk/*` subpaths. Gods Eye core/test code may use the repo
...
- Avoid the root `godseye/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `godseye/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `godseye/plugin-sdk/channel-actions`.
...
`godseye/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Gods Eye writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Gods Eye's
...
- if both `catalog` and `discovery` are registered, Gods Eye uses `catalog`
...
- Read-only command paths such as `godseye status`, `godseye status --all`,
  `godseye channels status`, `godseye channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `godseye.extensions`:
...
Security guardrail: every `godseye.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `godseye plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `godseye.setupEntry` can point at a lightweight setup-only module.
When Gods Eye needs setup surfaces for a disabled channel plugin, or
...
Optional: `godseye.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Gods Eye load the
...
Channel plugins can advertise setup/discovery metadata via `godseye.channel` and
install hints via `godseye.install`. This keeps the core catalog data-free.
...
Gods Eye can also merge **external channel catalogs** (for example, an MPM
...
Or point `GODSEYE_PLUGIN_CATALOG_PATHS` (or `GODSEYE_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "godseye": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "godseye/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Gods EyePluginApi` and/or `api.runtime` with the smallest useful
...
This is how Gods Eye stays opinionated without becoming hardcoded to one
