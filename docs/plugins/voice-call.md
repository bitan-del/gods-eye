...
read_when:
  - You want to place an outbound voice call from Gods Eye
...
Voice calls for Gods Eye via a plugin. Supports outbound notifications and
...
- Use `godseye voicecall ...` or the `voice_call` tool
...
```bash
godseye plugins install ./extensions/voice-call
...
godseye voicecall start --to "+15555550123"   # alias for call
...
godseye voicecall latency                     # summarize turn latency from logs
