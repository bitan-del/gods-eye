...
summary: "Architecture of Gods Eye's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Gods Eye
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Gods Eye integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Gods Eye uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Gods Eye directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Gods Eye's local TUI mode)                                             |
...
├── pi-tools.ts                    # createGods EyeCodingTools()
...
├── godseye-tools.ts              # Gods Eye-specific tools
...
Image injection is prompt-local: Gods Eye loads image refs from the current prompt and
...
2. **Custom Replacements**: Gods Eye replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Gods Eye Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Gods Eye's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Gods Eye CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Gods Eye wraps this with `guardSessionManager()` for tool result safety.
...
Gods Eye maintains an auth profile store with multiple API keys per provider:
...
Gods Eye loads custom pi extensions for specialized behavior:
...
Gods Eye also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Gods Eye Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Gods Eye tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.godseye/agents/<agentId>/sessions/` (or `$GODSEYE_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `GODSEYE_LIVE_TEST=1`)
