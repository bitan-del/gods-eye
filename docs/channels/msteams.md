...
```bash
godseye plugins install @godseye/msteams
```

Local checkout (when running from a git repo):

```bash
godseye plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Gods Eye will offer the local install path automatically.
...
3. Configure Gods Eye with those credentials.
...
- Talk to Gods Eye via Teams DMs, group chats, or channels.
...
- On startup, Gods Eye resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.godseye/godseye.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Gods Eye, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `godseye-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `godseye plugins install @godseye/msteams`
   - From a local checkout: `godseye plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "Gods Eye in Teams", full: "Gods Eye in Teams" },
...
Gods Eye handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Gods Eye only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Gods EyeShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Gods Eye sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `godseye message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.godseye/msteams-polls.json`.
...
```bash
godseye message send --channel msteams \
...
godseye message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
godseye message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
godseye message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
godseye message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
