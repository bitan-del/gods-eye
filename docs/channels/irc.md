...
read_when:
  - You want to connect Gods Eye to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

# IRC

Use IRC when you want Gods Eye in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.godseye/godseye.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Gods Eye defaults to **mention-gating** in group contexts.
