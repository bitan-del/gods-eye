...
Tlon is a decentralized messenger built on Urbit. Gods Eye connects to your Urbit ship and can
...
```bash
godseye plugins install @godseye/tlon
```

Local checkout (when running from a git repo):

```bash
godseye plugins install ./extensions/tlon
```
...
By default, Gods Eye blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `godseye message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Gods Eye replies in-thread.
