...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Gods Eye HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `godseye pairing approve slack <code>`.
...
- The interactive callback values are Gods Eye-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Gods Eye falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Gods Eye is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Gods Eye",
    "description": "Slack connector for Gods Eye"
  },
  "features": {
    "bot_user": {
      "display_name": "Gods Eye",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/godseye",
        "description": "Send a message to Gods Eye",
        "should_escape": false
      }
    ]
  },
...
Gods Eye supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Gods Eye falls back to normal delivery for remaining payloads.
