...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Gods Eye to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Gods Eye releases, so no separate plugin install
is required.
...
```bash
godseye plugins install @godseye/feishu
```
...
If you just installed Gods Eye, run onboarding:
...
2. Configuring app credentials in Gods Eye
...
1. You already ran `godseye channels add` for Feishu
2. The gateway is running (`godseye gateway status`)
...
```bash
godseye pairing approve feishu <CODE>
```
...
2. Run `godseye logs --follow` and look for `chat_id`
...
2. Run `godseye logs --follow` and look for `open_id`
...
| `godseye gateway restart` | Restart gateway service       |
| `godseye logs --follow`   | Tail gateway logs             |
...
4. Check logs: `godseye logs --follow`
...
5. Ensure the gateway is running: `godseye gateway status`
6. Check logs: `godseye logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.godseye/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.godseye/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
