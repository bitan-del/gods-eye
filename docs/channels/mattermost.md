---
summary: "Mattermost bot setup and Gods Eye config"
read_when:
...
```bash
godseye plugins install @godseye/mattermost
```

Local checkout (when running from a git repo):

```bash
godseye plugins install ./extensions/mattermost
```
...
4. Configure Gods Eye and start the gateway.
...
Native slash commands are opt-in. When enabled, Gods Eye registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Gods Eye derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Gods Eye.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Gods Eye.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Gods Eye, not `404`.
...
- Approve via:
  - `godseye pairing list mattermost`
  - `godseye pairing approve mattermost <CODE>`
...
Use these target formats with `godseye message send` or cron/webhooks:
...
Gods Eye resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Gods Eye sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Gods Eye sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Gods Eye derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Gods Eye run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="godseye-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"godseye-mattermost-interactions",
...
`godseye message send` and cron/webhook deliveries.
