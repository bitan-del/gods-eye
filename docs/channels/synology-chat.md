---
summary: "Synology Chat webhook setup and Gods Eye config"
read_when:
  - Setting up Synology Chat with Gods Eye
...
```bash
godseye plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `godseye onboard` now shows Synology Chat in the same channel setup list as `godseye channels add`.
   - Non-interactive setup: `godseye channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Gods Eye gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Gods Eye.
   - Guided: `godseye onboard`
   - Direct: `godseye channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `godseye pairing list synology-chat`
  - `godseye pairing approve synology-chat <CODE>`
...
```bash
godseye message send --channel synology-chat --target 123456 --text "Hello from Gods Eye"
godseye message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Gods Eye now rejects duplicate exact paths
