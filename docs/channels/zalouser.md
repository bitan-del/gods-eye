...
read_when:
  - Setting up Zalo Personal for Gods Eye
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Gods Eye.
...
- Install via CLI: `godseye plugins install @godseye/zalouser`
- Or from a source checkout: `godseye plugins install ./extensions/zalouser`
...
- `godseye pairing list zalouser`
- `godseye pairing approve zalouser <code>`
...
- On startup, Gods Eye resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Gods Eye stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Gods Eye state. Example:
...
- Gods Eye sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Gods Eye sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `godseye channels logout --channel zalouser && godseye channels login --channel zalouser`
...
- The channel now runs fully in Gods Eye without external CLI binaries.
