...
read_when:
  - Setting up Twitch chat integration for Gods Eye
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Gods Eye connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `GODSEYE_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "godseye", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use GODSEYE_TWITCH_ACCESS_TOKEN env var)
...
```bash
GODSEYE_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
