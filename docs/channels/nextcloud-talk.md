...
```bash
godseye plugins install @godseye/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
godseye plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Gods Eye" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `godseye pairing list nextcloud-talk`
  - `godseye pairing approve nextcloud-talk <CODE>`
