...
Gods Eye routes replies **back to the channel where a message came from**. The
...
Gods Eye infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Gods Eye still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Gods Eye would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.godseye/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.godseye`):

- `~/.godseye/agents/<agentId>/sessions/sessions.json`
