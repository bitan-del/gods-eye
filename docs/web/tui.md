...
```bash
godseye tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `godseye tui --deliver`
...
- Local shell commands receive `GODSEYE_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `GODSEYE_THEME=light` before launching `godseye tui`.
- To force the original dark palette instead, set `GODSEYE_THEME=dark`.
...
- Check the Gateway logs: `godseye logs --follow`.
- Confirm the agent can run: `godseye status` and `godseye models status`.
...
- No agents in picker: check `godseye agents list` and your routing config.
