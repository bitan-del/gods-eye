...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/godseye`)
...
If the page fails to load, start the Gateway first: `godseye gateway`.
...
created. Re-run `godseye devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `godseye devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.godseye/godseye.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop godseye`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Gods Eye
...
Gods Eye **blocks** Control UI connections without device identity.
