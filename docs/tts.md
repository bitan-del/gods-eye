...
Gods Eye can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Gods Eye can send audio.
...
Gods Eye defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `godseye.json`.
...
- If `provider` is **unset**, Gods Eye prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.godseye/settings/tts.json`, override with `GODSEYE_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Gods Eye retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Gods Eye:
...
Discord note: `/tts` is a built-in Discord command, so Gods Eye registers
