---
summary: "Optional Docker-based setup and onboarding for Gods Eye"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Gods Eye on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export GODSEYE_IMAGE="ghcr.io/godseye/godseye:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/bitan-del/gods-eye/pkgs/container/godseye).
  ...
  <Step title="Complete onboarding">
    ...
    `godseye-gateway` directly. `godseye-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node godseye-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `GODSEYE_EXTRA_MOUNTS`
or `GODSEYE_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `godseye-cli` shares `godseye-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d godseye-gateway`, run onboarding
and setup-time config writes through `godseye-gateway` with
...
| `GODSEYE_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `GODSEYE_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `GODSEYE_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `GODSEYE_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `GODSEYE_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `GODSEYE_CONFIG_DIR` to `/home/node/.godseye` and
`GODSEYE_WORKSPACE_DIR` to `/home/node/.godseye/workspace`, so those paths
...
and rolling file logs under `/tmp/godseye/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/godseye/godseye/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/bitan-del/gods-eye/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export GODSEYE_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export GODSEYE_SANDBOX=1
    export GODSEYE_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm godseye-cli gateway probe
    docker compose run -T --rm godseye-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `godseye-cli` uses `network_mode: "service:godseye-gateway"` so CLI
    ...
    `no-new-privileges` on `godseye-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.godseye`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export GODSEYE_HOME_VOLUME="godseye_home"`
    2. **Bake system deps**: `export GODSEYE_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `GODSEYE_HOME_VOLUME` or `GODSEYE_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/bitan-del/gods-eye/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Gods Eye runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm godseye-cli config set gateway.mode local
    docker compose run --rm godseye-cli config set gateway.bind lan
    docker compose run --rm godseye-cli devices list --url ws://127.0.0.1:18789
    ```
