---
summary: "Host Gods Eye on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Gods Eye on Oracle Cloud
  - Looking for free VPS hosting for Gods Eye
...
Run a persistent Gods Eye Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@godseye`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=godseye --reset` to re-authenticate.

**Gateway will not start** -- Run `godseye doctor --non-interactive` and check logs with `journalctl --user -u godseye-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Gods Eye up to date
