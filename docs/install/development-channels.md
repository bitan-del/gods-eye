...
Gods Eye ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/godseye`, override with
  `GODSEYE_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
godseye update --tag godseye@2026.3.22
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `godseye update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Gods Eye prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `godseye update` would do without making changes:

```bash
godseye update --dry-run
godseye update --channel beta --dry-run
godseye update --tag 2026.3.22 --dry-run
godseye update --dry-run --json
```
...
When you switch channels with `godseye update`, Gods Eye also syncs plugin
sources:
