---
summary: "Run Gods Eye Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Gods Eye running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Gods Eye on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Gods Eye Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Gods Eye 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Gods Eye Gateway in Docker
- Persist `~/.godseye` + `~/.godseye/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-godseye-project --name="Gods Eye Gateway"
    gcloud config set project my-godseye-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create godseye-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh godseye-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Gods Eye repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    GODSEYE_IMAGE=godseye:latest
    GODSEYE_GATEWAY_TOKEN=change-me-now
    GODSEYE_GATEWAY_BIND=lan
    GODSEYE_GATEWAY_PORT=18789

    GODSEYE_CONFIG_DIR=/home/$USER/.godseye
    GODSEYE_WORKSPACE_DIR=/home/$USER/.godseye/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.godseye
    ```
    ...
    services:
      godseye-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - GODSEYE_GATEWAY_BIND=${GODSEYE_GATEWAY_BIND}
          - GODSEYE_GATEWAY_PORT=${GODSEYE_GATEWAY_PORT}
          - GODSEYE_GATEWAY_TOKEN=${GODSEYE_GATEWAY_TOKEN}
          - GOG_KEYRING_PASSWORD=${GOG_KEYRING_PASSWORD}
          - XDG_CONFIG_HOME=${XDG_CONFIG_HOME}
          - PATH=/home/linuxbrew/.linuxbrew/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
        volumes:
          - ${GODSEYE_CONFIG_DIR}:/home/node/.godseye
          - ${GODSEYE_WORKSPACE_DIR}:/home/node/.godseye/workspace
        ports:
          # Recommended: keep the Gateway loopback-only on the VM; access via SSH tunnel.
          # To expose it publicly, remove the `127.0.0.1:` prefix and firewall accordingly.
          - "127.0.0.1:${GODSEYE_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`GODSEYE_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm godseye-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh godseye-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop godseye-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type godseye-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start godseye-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create godseye-deploy \
     --display-name="Gods Eye Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-godseye-project \
     --member="serviceAccount:godseye-deploy@my-godseye-project.iam.gserviceaccount.com" \
