---
summary: "Run Gods Eye Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Gods Eye running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Gods Eye Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Gods Eye on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Gods Eye.
...
- Install Gods Eye with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-godseye"
    LOCATION="westus2"
    VNET_NAME="vnet-godseye"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-godseye-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-godseye-vm"
    VM_NAME="vm-godseye"
    ADMIN_USERNAME="godseye"
    BASTION_NAME="bas-godseye"
    BASTION_PIP_NAME="pip-godseye-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Gods Eye (in the VM shell)">
    ...
    The installer installs Node LTS and dependencies if not already present, installs Gods Eye, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Gods Eye onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Gods Eye Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Gods Eye Azure deployment with the GitHub Copilot model provider: [Gods Eye on Azure with GitHub Copilot](https://github.com/johnsonshi/godseye-azure-github-copilot)
