---
summary: "Move (migrate) an Gods Eye install from one machine to another"
read_when:
  - You are moving Gods Eye to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Gods Eye to a New Machine

This guide moves an Gods Eye gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.godseye/` by default) and your **workspace**, you preserve:

- **Config** -- `godseye.json` and all gateway settings
...
<Tip>
Run `godseye status` on the old machine to confirm your state directory path.
Custom profiles use `~/.godseye-<profile>/` or a path set via `GODSEYE_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf godseye-state.tgz .godseye
    ```

    If you use multiple profiles (e.g. `~/.godseye-work`), archive each separately.

  </Step>

  <Step title="Install Gods Eye on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.godseye/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `GODSEYE_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `godseye doctor`.
  </Accordion>

  <Accordion title="Copying only godseye.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-godseye-store-its-data).
  </Accordion>
...
- [ ] `godseye status` shows the gateway running
