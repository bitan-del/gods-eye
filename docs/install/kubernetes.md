---
summary: "Deploy Gods Eye Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Gods Eye on a Kubernetes cluster
  - You want to test Gods Eye in a Kubernetes environment
title: "Kubernetes"
---

# Gods Eye on Kubernetes

A minimal starting point for running Gods Eye on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Gods Eye is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/godseye 18789:18789 -n godseye
...
```bash
kubectl get secret godseye-secrets -n godseye -o jsonpath='{.data.GODSEYE_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: godseye (configurable via GODSEYE_NAMESPACE)
├── Deployment/godseye        # Single pod, init container + gateway
├── Service/godseye           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/godseye-config  # godseye.json + AGENTS.md
└── Secret/godseye-secrets    # Gateway token + API keys
```
...
Edit `godseye.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret godseye-secrets -n godseye \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/godseye -n godseye
```

### Custom namespace

```bash
GODSEYE_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/godseye/godseye:latest # or pin to a specific version from https://github.com/bitan-del/gods-eye/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # godseye.json + AGENTS.md
