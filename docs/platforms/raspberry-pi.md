---
summary: "Gods Eye on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Gods Eye on a Raspberry Pi
  - Running Gods Eye on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Gods Eye on Raspberry Pi

## Goal

Run a persistent, always-on Gods Eye Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u godseye -f
```

## 9) Access the Gods Eye Dashboard
...
```bash
ssh user@gateway-host 'godseye dashboard --no-open'
```
...
(or `GODSEYE_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/godseye-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/godseye-compile-cache
mkdir -p /var/tmp/godseye-compile-cache
export GODSEYE_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `GODSEYE_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Gods Eye, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit godseye
```

```ini
[Service]
Environment=GODSEYE_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/godseye-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Gods Eye state/cache on SSD-backed storage to avoid SD-card
...
Most Gods Eye features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled godseye

# Enable if not
sudo systemctl enable godseye

# Start on boot
sudo systemctl start godseye
```
...
```bash
# Check logs
journalctl -u godseye --no-pager -n 100

# Common fix: rebuild
cd ~/godseye  # if using hackable install
npm run build
sudo systemctl restart godseye
```
