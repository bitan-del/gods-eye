---
summary: "Gods Eye on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Gods Eye on DigitalOcean
  - Looking for cheap VPS hosting for Gods Eye
title: "DigitalOcean (Platform)"
---

# Gods Eye on DigitalOcean

## Goal

Run a persistent Gods Eye Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status godseye-gateway.service

# View logs
journalctl --user -u godseye-gateway.service -f
```
...
```bash
godseye config set gateway.bind tailnet
godseye gateway restart
```
...
```bash
godseye channels login whatsapp
# Scan QR code
```
...
- `~/.godseye/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u godseye --no-pager -n 50
```
