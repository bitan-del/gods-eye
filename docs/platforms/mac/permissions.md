...
- Same path: run the app from a fixed location (for Gods Eye, `dist/Gods Eye.app`).
...
```bash
sudo tccutil reset Accessibility ai.godseye.mac
sudo tccutil reset ScreenCapture ai.godseye.mac
...
Workaround: move files into the Gods Eye workspace (`~/.godseye/workspace`) if you want to avoid per-folder grants.
