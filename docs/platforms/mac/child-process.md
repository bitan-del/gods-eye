...
service via the external `godseye` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.godseye.gateway`
  (or `ai.godseye.<profile>` when using `--profile`/`GODSEYE_PROFILE`; legacy `com.godseye.*` is supported).
