...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `godseye.internal.`), **or**
...
Run `godseye devices list` again before approval.

4. Verify connection:

```bash
godseye nodes status
godseye gateway call node.list --params "{}"
```
...
- `GODSEYE_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Gods Eye iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Gods Eye builds.
...
```bash
export GODSEYE_APNS_TEAM_ID="TEAMID"
export GODSEYE_APNS_KEY_ID="KEYID"
export GODSEYE_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_godseye-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `godseye.internal.`) and Tailscale split DNS.
...
```bash
godseye nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__godseye__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__godseye__/canvas/` and `/__godseye__/a2ui/`.
...
```bash
godseye nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__godseye; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
godseye nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `godseye devices list` and approve manually.
