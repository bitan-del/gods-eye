...
- `/usage cost` in chats: local cost summary aggregated from Gods Eye session logs.
- CLI: `godseye status --usage` prints a full per-provider breakdown.
- CLI: `godseye channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
