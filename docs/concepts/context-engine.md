...
read_when:
  - You want to understand how Gods Eye assembles model context
...
A **context engine** controls how Gods Eye builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Gods Eye ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.godseye/godseye.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Gods Eye plugin. Install
...
godseye plugins install -l ./my-context-engine
```
...
Every time Gods Eye runs a model prompt, the context engine participates at
...
Gods Eye currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Gods Eye
...
The built-in `legacy` engine preserves Gods Eye's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Gods Eye uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Gods Eye disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Gods Eye automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `godseye/plugin-sdk/core` to use
  Gods Eye's built-in compaction behavior.
...
Gods Eye resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Gods Eye's built-in summarization. Plugin engines can implement
...
- Use `godseye doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Gods Eye
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `godseye plugins install -l ./my-engine` to link a
