---
summary: "Delegate architecture: running Gods Eye as a named agent on behalf of an organization"
...
Goal: run Gods Eye as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Gods Eye agent that:
...
Gods Eye's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Gods Eye's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.godseye/workspace-delegate",
  tools: {
...
- Cron run history: `~/.godseye/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.godseye/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Gods Eye's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
godseye agents add delegate
```

This creates:

- Workspace: `~/.godseye/workspace-delegate`
- State: `~/.godseye/agents/delegate/agent`
- Sessions: `~/.godseye/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.godseye/workspace" },
      {
        id: "delegate",
        workspace: "~/.godseye/workspace-delegate",
        tools: {
...
~/.godseye/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.godseye/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.godseye/workspace-org",
        agentDir: "~/.godseye/agents/org-assistant/agent",
