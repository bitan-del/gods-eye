...
This guide summarizes a sane workflow for working on the pi integration in Gods Eye.
...
```bash
GODSEYE_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm godseye agent --message "Hello" --thinking low`
...
State lives under the Gods Eye state directory. Default is `~/.godseye`. If `GODSEYE_STATE_DIR` is set, use that directory instead.

To reset everything:

- `godseye.json` for config
