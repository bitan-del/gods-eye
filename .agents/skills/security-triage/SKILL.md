...
description: Triage GitHub security advisories for Gods Eye with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing Gods Eye security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/godseye/godseye/security-advisories/<GHSA>`.
...
```bash
gh api /repos/godseye/godseye/security-advisories/<GHSA>
gh api /repos/godseye/godseye/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view godseye version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo godseye/godseye --match title,body,comments -- "<terms>"
gh search prs --repo godseye/godseye --match title,body,comments -- "<terms>"
```
