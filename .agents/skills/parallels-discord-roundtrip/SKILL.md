...
```bash
export GODSEYE_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.godseye/godseye.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env GODSEYE_PARALLELS_DISCORD_TOKEN \
...
- Use the `godseye` wrapper for guest `message send/read`; `node godseye.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/godseye-parallels-smoke.*`
...
- host inbound nonce appears in `godseye message read` output
