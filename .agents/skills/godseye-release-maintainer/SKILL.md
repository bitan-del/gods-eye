---
name: godseye-release-maintainer
description: Maintainer workflow for Gods Eye releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Gods Eye Release Maintainer
...
- Core `godseye` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Gods Eye/Resources/Info.plist`
...
- Every Gods Eye release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/godseye/godseye/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `godseye` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `godseye YYYY.M.D-beta.N`
...
```bash
  GODSEYE_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/godseye-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Gods Eye npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `godseye/godseye` and the real mac preflight in
  `godseye/releases-private` for every release.
...
- Gods Eye publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `godseye/godseye` is now a
...
- Real mac preflight and real mac publish both use
  `godseye/releases-private/.github/workflows/godseye-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@godseye/godseye-release-managers` before publish continues.
- Mac publish uses
  `godseye/releases-private/.github/workflows/godseye-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `godseye/godseye`
  automatically when `GODSEYE_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Gods Eye releases.
- `@godseye/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/godseye-npm-release.yml` with `preflight_only=true`
   and wait for it to pass.
10. Start `.github/workflows/macos-release.yml` in `godseye/godseye` and wait
...
13. Start `.github/workflows/godseye-npm-release.yml` with the same tag for
    the real publish.
14. Wait for `npm-release` approval from `@godseye/godseye-release-managers`.
15. Start
    `godseye/releases-private/.github/workflows/godseye-macos-publish.yml`
...
- Use `godseye-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
