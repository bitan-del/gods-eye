...
import type { GodsEyeConfig, GodsEyePluginApi } from "../runtime-api.js";
...
function createApi(
  registrationMode: GodsEyePluginApi["registrationMode"],
  ...
  it("offers env shortcut only for the default account when env is present and config is empty", () => {
    ...
    expect(
      mattermostSetupWizard.envShortcut?.isAvailable?.({
        cfg: { channels: { mattermost: {} } } as GodsEyeConfig,
        accountId: "default",
      } as never),
    ).toBe(true);
  ...
  it("keeps env shortcut as a no-op patch for the selected account", () => {
    expect(
      mattermostSetupWizard.envShortcut?.apply?.({
        cfg: { channels: { mattermost: { enabled: false } } } as GodsEyeConfig,
        accountId: "default",
      } as never),
    ).toEqual({
      channels: {
