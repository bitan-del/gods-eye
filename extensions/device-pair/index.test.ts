...
import type { GodsEyePluginCommandDefinition, PluginCommandContext } from "godseye/plugin-sdk/core";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredGodsEyeTmpDir: vi.fn(() => path.join(os.tmpdir(), "godseye-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredGodsEyeTmpDir: pluginApiMocks.resolvePreferredGodsEyeTmpDir,
...
function createApi(params?: {
  runtime?: GodsEyePluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as GodsEyePluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: GodsEyePluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as GodsEyePluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredGodsEyeTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredGodsEyeTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the GodsEye iOS app:");
    expect(text).toContain("![GodsEye pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the GodsEye iOS app:");
