...
async function writeQrPngTempFile(data: string): Promise<string> {
  const pngBase64 = await renderQrPngBase64(data);
  const tmpRoot = resolvePreferredGodsEyeTmpDir();
...
type QrChannelSender = {
  resolveSend: (api: GodsEyePluginApi) => QrSendFn | undefined;
  createOpts: (params: {
...
function resolveAuthLabel(cfg: GodsEyePluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.GODSEYE_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.GODSEYE_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: GodsEyePluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for GodsEye devices",
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (target && canSendQrPngToChannel(channel)) {
            ...
            try {
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                ctx,
                target,
                caption: ["Scan this QR code with the GodsEye iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the GodsEye iOS app:",
                "",
