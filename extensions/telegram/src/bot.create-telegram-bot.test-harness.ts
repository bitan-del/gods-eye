import { resolveDefaultModelForAgent } from "godseye/plugin-sdk/agent-runtime";
import type { GodsEyeConfig } from "godseye/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "godseye/plugin-sdk/reply-runtime";
import type { MsgContext } from "godseye/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "godseye/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "godseye/plugin-sdk/reply-runtime";
import type { MockFn } from "godseye/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("godseye/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("godseye/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("godseye/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/godseye-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("godseye/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("godseye/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("godseye/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("godseye/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("godseye/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: GodsEyeConfig): {
...
vi.doMock("godseye/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("godseye/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("godseye/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.doMock("godseye/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("godseye/plugin-sdk/infra-runtime", async (importOriginal) => {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: GodsEyeConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: GodsEyeConfig) => {
