...
import { getRuntimeConfigSnapshot } from "godseye/plugin-sdk/config-runtime";
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): GodsEyeConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: GodsEyeConfig): TelegramAccountConfig => {
    try {
