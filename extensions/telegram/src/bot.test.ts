...
import type { PluginInteractiveTelegramHandlerContext } from "godseye/plugin-sdk/core";
...
describe("createTelegramBot", () => {
  ...
  it("blocks DM model-selection callbacks for unpaired users when inline buttons are DM-scoped", async () => {
    ...
    const storePath = `/tmp/godseye-telegram-callback-authz-${process.pid}-${Date.now()}.json`;
  ...
  it("routes compact model callbacks by inferring provider", async () => {
    ...
    const storePath = `/tmp/godseye-telegram-model-compact-${process.pid}-${Date.now()}.json`;
    const config = {
      agents: {
        defaults: {
  ...
  it("resets overrides when selecting the configured default model", async () => {
    ...
    const storePath = `/tmp/godseye-telegram-model-default-${process.pid}-${Date.now()}.json`;
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Gods Eye" },
        },
      },
      me: { id: 999, username: "godseye_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
