...
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "godseye/plugin-sdk/setup";
...
function shouldShowTelegramDmAccessWarning(cfg: GodsEyeConfig, accountId: string): boolean {
...
function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`godseye config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`godseye config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
