...
import type { GodsEyePluginApi, GodsEyePluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<GodsEyePluginApi> = {}): GodsEyePluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<GodsEyePluginToolContext> = {}): GodsEyePluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: GodsEyePluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
...
describe("resolveWindowsLobsterSpawn", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-lobster-win-spawn-"));
    setProcessPlatform("win32");
  });
