...
vi.mock("godseye/plugin-sdk/config-runtime", async () => {
  const original = (await vi.importActual("godseye/plugin-sdk/config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("godseye/plugin-sdk/text-runtime", async () => {
  const original = (await vi.importActual("godseye/plugin-sdk/text-runtime")) as Record<
    string,
    unknown
  >;
  return {
