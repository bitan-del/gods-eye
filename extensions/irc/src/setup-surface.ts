...
import type { ChannelSetupDmPolicy } from "godseye/plugin-sdk/setup";
import type { ChannelSetupWizard } from "godseye/plugin-sdk/setup";
import { formatDocsLink } from "godseye/plugin-sdk/setup";
import type { WizardPrompter } from "godseye/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "Gods Eye",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: godseye channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
