import type { SlackSlashCommandConfig } from "godseye/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "godseye");
  const name = normalizedName || "godseye";
  return {
