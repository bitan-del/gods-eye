...
Read-only diff viewer plugin for **Gods Eye** agents.
...
Set plugin-wide defaults in `~/.godseye/godseye.json`:
...
Before:
Gods Eye supports plugins.

After:
Gods Eye supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/godseye-diffs`).
