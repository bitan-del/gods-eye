...
import type { GodsEyePluginApi, GodsEyePluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("godseye-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<GodsEyePluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: GodsEyePluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<GodsEyePluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as GodsEyePluginApi);
