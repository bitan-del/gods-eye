...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleGodsEyeDevices.length > 0) {
      params.logger.warn(
        `matrix: stale GodsEye devices detected for ${params.auth.userId}: ${deviceHealth.staleGodsEyeDevices.map((device) => device.deviceId).join(", ")}. Run 'godseye matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
