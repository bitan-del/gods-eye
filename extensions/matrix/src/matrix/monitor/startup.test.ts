...
const hoisted = vi.hoisted(() => ({
  ...
  summarizeMatrixDeviceHealth: vi.fn(() => ({
    staleGodsEyeDevices: [] as Array<{ deviceId: string }>,
  })),
...
describe("runMatrixStartupMaintenance", () => {
  beforeEach(() => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockClear().mockReturnValue({ staleGodsEyeDevices: [] });
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockReturnValue({
      staleGodsEyeDevices: [{ deviceId: "DEV123" }],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale GodsEye devices detected for @bot:example.org: DEV123. Run 'godseye matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'godseye matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
