...
import { agentCommandFromIngress } from "godseye/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "godseye/plugin-sdk/agent-runtime";
...
import type { DiscordAccountConfig, TtsConfig } from "godseye/plugin-sdk/config-runtime";
...
import { transcribeAudioFile } from "godseye/plugin-sdk/media-understanding-runtime";
...
import { parseTtsDirectives } from "godseye/plugin-sdk/speech";
import { textToSpeech } from "godseye/plugin-sdk/speech-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: GodsEyeConfig; override?: TtsConfig }): {
  cfg: GodsEyeConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredGodsEyeTmpDir(), "discord-voice-"));
