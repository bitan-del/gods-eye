...
const BASE_CFG: Config = {
  agents: {
    defaults: {
      ...
      workspace: "/tmp/godseye",
    },
  },
  messages: {
    inbound: { debounceMs: 0 },
  },
  session: { store: "/tmp/godseye-sessions.json" },
};
...
describe("discord tool result dispatch", () => {
  it("accepts guild messages when mentionPatterns match", async () => {
    const cfg = createMentionRequiredGuildConfig({
      messages: {
        inbound: { debounceMs: 0 },
        groupChat: { mentionPatterns: ["\\bgodseye\\b"] },
      },
    } as Partial<Config>);
    ...
    await handler(createGuildMessageEvent({ messageId: "m2", content: "godseye: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Gods Eye" },
          },
        },
      }),
      client,
