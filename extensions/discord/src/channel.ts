...
} from "godseye/plugin-sdk/allowlist-config-edit";
...
import { createOpenProviderConfiguredRouteWarningCollector } from "godseye/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "godseye/plugin-sdk/channel-targets";
...
function hasDiscordExecApprovalDmRoute(cfg: GodsEyeConfig): boolean {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). GodsEye adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
