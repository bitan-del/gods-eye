...
metadata: { "godseye": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Gods Eye `sessions_spawn`
- **File I/O** == Gods Eye `read`/`write`
- **Remote fetch** == Gods Eye `web_fetch` (or `exec` with curl when POST is required)
