...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from GodsEye config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "godseye-session-1234",
        backendId: "openshell",
        runtimeLabel: "godseye-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "godseye-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "godseye-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "godseye-session-5678",
        backendId: "openshell",
        runtimeLabel: "godseye-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "godseye",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "godseye-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "godseye-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("godseye-openshell-fs-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("godseye-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("godseye-openshell-agent-");
  ...
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("godseye-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("godseye-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("godseye-openshell-remote-agent-");
