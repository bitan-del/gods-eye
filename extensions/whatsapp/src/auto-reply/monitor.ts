import { resolveInboundDebounceMs } from "godseye/plugin-sdk/channel-inbound";
import { formatCliCommand } from "godseye/plugin-sdk/cli-runtime";
import { waitForever } from "godseye/plugin-sdk/cli-runtime";
import { hasControlCommand } from "godseye/plugin-sdk/command-auth";
import { loadConfig } from "godseye/plugin-sdk/config-runtime";
import { formatDurationPrecise } from "godseye/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "godseye/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "godseye/plugin-sdk/reply-history";
import { getReplyFromConfig } from "godseye/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "godseye/plugin-sdk/routing";
import { logVerbose } from "godseye/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "godseye/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("godseye channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("godseye channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
