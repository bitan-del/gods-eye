import { loadConfig, type GodsEyeConfig } from "godseye/plugin-sdk/config-runtime";
...
import { getChildLogger } from "godseye/plugin-sdk/text-runtime";
...
import { toWhatsappJid } from "godseye/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: GodsEyeConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
