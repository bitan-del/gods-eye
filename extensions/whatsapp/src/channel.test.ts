...
const hoisted = vi.hoisted(() => ({
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/godseye-whatsapp-test",
  })),
}));
...
vi.mock("godseye/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("godseye/plugin-sdk/setup")>(
    "godseye/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/godseye-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("godseye channels login"),
      "WhatsApp",
    );
  });
