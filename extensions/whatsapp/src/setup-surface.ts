...
function mergeWhatsAppConfig(
  cfg: GodsEyeConfig,
  patch: Partial<NonNullable<NonNullable<GodsEyeConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: GodsEyeConfig, dmPolicy: DmPolicy): GodsEyeConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: GodsEyeConfig, allowFrom?: string[]): GodsEyeConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: GodsEyeConfig, selfChatMode: boolean): GodsEyeConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: GodsEyeConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so GodsEye can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for GodsEye" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("godseye channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
