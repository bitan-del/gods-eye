...
const CONFIG_KEY = Symbol.for("godseye:testConfigMock");
...
vi.mock("godseye/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/media-runtime")>();
  const mockModule = Object.create(null) as Record<string, unknown>;
  Object.defineProperties(mockModule, Object.getOwnPropertyDescriptors(actual));
...
vi.mock("godseye/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("godseye/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/godseye-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("godseye:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("godseye:lastSocket")];
  if (typeof getter === "function") {
