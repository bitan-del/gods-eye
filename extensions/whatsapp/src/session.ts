...
import { VERSION } from "godseye/plugin-sdk/cli-runtime";
import { danger, success } from "godseye/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "godseye/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "godseye/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("godseye channels login")}`,
              ),
            );
          }
        }
