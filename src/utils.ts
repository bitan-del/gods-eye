...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".godseye");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.GODSEYE_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$GODSEYE_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via GODSEYE_STATE_DIR.
