...
export const POSIX_GODSEYE_TMP_DIR = "/tmp/godseye";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredGodsEyeTmpDirOptions = {
...
export function resolvePreferredGodsEyeTmpDir(
  options: ResolvePreferredGodsEyeTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "godseye" : `godseye-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[godseye] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback GodsEye temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback GodsEye temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_GODSEYE_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_GODSEYE_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_GODSEYE_TMP_DIR)) {
      return POSIX_GODSEYE_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_GODSEYE_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_GODSEYE_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_GODSEYE_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_GODSEYE_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_GODSEYE_TMP_DIR;
