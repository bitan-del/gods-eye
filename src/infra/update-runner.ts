...
const DEFAULT_PACKAGE_NAME = "godseye";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["godseye", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step("godseye doctor", doctorArgv, gitRoot, { GODSEYE_UPDATE_IN_PROGRESS: "1" }),
    );
