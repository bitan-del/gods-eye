...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.GODSEYE_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Gods Eye";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(GodsEye\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.GODSEYE_MDNS_HOSTNAME?.trim() || "godseye";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
