...
import { POSIX_GODSEYE_TMP_DIR, resolvePreferredGodsEyeTmpDir } from "./tmp-godseye-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredGodsEyeTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `godseye-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredGodsEyeTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_GODSEYE_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredGodsEyeTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredGodsEyeTmpDir", () => {
  it("prefers /tmp/godseye when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_GODSEYE_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/godseye when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_GODSEYE_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/godseye when /tmp/godseye is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/godseye when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/godseye exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_GODSEYE_TMP_DIR) {
  ...
  it("falls back when /tmp/godseye is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/godseye is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/godseye is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/godseye permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_GODSEYE_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_GODSEYE_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_GODSEYE_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/godseye after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_GODSEYE_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_GODSEYE_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_GODSEYE_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_GODSEYE_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback GodsEye temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "godseye");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredGodsEyeTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_GODSEYE_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback GodsEye temp dir/);
  });
});
