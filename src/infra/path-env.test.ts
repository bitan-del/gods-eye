...
let ensureGodsEyeCliOnPath: typeof import("./path-env.js").ensureGodsEyeCliOnPath;

describe("ensureGodsEyeCliOnPath", () => {
  const envKeys = [
    "PATH",
    "GODSEYE_PATH_BOOTSTRAPPED",
    "GODSEYE_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureGodsEyeCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/godseye-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "godseye");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureGodsEyeCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling godseye exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.GODSEYE_PATH_BOOTSTRAPPED;
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "godseye");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.GODSEYE_PATH_BOOTSTRAPPED;
      delete process.env.GODSEYE_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.GODSEYE_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/godseye-path/case-linuxbrew");
