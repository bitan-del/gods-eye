...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.GODSEYE_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for GodsEye. You can keep running the web gateway without it: \`${formatCliCommand("godseye gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
