...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/godseye-state", "devices")).toEqual({
      dir: path.join("/tmp/godseye-state", "devices"),
      pendingPath: path.join("/tmp/godseye-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/godseye-state", "devices", "paired.json"),
    });
  });
