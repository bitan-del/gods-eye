 ...
 * returning control to the caller (typically `triggerGodsEyeRestart` →
...
/**
 * Parse godseye gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("godseye")) {
 ...
 * Returns only PIDs that belong to godseye gateway processes (not the current process).
 */
