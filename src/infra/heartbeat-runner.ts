...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: GodsEyeConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: GodsEyeConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

function resolveHeartbeatConfig(cfg: GodsEyeConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: GodsEyeConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: GodsEyeConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: GodsEyeConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: GodsEyeConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
