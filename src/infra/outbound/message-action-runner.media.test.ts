...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred GodsEye tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideGodsEye = path.join(os.tmpdir(), "outside-godseye", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
