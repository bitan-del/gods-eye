import { mapAllowFromEntries } from "godseye/plugin-sdk/channel-config-helpers";
...
export function resolveOutboundTarget(params: {
  ...
  if (params.channel === INTERNAL_MESSAGE_CHANNEL) {
    return {
      ok: false,
      error: new Error(
        `Delivering to WebChat is not supported via \`${formatCliCommand("godseye agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
      ),
    };
  }
