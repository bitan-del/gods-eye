...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: GodsEyeConfig | null = null;
  ...
  get(key: string, cfg: GodsEyeConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: GodsEyeConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: GodsEyeConfig): void {
