...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.godseye/.env as fallback without overriding CWD .env", async () => {
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "GODSEYE_STATE_DIR=./evil-state",
            "GODSEYE_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.GODSEYE_CONFIG_PATH).toBeUndefined();
  ...
  it("blocks GODSEYE_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.GODSEYE_STATE_DIR).toBeUndefined();
        expect(process.env.GODSEYE_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (GODSEYE_AGENT_DIR, PI_CODING_AGENT_DIR, GODSEYE_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "GODSEYE_AGENT_DIR=./evil-agent",
            "PI_CODING_AGENT_DIR=./evil-coding",
            "GODSEYE_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );
        ...
        expect(process.env.GODSEYE_AGENT_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.GODSEYE_OAUTH_DIR).toBeUndefined();
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "GODSEYE_STATE_DIR=./evil-state\n");
