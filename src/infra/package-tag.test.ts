...
describe("normalizePackageTagInput", () => {
  const packageNames = ["godseye", "@godseye/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("godseye@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@godseye/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("godseye@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("godseye", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @godseye/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("godseyeer@beta", packageNames)).toBe("godseyeer@beta");
  });
});
