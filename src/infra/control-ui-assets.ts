...
import { resolveGodsEyePackageRoot, resolveGodsEyePackageRootSync } from "./godseye-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveGodsEyePackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "godseye" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/godseye -> .../dist/index.js)
