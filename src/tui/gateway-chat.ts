...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set GODSEYE_GATEWAY_TOKEN/GODSEYE_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.GODSEYE_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.GODSEYE_GATEWAY_PASSWORD);
