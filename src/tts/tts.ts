...
export function resolveTtsConfig(cfg: GodsEyeConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.GODSEYE_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: GodsEyeConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: GodsEyeConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredGodsEyeTmpDir();
