...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("godseye-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("godseye-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("godseye-lid-a-", async (first) => {
      await withTempDir("godseye-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.godseye when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "godseye-config-dir-"));
    try {
  ...
  it("expands GODSEYE_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/godseye-home",
      GODSEYE_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/godseye-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers GODSEYE_HOME over HOME", () => {
    vi.stubEnv("GODSEYE_HOME", "/srv/godseye-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/godseye-home"));
...
describe("shortenHomePath", () => {
  it("uses $GODSEYE_HOME prefix when GODSEYE_HOME is set", () => {
    vi.stubEnv("GODSEYE_HOME", "/srv/godseye-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/godseye-home")}/.godseye/godseye.json`)).toBe(
      "$GODSEYE_HOME/.godseye/godseye.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $GODSEYE_HOME replacement when GODSEYE_HOME is set", () => {
    vi.stubEnv("GODSEYE_HOME", "/srv/godseye-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/godseye-home")}/.godseye/godseye.json`),
    ).toBe("config: $GODSEYE_HOME/.godseye/godseye.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/godseye", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "godseye"),
    );
  });
  ...
  it("prefers GODSEYE_HOME for tilde expansion", () => {
    vi.stubEnv("GODSEYE_HOME", "/srv/godseye-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/godseye")).toBe(path.resolve("/srv/godseye-home", "godseye"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/godseye", env)).toBe(path.resolve("/srv/godseye-home", "godseye"));
  });
