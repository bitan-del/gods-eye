...
This directory contains hooks that ship with Gods Eye. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.godseye/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.godseye/godseye.json`:
...
2. Restart gateway: `pkill -9 -f 'godseye.*gateway' && pnpm godseye gateway`
3. Enable the hook: `godseye hooks enable my-hook`
