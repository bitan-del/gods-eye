...
2. **Appends to log file** - Writes a JSON line to `~/.godseye/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.godseye/logs/commands.log ~/.godseye/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/godseye`:
   ```
   /home/username/.godseye/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
