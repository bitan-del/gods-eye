...
`;
    ...
    expect(parsed.godseye.emoji).toBe("💾");
    expect(parsed.godseye.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.godseye.emoji).toBe("📝");
    expect(parsed.godseye.events).toEqual(["command"]);
    expect(parsed.godseye.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.godseye.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"godseye": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"godseye": {"events": ["test"]}}');
  });
...
describe("resolveGodsEyeMetadata", () => {
  it("extracts godseye metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveGodsEyeMetadata(frontmatter);
  ...
  it("returns undefined when godseye key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        godseye: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with GodsEye" },
            { id: "npm", kind: "npm", package: "@godseye/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@godseye/hook");
  });
    ...
    "godseye":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with GodsEye" }],
      },
  }
---
...
`;
    ...
    const godseye = resolveGodsEyeMetadata(frontmatter);
    expect(godseye).toBeDefined();
    expect(godseye?.emoji).toBe("💾");
    expect(godseye?.events).toEqual(["command:new", "command:reset"]);
    expect(godseye?.requires?.config).toEqual(["workspace.dir"]);
    expect(godseye?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const godseye = resolveGodsEyeMetadata(frontmatter);
    expect(godseye?.emoji).toBe("disk");
    expect(godseye?.events).toEqual(["command:new"]);
  });
});
