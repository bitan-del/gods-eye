...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "godseye-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.GODSEYE_BUNDLED_HOOKS_DIR;
    process.env.GODSEYE_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.GODSEYE_BUNDLED_HOOKS_DIR;
    } else {
      process.env.GODSEYE_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"godseye":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): GodsEyeConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("godseye-plugin");
  ...
  it("does not treat Claude hooks.json bundles as GodsEye hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".godseye", "extensions", "claude-bundle");
