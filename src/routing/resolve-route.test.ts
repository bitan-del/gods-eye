...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: GodsEyeConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: GodsEyeConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/godseye-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<GodsEyeConfig["bindings"]>[number];
