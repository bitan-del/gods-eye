...
function listAgents(cfg: GodsEyeConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: GodsEyeConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<GodsEyeConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: GodsEyeConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: GodsEyeConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: GodsEyeConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<GodsEyeConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  GodsEyeConfig,
  {
    bindingsRef: GodsEyeConfig["bindings"];
    agentsRef: GodsEyeConfig["agents"];
    sessionRef: GodsEyeConfig["session"];
...
function resolveRouteCacheForConfig(cfg: GodsEyeConfig): Map<string, ResolvedAgentRoute> {
