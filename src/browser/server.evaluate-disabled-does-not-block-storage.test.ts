  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          godseye: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.GODSEYE_GATEWAY_PORT;
    process.env.GODSEYE_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.GODSEYE_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.GODSEYE_GATEWAY_PASSWORD;
