...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "godseye" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.GODSEYE_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "godseye",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/godseye" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    godseye: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchGodsEyeChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "godseye";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.GODSEYE_GATEWAY_PORT;
  process.env.GODSEYE_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.GODSEYE_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.GODSEYE_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.GODSEYE_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.GODSEYE_GATEWAY_PASSWORD;
  } else {
    process.env.GODSEYE_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
