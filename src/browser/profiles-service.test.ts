...
vi.mock("./chrome.js", () => ({
  resolveGodsEyeUserDataDir: vi.fn(() => "/tmp/godseye-test/godseye/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveGodsEyeUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "godseye-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveGodsEyeUserDataDir).mockReturnValue(userDataDir);
