...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".godseye-profile-decorated");
}
 ...
 */
export function decorateGodsEyeProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_GODSEYE_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_GODSEYE_BROWSER_COLOR).toUpperCase();
