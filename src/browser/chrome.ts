...
export function resolveGodsEyeUserDataDir(profileName = DEFAULT_GODSEYE_BROWSER_PROFILE_NAME) {
...
export function buildGodsEyeChromeLaunchArgs(params: {
...
export async function launchGodsEyeChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveGodsEyeUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_GODSEYE_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildGodsEyeChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateGodsEyeProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 godseye browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`godseye browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`godseye browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 godseye browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopGodsEyeChrome(
