...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredGodsEyeTmpDir: vi.fn(() => "/tmp/godseye"),
}));
vi.mock("../infra/tmp-godseye-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredGodsEyeTmpDir.mockReturnValue("/tmp/godseye");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".godseye-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredGodsEyeTmpDir.mockReturnValue("/tmp/godseye-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "godseye-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "godseye-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredGodsEyeTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "godseye-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "godseye-preferred", "downloads")}${path.sep}`),
    );
  });
