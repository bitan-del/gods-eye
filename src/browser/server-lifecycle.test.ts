...
const { stopGodsEyeChromeMock } = vi.hoisted(() => ({
  stopGodsEyeChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopGodsEyeChrome: stopGodsEyeChromeMock,
}));
...
beforeEach(async () => {
  ...
  stopGodsEyeChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["godseye", "user"]);
    ...
    expect(stopMap.godseye).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopGodsEyeChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("godseye browser stop failed: Error: oops");
  });
});
