...
describe("canvas host state dir defaults", () => {
  ...
  it("uses GODSEYE_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("godseye-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("GodsEye Canvas");
      } finally {
        await handler.close();
      }
    });
