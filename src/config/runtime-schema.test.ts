...
import type { ConfigFileSnapshot, GodsEyeConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => GodsEyeConfig>());
const mockReadConfigFileSnapshot = vi.hoisted(() => vi.fn<() => Promise<ConfigFileSnapshot>>());
const mockLoadGodsEyePlugins = vi.hoisted(() => vi.fn());
...
vi.mock("../plugins/loader.js", () => ({
  loadGodsEyePlugins: (...args: unknown[]) => mockLoadGodsEyePlugins(...args),
}));
...
function makeSnapshot(params: { valid: boolean; config?: GodsEyeConfig }): ConfigFileSnapshot {
  return {
...
describe("readBestEffortRuntimeConfigSchema", () => {
  ...
  it("uses scoped plugin registry channels for valid configs", async () => {
    ...
    mockLoadGodsEyePlugins.mockReturnValueOnce({
      plugins: [
        {
          id: "demo",
          name: "Demo",
    ...
    expect(mockLoadGodsEyePlugins).toHaveBeenCalledWith(
  ...
  it("does not fall back to active registry channels when invalid fallback load throws", async () => {
    mockReadConfigFileSnapshot.mockResolvedValueOnce(makeSnapshot({ valid: false }));
    mockLoadGodsEyePlugins.mockImplementationOnce(() => {
...
describe("loadGatewayRuntimeConfigSchema", () => {
  ...
  it("preserves gateway channel source and loader options", async () => {
    ...
    expect(mockLoadGodsEyePlugins.mock.calls[0]?.[0]?.activate).toBeUndefined();
    expect(channelProps?.telegram).toBeTruthy();
