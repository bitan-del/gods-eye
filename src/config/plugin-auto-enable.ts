...
const ENV_CATALOG_PATHS = ["GODSEYE_PLUGIN_CATALOG_PATHS", "GODSEYE_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: GodsEyeConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: GodsEyeConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.godseye) || !isRecord(entry.godseye.channel)) {
      continue;
    }
    const channel = entry.godseye.channel;
...
function collectCandidateChannelIds(cfg: GodsEyeConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: GodsEyeConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: GodsEyeConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: GodsEyeConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: GodsEyeConfig, pluginId: string): GodsEyeConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
