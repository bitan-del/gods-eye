...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("godseye-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "godseye-sandbox-browser-global" },
      agentBrowser: { network: "godseye-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("godseye-sandbox-browser-agent");
  });
