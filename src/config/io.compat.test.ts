...
import { parseGodsEyeVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.godseye/godseye.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".godseye", 19001);
  ...
  it("defaults to ~/.godseye/godseye.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".godseye", "godseye.json"));
    });
  });

  it("uses GODSEYE_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { GODSEYE_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".godseye", "godseye.json"));
    });
  });

  it("honors explicit GODSEYE_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".godseye", 20002, "custom.json");
      const io = createIoForHome(home, { GODSEYE_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "godseye.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseGodsEyeVersion(VERSION);
    if (!parsedVersion) {
    ...
    await withTempHome(async (home) => {
      ...
      expect(logger.warn).not.toHaveBeenCalledWith(
        expect.stringContaining("Config was last written by a newer GodsEye"),
      );
      expect(io.configPath).toBe(configPath);
    });
  });
