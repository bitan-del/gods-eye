...
describe("config io write", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-config-io-"));
  });
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".godseye", "godseye.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".godseye", "logs", "config-audit.jsonl");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withSuiteHome(async (home) => {
      ...
      const invalidConfig: GodsEyeConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "godseye config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'godseye config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(lines.length).toBeGreaterThan(0);
      await expect(
        fs.stat(path.join(home, ".godseye", "logs", "config-audit.jsonl")),
      ).resolves.toBeDefined();
      await expect(
        fs.stat(path.resolve("undefined", ".godseye", "logs", "config-audit.jsonl")),
      ).rejects.toThrow();
    });
  });
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withSuiteHome(async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        gatewayPatch: { bind: "loopback" },
        env: {
          GODSEYE_WATCH_MODE: "1",
          GODSEYE_WATCH_SESSION: "watch-session-1",
          GODSEYE_WATCH_COMMAND: "gateway --force",
        } as NodeJS.ProcessEnv,
      });
