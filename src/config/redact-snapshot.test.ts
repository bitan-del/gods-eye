...
import type { ConfigFileSnapshot } from "./types.godseye.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.godseye/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/godseye/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/godseye/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/godseye/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/godseye/nickserv-password.txt");
