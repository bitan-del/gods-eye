...
import type { GodsEyeConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: GodsEyeConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: GodsEyeConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: GodsEyeConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = GodsEyeSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as GodsEyeConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as GodsEyeConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as GodsEyeConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as GodsEyeConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; GodsEye mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: GodsEyeConfig | null | undefined;

  const ensureCompatConfig = (): GodsEyeConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native GodsEye config schema.
