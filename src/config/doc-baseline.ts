...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.GODSEYE_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveGodsEyePackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    GODSEYE_STATE_DIR: path.join(os.tmpdir(), "godseye-config-doc-baseline-state"),
    GODSEYE_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.GODSEYE_CONFIG_DOC_BASELINE_DEBUG === "1"
