...
import type { GodsEyeConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  godseye config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  godseye config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: GodsEyeConfig,
  pathSegments: string[],
): { changed: boolean; next: GodsEyeConfig } {
...
function coerceConfig(value: unknown): GodsEyeConfig {
...
function stampConfigVersion(cfg: GodsEyeConfig): GodsEyeConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: GodsEyeConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer GodsEye (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as GodsEyeConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): GodsEyeConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as GodsEyeConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: GodsEyeConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as GodsEyeConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.GODSEYE_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.GODSEYE_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.GODSEYE_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.GODSEYE_WATCH_SESSION === "string" &&
        deps.env.GODSEYE_WATCH_SESSION.trim().length > 0
          ? deps.env.GODSEYE_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.GODSEYE_WATCH_COMMAND === "string" &&
        deps.env.GODSEYE_WATCH_COMMAND.trim().length > 0
          ? deps.env.GODSEYE_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `GODSEYE_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: GodsEyeConfig | null = null;
let runtimeConfigSourceSnapshot: GodsEyeConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.GODSEYE_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.GODSEYE_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): GodsEyeConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): GodsEyeConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: GodsEyeConfig;
...
export function projectConfigOntoRuntimeSourceSnapshot(config: GodsEyeConfig): GodsEyeConfig {
...
export function loadConfig(): GodsEyeConfig {
...
export async function readBestEffortConfig(): Promise<GodsEyeConfig> {
...
export async function writeConfigFile(
  ...
  // edits to godseye.json remain visible (no stale snapshot).
}
