...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "godseye.json"),
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ GODSEYE_STATE_DIR: sharedChannelSecurityStateDir }, () =>
  ...
  const runSharedExtensionsAudit = async (config: GodsEyeConfig) => {
    return runSecurityAudit({
      ...
      configPath: path.join(sharedExtensionsStateDir, "godseye.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-security-audit-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { GODSEYE_HOME: isolatedHome });
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/godseye-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "godseye-sbx-browser-old\ngodseye-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "godseye-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "godseye-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("godseye-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("godseye-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "godseye-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "godseye-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-godseye.json");
    ...
    await Promise.all(
      cases
        ...
        .map(async (testCase) => {
          ...
          const configPath = path.join(fixture.stateDir, "godseye.json");
          await fs.writeFile(configPath, "{}\n", "utf-8");
          if (!isWindows) {
            await fs.chmod(configPath, 0o600);
          }
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.godseye/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      ...
      configPath: "/Users/test/Dropbox/.godseye/godseye.json",
    });
    ...
    const cases = [
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@godseye/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@godseye/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.GODSEYE_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.GODSEYE_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
