...
type ConfiguredBinding = NonNullable<GodsEyeConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<GodsEyeConfig>,
): GodsEyeConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: GodsEyeConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/godseye" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
