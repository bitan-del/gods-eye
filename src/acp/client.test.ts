...
const createTempDir = () => tempDirs.make("godseye-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets GODSEYE_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "godseye",
    });

    expect(env.GODSEYE_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("godseye");
  });

  it("overrides pre-existing GODSEYE_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      GODSEYE_SHELL: "wrong",
    });
    expect(env.GODSEYE_SHELL).toBe("acp-client");
  ...
  it("preserves GODSEYE_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        GODSEYE_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["GODSEYE_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default GodsEye bridge", () => {
    ...
    expect(env.GODSEYE_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.GODSEYE_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "godseye",
        serverArgs: ["acp"],
        defaultServerCommand: "godseye",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("GODSEYE_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "godseye", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "godseye", "dist", "entry.js");
    const shimPath = path.join(dir, "godseye.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\godseye\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/godseye-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/godseye-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/godseye-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
