...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        GODSEYE_BASE_ENV: "base",
        GODSEYE_TO_REMOVE: undefined,
      },
      env: {
        GODSEYE_TEST_ENV: "ok",
      },
    ...
    expect(resolved.GODSEYE_BASE_ENV).toBe("base");
    expect(resolved.GODSEYE_TEST_ENV).toBe("ok");
    expect(resolved.GODSEYE_TO_REMOVE).toBeUndefined();
    expect(resolved.GODSEYE_CLI).toBe(GODSEYE_CLI_ENV_VALUE);
  });
