...
vi.mock("../infra/path-env.js", () => ({
  ensureGodsEyeCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "godseye", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "godseye", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "godseye", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "godseye", "--container", "demo", "status"]);
