...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `godseye doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
  ...
  it("installs marketplace plugins and persists config", async () => {
    ...
    const installedCfg = {
      ...
      plugins: {
        ...
        installs: {
          alpha: {
            source: "marketplace",
            installPath: "/tmp/godseye-state/extensions/alpha",
          },
        },
      },
    } as GodsEyeConfig;
    ...
    installPluginFromMarketplace.mockResolvedValue({
      ok: true,
      pluginId: "alpha",
      targetDir: "/tmp/godseye-state/extensions/alpha",
      version: "1.2.3",
  ...
  it("installs ClawHub plugins and persists source metadata", async () => {
      ...
      plugins: {
        ...
        installs: {
          demo: {
            ...
            installPath: "/tmp/godseye-state/extensions/demo",
            clawhubPackage: "demo",
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: true,
      pluginId: "demo",
      targetDir: "/tmp/godseye-state/extensions/demo",
      version: "1.2.3",
      packageName: "demo",
      clawhub: {
        source: "clawhub",
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "godseye skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "godseye skills install demo" instead.');
  });
  ...
  it("falls back to installing hook packs from npm specs", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing godseye.plugin.json",
    });
