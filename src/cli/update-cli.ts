...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["godseye update", "Update a source checkout (git)"],
        ["godseye update --channel beta", "Switch to beta channel (git + npm)"],
        ["godseye update --channel dev", "Switch to dev channel (git + npm)"],
        ["godseye update --tag beta", "One-off update to a dist-tag or version"],
        ["godseye update --tag main", "One-off package install from GitHub main"],
        ["godseye update --dry-run", "Preview actions without changing anything"],
        ["godseye update --no-restart", "Update without restarting the service"],
        ["godseye update --json", "Output result as JSON"],
        ["godseye update --yes", "Non-interactive (accept downgrade prompts)"],
        ["godseye update wizard", "Interactive update wizard"],
        ["godseye --update", "Shorthand for godseye update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run godseye update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.gods-eye.org/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["godseye update status", "Show channel + version status."],
          ["godseye update status --json", "JSON output."],
          ["godseye update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.gods-eye.org/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
