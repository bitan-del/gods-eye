...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv("GODSEYE_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("godseye devices approve <requestId>");
  });
  ...
  it("uses GODSEYE_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("GODSEYE_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
