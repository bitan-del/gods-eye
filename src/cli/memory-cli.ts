    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["godseye memory status", "Show index and provider status."],
          ["godseye memory status --deep", "Probe embedding provider readiness."],
          ["godseye memory index --force", "Force a full reindex."],
          ['godseye memory search "meeting notes"', "Quick search using positional query."],
          [
            'godseye memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["godseye memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.gods-eye.org/cli/memory")}\n`,
    );
