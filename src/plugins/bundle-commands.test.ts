...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("godseye-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("godseye-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.GODSEYE_HOME;
      delete process.env.GODSEYE_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".godseye", "extensions", "compound-bundle");
