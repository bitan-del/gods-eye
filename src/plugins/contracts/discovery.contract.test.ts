...
describe("provider discovery contract", () => {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("godseye/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("godseye/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("godseye/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
    ...
    vi.doMock("godseye/plugin-sdk/provider-setup", async () => {
      const actual = await vi.importActual<object>("godseye/plugin-sdk/provider-setup");
      return {
        ...actual,
    ...
    vi.doMock("godseye/plugin-sdk/ollama-setup", async () => {
      const actual = await vi.importActual<object>("godseye/plugin-sdk/ollama-setup");
      return {
        ...actual,
        buildOllamaProvider: (...args: unknown[]) => buildOllamaProviderMock(...args),
      };
