...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".godseye", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".godseye", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "godseye-home");
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".godseye", "extensions"),
    });
  });
});
