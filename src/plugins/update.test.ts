...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-godseye",
      targetDir: "/tmp/opik-godseye",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-godseye": {
              source: "npm",
              spec: "@opik/opik-godseye",
              integrity: "sha512-old",
              installPath: "/tmp/opik-godseye",
            },
          },
        },
      },
      pluginIds: ["opik-godseye"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @godseye/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @godseye/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "godseye-codex-app-server",
      targetDir: "/tmp/godseye-codex-app-server",
        ...
        plugins: {
          installs: {
            "godseye-codex-app-server": {
              ...
              resolvedSpec: "godseye-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "godseye-codex-app-server@beta",
        expectedPluginId: "godseye-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["godseye-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "godseye-codex-app-server@beta",
      installPath: "/tmp/godseye-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "godseye-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "godseye-codex-app-server@0.2.0-beta.4",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
      ...
      config: {
        plugins: {
          installs: {
            "godseye-codex-app-server": {
              source: "npm",
              spec: "godseye-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/godseye-codex-app-server",
            },
          },
    ...
    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "godseye-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@godseye/voice-call",
      targetDir: "/tmp/godseye-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@godseye/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@godseye/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@godseye/voice-call");
    expect(result.config.plugins?.entries?.["@godseye/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@godseye/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@godseye/voice-call",
      installPath: "/tmp/godseye-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { GODSEYE_HOME: "/srv/godseye-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/godseye-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
