...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: GodsEyePluginDefinition;
  register?: GodsEyePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as GodsEyePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as GodsEyePluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.GODSEYE_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverGodsEyePlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverGodsEyePlugins>["candidates"][number];
  right: ReturnType<typeof discoverGodsEyePlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadGodsEyePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadGodsEyePlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = (modulePath: string) => {
    ...
    // Pass loader's moduleUrl so the godseye root can always be resolved even when
    // loading external plugins from outside the installation directory (e.g. ~/.godseye/extensions/).
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into GodsEye yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: GodsEyePluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as GodsEyePluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
