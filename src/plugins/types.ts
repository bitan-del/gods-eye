 ...
 */
export type GodsEyePluginConfigSchema = {
...
export type GodsEyePluginToolContext = {
...
export type GodsEyePluginToolFactory = (
  ctx: GodsEyePluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type GodsEyePluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type GodsEyePluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<GodsEyeConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). GodsEye uses this to expose
 ...
 *
 * Runs after GodsEye resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common GodsEye auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and GodsEye wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before GodsEye builds its generic
...
/**
 * Provider-owned stream wrapper hook after GodsEye applies its generic
 ...
 *
 * Return `true` or `false` to override GodsEye's built-in provider cache TTL
 ...
 *
 * Runs only after GodsEye exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after GodsEye loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * GodsEye calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic GodsEye wrappers.
   *
   ...
   *
   * Called after GodsEye resolves the raw configured credential but before the
   ...
   * hint than GodsEye's generic auth-store guidance.
   */
   ...
   * `errorMessage` when GodsEye should surface a provider-specific hint for
   ...
   * merging. GodsEye deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * GodsEye uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `godseye doctor`.
   *
   ...
   *
   * GodsEye calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep GodsEye's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: GodsEyeConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: GodsEyeConfig, value: unknown) => void;
  applySelectionConfig?: (config: GodsEyeConfig) => GodsEyeConfig;
...
export type GodsEyePluginGatewayMethod = {
  method: string;
 ...
 */
export type GodsEyePluginCommandDefinition = {
...
export type GodsEyePluginHttpRouteAuth = "gateway" | "plugin";
export type GodsEyePluginHttpRouteMatch = "exact" | "prefix";

export type GodsEyePluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type GodsEyePluginHttpRouteParams = {
  path: string;
  handler: GodsEyePluginHttpRouteHandler;
  auth: GodsEyePluginHttpRouteAuth;
  match?: GodsEyePluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type GodsEyePluginCliContext = {
  program: Command;
  config: GodsEyeConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type GodsEyePluginCliRegistrar = (ctx: GodsEyePluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type GodsEyePluginServiceContext = {
  config: GodsEyeConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type GodsEyePluginService = {
  id: string;
  start: (ctx: GodsEyePluginServiceContext) => void | Promise<void>;
  stop?: (ctx: GodsEyePluginServiceContext) => void | Promise<void>;
};

export type GodsEyePluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type GodsEyePluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: GodsEyePluginApi) => void | Promise<void>;
  activate?: (api: GodsEyePluginApi) => void | Promise<void>;
};

export type GodsEyePluginModule =
  | GodsEyePluginDefinition
  | ((api: GodsEyePluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type GodsEyePluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | GodsEyePluginToolFactory,
    opts?: GodsEyePluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: GodsEyePluginHookOptions,
  ) => void;
  registerHttpRoute: (params: GodsEyePluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: GodsEyePluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: GodsEyePluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: GodsEyePluginService) => void;
   ...
   */
  registerCommand: (command: GodsEyePluginCommandDefinition) => void;
...
export type PluginFormat = "godseye" | "bundle";
