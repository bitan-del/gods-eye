...
import { clearPluginDiscoveryCache, discoverGodsEyePlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("godseye-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverGodsEyePlugins>[0],
) {
  return discoverGodsEyePlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      godseye: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverGodsEyePlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".godseye", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".godseye", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverGodsEyePlugins({
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@godseye/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("godseye");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@godseye/escape-pack",
  ...
  it("skips missing package extension entries without escape diagnostics", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@godseye/missing-entry-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverGodsEyePlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverGodsEyePlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverGodsEyePlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
