...
const APPROVALS_PATH = "~/.godseye/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "godseye-app-server:thread:",
  "godseye-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("godseye.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("godseye.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to GodsEye instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
