...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      godseye: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(bundleTempRoot, "godseye-plugin-install-"));
  return suiteTempRoot;
}
...
describe("installPluginFromArchive", () => {
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@godseye/voice-call" });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@godseye/zipper" });
  });

  it("installs flat-root plugin archives from ClawHub-style downloads", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
        name: "@godseye/rootless",
        version: "0.0.1",
        godseye: { extensions: ["./dist/index.js"] },
      },
  ...
  it("rejects packages without godseye.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@godseye/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("godseye.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_GODSEYE_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when godseye.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@godseye/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing godseye.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_GODSEYE_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without godseye.extensions");
  });
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        godseye: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.godseye).toBeUndefined();
  ...
  it("rejects plugins whose minHostVersion is newer than the current host", async () => {
    vi.stubEnv("GODSEYE_VERSION", "2026.3.21");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      godseye?: { install?: Record<string, unknown> };
    };
    manifest.godseye = {
      ...manifest.godseye,
      install: {
        ...manifest.godseye?.install,
    ...
    expect(result.error).toContain("requires GodsEye >=2026.3.22, but this host is 2026.3.21");
  ...
  it("rejects plugins with invalid minHostVersion metadata", async () => {
    ...
    expect(result.error).toContain("invalid package.json godseye.install.minHostVersion");
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });

  it("reports unknown host versions distinctly for minHostVersion-gated plugins", async () => {
    vi.stubEnv("GODSEYE_VERSION", "unknown");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      godseye?: { install?: Record<string, unknown> };
  ...
  it("uses godseye.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@godseye/cognee-godseye"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("keeps scoped install ids aligned across manifest and package-name cases", async () => {
    const scenarios = [
      ...
      },
      {
        ...
        expectedPluginId: "@godseye/test-plugin",
        install: (pluginDir: string, extensionsDir: string) =>
          installPluginFromDir({
            dirPath: pluginDir,
            extensionsDir,
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@godseye/voice-call@0.0.1",
              name: "@godseye/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@godseye/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@godseye/voice-call@0.0.1",
    });
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@godseye/voice-call@0.0.2-beta.1",
      name: "@godseye/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@godseye/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@godseye/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls,
        expectedSpec: "@godseye/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
