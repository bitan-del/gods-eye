...
function createSinglePluginWithEmptySlotsConfig(): GodsEyeConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): GodsEyeConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "godseye-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-godseye-extensions/my-plugin",
      },
      extensionsDir,
    });
