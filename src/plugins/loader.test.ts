...
import { __testing, clearPluginLoaderCache, loadGodsEyePlugins } from "./loader.js";
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadGodsEyePlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "godseye-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.GODSEYE_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "godseye.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.GODSEYE_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          godseye: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.GODSEYE_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.GODSEYE_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadGodsEyePlugins>) {
...
function useNoBundledPlugins() {
  process.env.GODSEYE_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadGodsEyePlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadGodsEyePlugins>[0], "cache" | "config">,
) {
  return loadGodsEyePlugins({
    cache: false,
    ...options,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".godseye", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ GODSEYE_STATE_DIR: stateDir, ...params.env }, () =>
    loadGodsEyePlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadGodsEyePlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadGodsEyePlugins>;
  loadSecond: () => ReturnType<typeof loadGodsEyePlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadGodsEyePlugins>;
  loadVariant: () => ReturnType<typeof loadGodsEyePlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const godseyeHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(godseyeHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    GODSEYE_HOME: godseyeHome,
    HOME: ignoredHome,
    GODSEYE_STATE_DIR: stateDir,
    GODSEYE_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadGodsEyePlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.GODSEYE_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.GODSEYE_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".godseye", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ GODSEYE_STATE_DIR: stateDir }, () =>
      loadGodsEyePlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadGodsEyePlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadGodsEyePlugins(options);
          const scoped = loadGodsEyePlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadGodsEyePlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadGodsEyePlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadGodsEyePlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadGodsEyePlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadGodsEyePlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadGodsEyePlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadGodsEyePlugins({
              ...
              env: {
                ...process.env,
                GODSEYE_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadGodsEyePlugins({
              ...
              env: {
                ...process.env,
                GODSEYE_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadGodsEyePlugins({
              ...
              env: {
                ...
                GODSEYE_STATE_DIR: stateDir,
                GODSEYE_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadGodsEyePlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(godseyeHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadGodsEyePlugins(options),
          loadVariant: () =>
            loadGodsEyePlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadGodsEyePlugins({
      env: {
        ...
        HOME: homeDir,
        GODSEYE_HOME: undefined,
        GODSEYE_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers GODSEYE_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const godseyeHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "godseye-home-demo",
      dir: path.join(godseyeHome, "plugins", "godseye-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "godseye-home-demo", register() {} };`,
    });
    ...
    const registry = loadGodsEyePlugins({
      ...
      config: {
        plugins: {
          allow: ["godseye-home-demo"],
          entries: {
            "godseye-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/godseye-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "godseye-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadGodsEyePlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadGodsEyePlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    const setupRegistry = loadGodsEyePlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@godseye/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@godseye/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@godseye/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@godseye/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ GODSEYE_STATE_DIR: stateDir }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".godseye", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("godseye/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ GODSEYE_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadGodsEyePlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__godseyeLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("godseye/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("godseye/plugin-sdk");
