...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-whatsapp-boundary-"));
  ...
  fs.writeFileSync(path.join(rootDir, "godseye.mjs"), "export {};\n", "utf8");
  ...
  fs.writeFileSync(
    ...
    [
      'const key = Symbol.for("godseye.whatsapp.activeListenerState");',
      "const g = globalThis;",
      "if (!g[key]) {",
