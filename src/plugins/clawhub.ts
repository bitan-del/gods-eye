...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "godseye skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this GodsEye runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires GodsEye >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
  ...
  try {
    ...
    return {
      ...
      clawhub: {
        source: "clawhub",
        clawhubUrl:
          params.baseUrl?.trim() || process.env.GODSEYE_CLAWHUB_URL?.trim() || "https://clawhub.ai",
