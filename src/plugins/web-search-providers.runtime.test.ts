...
let loadGodsEyePluginsMock: ReturnType<typeof vi.fn>;
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadPluginManifestRegistryMock = vi
      ...
      .mockReturnValue({
        plugins: [
          {
            ...
            manifestPath: "/tmp/brave/godseye.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
          ...
          },
          {
            ...
            manifestPath: "/tmp/noise/godseye.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
        ...
        ? R
        : never);
    loadGodsEyePluginsMock = vi
      .spyOn(loaderModule, "loadGodsEyePlugins")
  ...
  it("loads bundled providers through the plugin loader in alphabetical order", () => {
    ...
    expect(loadGodsEyePluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("scopes plugin loading to manifest-declared web-search candidates", () => {
    ...
    expect(loadGodsEyePluginsMock).toHaveBeenCalledWith(
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { GODSEYE_HOME: "/tmp/godseye-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    const env = {
      GODSEYE_HOME: "/tmp/godseye-home-a",
    ...
    env.GODSEYE_HOME = "/tmp/godseye-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadGodsEyePluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when discovery cache ttl is zero", () => {
    ...
    const env = {
      GODSEYE_HOME: "/tmp/godseye-home",
      GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "0",
    } as NodeJS.ProcessEnv;
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const env = {
      GODSEYE_HOME: "/tmp/godseye-home",
      GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "5",
      GODSEYE_PLUGIN_MANIFEST_CACHE_MS: "20",
    } as NodeJS.ProcessEnv;
  ...
  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    ...
    const env = {
      GODSEYE_HOME: "/tmp/godseye-home",
      GODSEYE_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    } as NodeJS.ProcessEnv;
    ...
    env.GODSEYE_PLUGIN_DISCOVERY_CACHE_MS = "5";
  ...
  it("prefers the active plugin registry for runtime resolution", () => {
    ...
    expect(loadGodsEyePluginsMock).not.toHaveBeenCalled();
  });
});
