...
const { makeStorePath } = createCronStoreHarness({ prefix: "godseye-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses GODSEYE_HOME for tilde expansion", () => {
    vi.stubEnv("GODSEYE_HOME", "/srv/godseye-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/godseye-home", "cron", "jobs.json"));
  });
});
