...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.GODSEYE_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.godseye/agents/<agentId>/agent/).
  const agentId = normalizedRequested ?? defaultAgentId;
  const agentCfg = buildCronAgentDefaultsConfig({
    defaults: params.cfg.agents?.defaults,
    agentConfigOverride,
  });
  const cfgWithAgentDefaults: GodsEyeConfig = {
    ...params.cfg,
      ...
      const fallbackResult = await runWithModelFallback({
        ...
        run: async (providerOverride, modelOverride, runOptions) => {
          ...
          if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
            ...
            // the configured timeoutSeconds. See: https://github.com/bitan-del/gods-eye/issues/29774
          ...
          const result = await runEmbeddedPiAgent({
            ...
            // inherit owner-only tooling like local `godseye agent` runs.
            senderIsOwner: true,
            messageChannel,
