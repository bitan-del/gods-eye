...
const GODSEYE_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of GODSEYE_COMPILED_SECRET_TARGETS) {
...
const GODSEYE_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargets(config: GodsEyeConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: GODSEYE_COMPILED_SECRET_TARGETS,
    entriesById: GODSEYE_TARGETS_BY_ID,
  });
