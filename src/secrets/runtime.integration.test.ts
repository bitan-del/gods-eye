...
describe("secrets runtime snapshot integration", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "GODSEYE_DISABLE_PLUGIN_DISCOVERY_CACHE",
      "GODSEYE_VERSION",
    ]);
    delete process.env.GODSEYE_BUNDLED_PLUGINS_DIR;
    process.env.GODSEYE_DISABLE_PLUGIN_DISCOVERY_CACHE = "1";
    delete process.env.GODSEYE_VERSION;
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          ...
          agentDirs: ["/tmp/godseye-agent-main"],
          loadAuthStore: () =>
        ...
        expect(
          ensureAuthProfileStore("/tmp/godseye-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("godseye-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("godseye-secrets-runtime-refresh-fail-", async (home) => {
      const configDir = path.join(home, ".godseye");
      const secretFile = path.join(configDir, "secrets.json");
      const agentDir = path.join(configDir, "agents", "main", "agent");
      const authStorePath = path.join(agentDir, "auth-profiles.json");
  ...
  it("keeps last-known-good web runtime snapshot when reload introduces unresolved active web refs", async () => {
    await withTempHome("godseye-secrets-runtime-web-reload-lkg-", async (home) => {
      const prepared = await prepareSecretsRuntimeSnapshot({
        config: asConfig({
          tools: {
            web: {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("godseye-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".godseye", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".godseye", "agents", "ops", "agent");
