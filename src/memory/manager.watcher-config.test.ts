...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): GodsEyeConfig {
    const defaults: NonNullable<NonNullable<GodsEyeConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: GodsEyeConfig) {
