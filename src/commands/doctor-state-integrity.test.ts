...
type EnvSnapshot = {
  HOME?: string;
  GODSEYE_HOME?: string;
  GODSEYE_STATE_DIR?: string;
  GODSEYE_OAUTH_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    GODSEYE_OAUTH_DIR: process.env.GODSEYE_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: GodsEyeConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: GodsEyeConfig) {
...
async function runStateIntegrityText(cfg: GodsEyeConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.GODSEYE_HOME = tempHome;
    process.env.GODSEYE_STATE_DIR = path.join(tempHome, ".godseye");
    delete process.env.GODSEYE_OAUTH_DIR;
    fs.mkdirSync(process.env.GODSEYE_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when GODSEYE_OAUTH_DIR is explicitly configured", async () => {
    process.env.GODSEYE_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: GodsEyeConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints godseye-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: GodsEyeConfig = {};
    ...
    expect(text).toMatch(/godseye sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/godseye sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /godseye sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
