...
import { readConfigFileSnapshot, writeConfigFile, type GodsEyeConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("godseye config set gateway.auth.mode token")} or ${formatCliCommand("godseye config set gateway.auth.mode password")}.`,
  ].join(" ");
}
