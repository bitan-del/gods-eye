...
import type { GodsEyeConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: GodsEyeConfig): string[] {
...
export async function noteSecurityWarnings(cfg: GodsEyeConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("godseye security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.godseye/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("godseye approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("godseye configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("godseye config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("godseye doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("godseye config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('godseye config set session.dmScope "per-channel-peer"') +
