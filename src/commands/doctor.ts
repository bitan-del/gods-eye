...
function resolveMode(cfg: GodsEyeConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("GodsEye doctor");

  const root = await resolveGodsEyePackageRoot({
  ...
  let cfg: GodsEyeConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("godseye configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("godseye config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("godseye setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("godseye config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("godseye config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("godseye doctor --fix")}" to apply changes.`);
  }
