...
import { GodsEyeSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: GodsEyeConfig): {
  config: GodsEyeConfig;
  removed: string[];
} {
  const parsed = GodsEyeSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: GodsEyeConfig): void {
