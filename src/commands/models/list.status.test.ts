...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveGodsEyeAgentDir: vi.fn().mockReturnValue("/tmp/godseye-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/godseye-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/godseye-agent/auth-profiles.json"),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveGodsEyeAgentDir: mocks.resolveGodsEyeAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/godseye-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveGodsEyeAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/godseye-dev/godseye.json");
    expect(payload.auth.storePath).toBe("/tmp/godseye-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/godseye-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/godseye-agent-custom");
