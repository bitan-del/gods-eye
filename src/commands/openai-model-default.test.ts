...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: GodsEyeConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: GodsEyeConfig) => { changed: boolean; next: GodsEyeConfig };
  defaultModel: string;
  overrideConfig: GodsEyeConfig;
  alreadyDefaultConfig: GodsEyeConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: GodsEyeConfig) => config,
      applyDefaultConfig: (config: GodsEyeConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: GodsEyeConfig = { agents: { defaults: {} } };
