...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.godseye/godseye.json",
...
vi.mock("./onboard-helpers.js", () => ({
  ...
  applyWizardMetadata: (cfg: GodsEyeConfig) => cfg,
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.applySearchProviderSelection.mockImplementation((cfg: GodsEyeConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by applySearchKey", async () => {
    ...
    mocks.hasExistingKey.mockReturnValue(false);
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchKey.mockImplementation((cfg: GodsEyeConfig, provider: string, key: string) =>
  ...
  it("applies provider selection side effects when a key already exists via secret ref or env", async () => {
    ...
    mocks.hasKeyInEnv.mockReturnValue(false);
    mocks.applySearchProviderSelection.mockImplementation((cfg: GodsEyeConfig, provider: string) =>
      createEnabledWebSearchConfig(provider, {
        enabled: true,
      })(cfg),
    );
