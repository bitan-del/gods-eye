...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("godseye pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelSetupDmPolicy) => {
    ...
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`godseye pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!plugin) {
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("godseye plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
        await refreshStatus(channel);
        return true;
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("godseye channels add")}\``,
        },
      ],
