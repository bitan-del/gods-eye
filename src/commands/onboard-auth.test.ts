...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under GODSEYE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("godseye-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-oauth-sync-"));
    ...
    process.env.GODSEYE_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-oauth-nosync-"));
    process.env.GODSEYE_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside GODSEYE_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-oauth-external-"));
    process.env.GODSEYE_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* GODSEYE_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to GODSEYE_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("godseye-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
