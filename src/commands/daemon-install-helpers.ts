...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.GODSEYE_PROFILE)
        : undefined,
  ...
  //   1. ~/.godseye/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (godseye.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, GODSEYE_* — highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("godseye gateway install")}\` after fixing the error.`;
}
