...
type PromptDefaultModelResult = { model?: string; config?: GodsEyeConfig };
...
function resolveConfiguredModelRaw(cfg: GodsEyeConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: GodsEyeConfig): string[] {
...
export function applyModelAllowlist(cfg: GodsEyeConfig, models: string[]): GodsEyeConfig {
