...
const ensureGodsEyeModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveGodsEyeAgentDir = vi.fn().mockReturnValue("/tmp/godseye-agent");
...
beforeEach(() => {
  ...
  ensureGodsEyeModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureGodsEyeModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureGodsEyeModelsJson).toHaveBeenCalled();
    expect(ensureGodsEyeModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
