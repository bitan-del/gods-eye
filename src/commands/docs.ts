...
const SEARCH_TOOL = "https://docs.gods-eye.org/mcp.SearchGodsEye";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.gods-eye.org");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('godseye docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.gods-eye.org/");
      runtime.log(`Search: ${formatCliCommand('godseye docs "your query"')}`);
    }
    return;
  }
