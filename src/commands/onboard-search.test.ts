...
function createPerplexityConfig(apiKey: string, enabled?: boolean): GodsEyeConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: GodsEyeConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): GodsEyeConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: GodsEyeConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: GodsEyeConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
