...
export function normalizeCompatibilityConfigValues(cfg: GodsEyeConfig): {
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as GodsEyeConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as GodsEyeConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<GodsEyeConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<GodsEyeConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<GodsEyeConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as GodsEyeConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as GodsEyeConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<GodsEyeConfig["tools"]>["media"],
      },
    };
  };
  ...
  const normalizeLegacyMistralModelMaxTokens = () => {
    ...
    next = {
      ...
      models: {
        ...next.models,
        providers: nextProviders as NonNullable<GodsEyeConfig["models"]>["providers"],
      },
    };
  };
