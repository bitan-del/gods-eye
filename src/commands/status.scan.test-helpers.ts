...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/godseye-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = GodsEyeConfig>(
  overrides: T = {} as T,
): GodsEyeConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): GodsEyeConfig {
