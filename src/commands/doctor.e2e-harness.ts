...
export const resolveGodsEyePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadGodsEyePlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveGodsEyePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.GODSEYE_STATE_DIR;
  originalUpdateInProgress = process.env.GODSEYE_UPDATE_IN_PROGRESS;
  process.env.GODSEYE_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-doctor-state-"));
