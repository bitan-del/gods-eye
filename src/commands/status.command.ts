...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByGodsEye ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByGodsEye ? "installed · " : "";
  ...
  runtime.log(theme.heading("GodsEye status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`godseye devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("godseye devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("godseye devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("godseye security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("godseye security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("godseye status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("godseye logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("godseye status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("godseye gateway probe")}`);
  }
}
