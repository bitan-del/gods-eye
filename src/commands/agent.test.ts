...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/godseye-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/godseye-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "godseye-agent-" });
}
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<GodsEyeConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<GodsEyeConfig["channels"]>["telegram"]>>,
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: GodsEyeConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as GodsEyeConfig },
    writeOptions: {},
