...
beforeAll(() => {
  envSnapshot = captureEnv(["GODSEYE_PROFILE"]);
...
vi.mock("../infra/godseye-root.js", () => ({
  resolveGodsEyePackageRoot: vi.fn().mockResolvedValue("/tmp/godseye"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/godseye/pnpm-lock.yaml",
      markerPath: "/tmp/godseye/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.godseye.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.godseye.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("godseye status --all") ||
          line.includes("godseye --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("GODSEYE_GATEWAY_TOKEN", "abcd1234", async () => {
