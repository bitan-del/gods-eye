...
export function hasExplicitMemorySearchConfig(cfg: GodsEyeConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: GodsEyeConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: GodsEyeConfig, agentId: string) => { store: { path: string } } | null;
