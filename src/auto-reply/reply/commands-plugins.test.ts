...
const workspaceHarness = createCommandWorkspaceHarness("godseye-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".godseye", "extensions", params.pluginId);
...
function buildCfg(): GodsEyeConfig {
  return {
    plugins: {
      enabled: true,
    },
