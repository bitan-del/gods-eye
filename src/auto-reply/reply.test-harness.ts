...
type HomeEnvSnapshot = {
  ...
  GODSEYE_STATE_DIR: string | undefined;
  GODSEYE_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    GODSEYE_AGENT_DIR: process.env.GODSEYE_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".godseye", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.GODSEYE_STATE_DIR = path.join(home, ".godseye");
    process.env.GODSEYE_AGENT_DIR = path.join(home, ".godseye", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".godseye", "agent");
