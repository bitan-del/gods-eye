 ...
 * A common mistake — caused by the similarity with the `godseye.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (godseye.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.GODSEYE_AUTH_STORE_READONLY === "1";
