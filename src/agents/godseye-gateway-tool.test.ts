...
async function loadFreshGodsEyeToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createGodsEyeTools } = await import("./godseye-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshGodsEyeToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-test-"));

    try {
      await withEnvAsync({ GODSEYE_STATE_DIR: stateDir, GODSEYE_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
        ...
        expect(parsed.payload?.doctorHint).toBe(
          "Run: godseye --profile isolated doctor --non-interactive",
        );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/godseye" } }\n}\n';
