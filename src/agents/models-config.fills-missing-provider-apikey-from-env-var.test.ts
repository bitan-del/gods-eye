...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): GodsEyeConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureGodsEyeModelsJson(validated.config);
  ...
  it("fills anthropic-vertex apiKey with the ADC sentinel when models exist", async () => {
    await withTempHome(async () => {
      const adcDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-adc-"));
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureGodsEyeModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
