...
describe("resolveGodsEyeAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          GODSEYE_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveGodsEyeAgentDir();
  ...
  it("honors GODSEYE_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          GODSEYE_STATE_DIR: undefined,
          GODSEYE_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveGodsEyeAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when GODSEYE_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers GODSEYE_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          GODSEYE_STATE_DIR: undefined,
          GODSEYE_AGENT_DIR: primaryOverride,
