...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("godseye-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("godseye-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("godseye-sbx-");
