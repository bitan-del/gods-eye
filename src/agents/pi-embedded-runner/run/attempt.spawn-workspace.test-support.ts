...
vi.mock("../../docs-path.js", () => ({
  resolveGodsEyeDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createGodsEyeCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("godseye.cache-ttl", data),
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-ctx-engine-agent-"));
