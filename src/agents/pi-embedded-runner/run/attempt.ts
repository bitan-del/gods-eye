...
import { resolveGodsEyeDocsPath } from "../../docs-path.js";
...
import { createGodsEyeCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "godseye.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "godseye.sessions_yield";
const SESSIONS_YIELD_ABORT_SETTLE_TIMEOUT_MS = process.env.GODSEYE_TEST_FAST === "1" ? 250 : 2_000;
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveGodsEyeAgentDir();
    ...
    const docsPath = await resolveGodsEyeDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/bitan-del/gods-eye/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("godseye:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/bitan-del/gods-eye/issues/8643
