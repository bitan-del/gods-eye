...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses GODSEYE_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      GODSEYE_HOME: "/srv/godseye-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/godseye-home"), ".godseye", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".godseye", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("godseye-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("godseye-workspace-");
    await fs.mkdir(path.join(tempDir, ".godseye"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-workspace-hardlink-"));
    try {
