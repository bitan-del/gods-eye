...
function mockContextDeps(params: {
  ...
  const ensureGodsEyeModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveGodsEyeAgentDir: () => "/tmp/godseye-agent",
  }));
  ...
  return { ensureGodsEyeModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real godseye startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "godseye", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "godseye", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "godseye", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureGodsEyeModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureGodsEyeModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
