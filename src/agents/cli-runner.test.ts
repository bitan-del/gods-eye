...
describe("runCliAgent with process supervisor", () => {
  ...
  it("hydrates prompt media refs into CLI image args", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredGodsEyeTmpDir(), "godseye-cli-prompt-image-"),
    );
    ...
    expect(argv[imageArgIndex + 1]).toContain("godseye-cli-images-");
  ...
  it("appends hydrated prompt media refs to generic backend prompts", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredGodsEyeTmpDir(), "godseye-cli-prompt-image-generic-"),
    );
    ...
    const appendedPath = argv.find((value) => value.includes("godseye-cli-images-"));
  ...
  it("prefers explicit images over prompt refs", async () => {
    ...
    const tempDir = await fs.mkdtemp(
      path.join(resolvePreferredGodsEyeTmpDir(), "godseye-cli-explicit-images-"),
    );
  ...
  it("falls back to per-agent workspace when workspaceDir is missing", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-cli-runner-"));
