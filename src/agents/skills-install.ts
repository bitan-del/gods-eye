...
async function collectSkillInstallScanWarnings(entry: SkillEntry): Promise<string[]> {
  ...
  try {
    ...
    if (summary.critical > 0) {
      ...
      warnings.push(
        `Skill "${skillName}" has ${summary.warn} suspicious code pattern(s). Run "godseye security audit --deep" for details.`,
      );
    }
  } catch (err) {
    warnings.push(
      `Skill "${skillName}" code safety scan failed (${String(err)}). Installation continues; run "godseye security audit --deep" after install.`,
    );
  }
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const trustedInstallSources = new Set(["godseye-bundled", "godseye-managed", "godseye-extra"]);
