...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    godseye: process.env.GODSEYE_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.godseye === undefined) {
      delete process.env.GODSEYE_GATEWAY_TOKEN;
    } else {
      process.env.GODSEYE_GATEWAY_TOKEN = envSnapshot.godseye;
    }
  });
  ...
  it("uses GODSEYE_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.GODSEYE_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
