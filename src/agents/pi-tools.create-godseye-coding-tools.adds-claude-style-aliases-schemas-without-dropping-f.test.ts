...
describe("createGodsEyeCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-alias-"));
    try {
      const tools = createGodsEyeCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts broader file/edit alias variants", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-alias-broad-"));
    try {
      const tools = createGodsEyeCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-structured-write-"));
    try {
      const tools = createGodsEyeCodingTools({ workspaceDir: tmpDir });
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-structured-edit-"));
    try {
