...
const DIRECT_ENABLED = Boolean(process.env.GODSEYE_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.GODSEYE_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.GODSEYE_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.GODSEYE_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.GODSEYE_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.GODSEYE_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.GODSEYE_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via GODSEYE_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
