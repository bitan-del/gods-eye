...
async function withOutsideHardlinkInGodsEyeTmp<T>(
  params: {
    godsEyeTmpDir: string;
  ...
  const hardlinkPath = path.join(params.godsEyeTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.godsEyeTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.godsEyeTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.godsEyeTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const godsEyeTmpDir = resolvePreferredGodsEyeTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred GodsEye tmp root",
      media: path.join(godsEyeTmpDir, "image.png"),
      expected: path.join(godsEyeTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred GodsEye tmp root",
      media: pathToFileURL(path.join(godsEyeTmpDir, "photo.png")).href,
      expected: path.join(godsEyeTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred GodsEye tmp root",
      media: path.join(godsEyeTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(godsEyeTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(godsEyeTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside godseye tmp root",
      media: path.join(os.tmpdir(), "outside-godseye", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked GodsEye tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(godsEyeTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(godsEyeTmpDir, { recursive: true });
      const symlinkPath = path.join(godsEyeTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked GodsEye tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGodsEyeTmp(
      {
        godsEyeTmpDir,
  ...
  it("rejects symlinked GodsEye tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInGodsEyeTmp(
