...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createGodsEyeTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/godseye-camera-snap-front-.*\.jpg$/);
  });
...
describe("nodes photos_latest", () => {
  ...
  it("returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/godseye-camera-snap-.*\.jpg$/);
  });
...
async function loadGodsEyeToolsForTest(): Promise<void> {
  vi.resetModules();
