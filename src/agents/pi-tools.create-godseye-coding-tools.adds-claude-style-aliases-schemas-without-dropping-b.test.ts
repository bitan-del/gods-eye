...
const defaultTools = createGodsEyeCodingTools({ senderIsOwner: true });
...
describe("createGodsEyeCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createGodsEyeCodingTools({
    ...
    const anthropicTools = createGodsEyeCodingTools({
      config: enabledConfig,
    ...
    const allowed = createGodsEyeCodingTools({
    ...
    const denied = createGodsEyeCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createGodsEyeCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
