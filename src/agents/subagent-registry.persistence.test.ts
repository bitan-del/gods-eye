...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["GODSEYE_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-subagent-"));
  ...
  it("uses isolated temp state when GODSEYE_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "godseye-test-state"));
  });
});
