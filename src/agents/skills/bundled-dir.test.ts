...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["GODSEYE_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns GODSEYE_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-bundled-override-"));
    process.env.GODSEYE_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.GODSEYE_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "godseye" }));
