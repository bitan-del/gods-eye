...
import { markGodsEyeExecEnv } from "../../infra/godseye-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "godseye.sandbox=1");
  args.push("--label", `godseye.sessionKey=${params.scopeKey}`);
  args.push("--label", `godseye.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `godseye.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markGodsEyeExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "godseye.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`godseye sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`godseye sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("godseye sandbox recreate --all");
}
