...
const loadGodsEyePlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadGodsEyePlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadGodsEyePlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadGodsEyePlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.gods-eye.org/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadGodsEyePlugins.mockReturnValue(pluginRegistry);
