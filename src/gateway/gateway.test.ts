...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "godseye");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".godseye");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-wizard-home-"));
      process.env.HOME = tempHome;
