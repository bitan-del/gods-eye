...
describe("gateway handshake timeouts", () => {
  ...
  test("prefers GODSEYE_HANDSHAKE_TIMEOUT_MS and falls back on the test-only env", () => {
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        GODSEYE_HANDSHAKE_TIMEOUT_MS: "75",
        GODSEYE_TEST_HANDSHAKE_TIMEOUT_MS: "20",
      }),
    ).toBe(75);
    expect(
      getPreauthHandshakeTimeoutMsFromEnv({
        GODSEYE_HANDSHAKE_TIMEOUT_MS: "",
        GODSEYE_TEST_HANDSHAKE_TIMEOUT_MS: "20",
        VITEST: "1",
      }),
    ).toBe(20);
