...
const GATEWAY_TEST_PLUGIN_REGISTRY_STATE_KEY = Symbol.for(
  "godseye.gatewayTestHelpers.pluginRegistryState",
);
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("godseye.gatewayTestHelpers.configRoot");

const hoisted = vi.hoisted(() => {
  const key = Symbol.for("godseye.gatewayTestHelpers.hoisted");
...
const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `godseye-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.GODSEYE_CONFIG_PATH = path.join(root, "godseye.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "godseye.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "godseye-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: GodsEyeConfig) => composeTestConfig(cfg as Record<string, unknown>),
    loadConfig: () => {
      const configPath = resolveConfigPath();
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadGodsEyePlugins: () => pluginRegistryState.registry,
  };
});
