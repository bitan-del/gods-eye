...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("godseye doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with GODSEYE_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with GODSEYE_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.GODSEYE_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://godseye-gateway.ai:18789",
      onConnectError,
    });
