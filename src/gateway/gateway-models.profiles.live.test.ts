...
import { ensureGodsEyeModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/config.js";
import type { ModelsConfig, GodsEyeConfig, ModelProviderConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.GODSEYE_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const PROVIDERS = parseFilter(process.env.GODSEYE_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.GODSEYE_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.GODSEYE_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const describeLive = isLiveTestEnabled(["GODSEYE_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.GODSEYE_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.GODSEYE_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<GodsEyeConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.GODSEYE_CONFIG_PATH,
    token: process.env.GODSEYE_GATEWAY_TOKEN,
    skipChannels: process.env.GODSEYE_SKIP_CHANNELS,
    skipGmail: process.env.GODSEYE_SKIP_GMAIL_WATCHER,
    skipCron: process.env.GODSEYE_SKIP_CRON,
    skipCanvas: process.env.GODSEYE_SKIP_CANVAS_HOST,
    agentDir: process.env.GODSEYE_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.GODSEYE_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.GODSEYE_SKIP_CHANNELS = "1";
  process.env.GODSEYE_SKIP_GMAIL_WATCHER = "1";
  process.env.GODSEYE_SKIP_CRON = "1";
  process.env.GODSEYE_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.GODSEYE_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveGodsEyeAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-live-state-"));
  process.env.GODSEYE_STATE_DIR = tempStateDir;
  ...
  process.env.GODSEYE_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.godseye-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveGodsEyeAgentDir();
  const sanitizedCfg: GodsEyeConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "godseye-live-"));
  const tempConfigPath = path.join(tempDir, "godseye.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.GODSEYE_CONFIG_PATH = tempConfigPath;
  ...
  try {
        ...
        try {
          ...
          for (
            ...
            toolText = await requestGatewayAgentText({
              ...
              modelKey,
              message: strictReply
                ? "GodsEye live tool probe (local, safe): " +
    ...
    process.env.GODSEYE_CONFIG_PATH = previous.configPath;
    process.env.GODSEYE_GATEWAY_TOKEN = previous.token;
    process.env.GODSEYE_SKIP_CHANNELS = previous.skipChannels;
    process.env.GODSEYE_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.GODSEYE_SKIP_CRON = previous.skipCron;
    process.env.GODSEYE_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.GODSEYE_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.GODSEYE_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureGodsEyeModelsJson(cfg);
      ...
      const rawModels = process.env.GODSEYE_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via GODSEYE_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.godseye-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
