...
declare const __GODSEYE_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "godseye";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["GODSEYE_VERSION"],
      runtimeVersion,
      env["GODSEYE_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current GodsEye version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __GODSEYE_VERSION__ === "string" ? __GODSEYE_VERSION__ : undefined,
  bundledVersion: process.env.GODSEYE_BUNDLED_VERSION,
});
