...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@godseye/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-catalog-"));
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-catalog-home-"));
        ...
        entries: [
          {
            ...
            godseye: {
              ...
              install: {
                npmSpec: "@godseye/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        GODSEYE_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        GODSEYE_HOME: home,
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-catalog-state-"));
        ...
        entries: [
          {
            ...
            godseye: {
              ...
              install: {
                npmSpec: "@godseye/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "godseye" }),
      "utf8",
    );
    fs.writeFileSync(
      path.join(bundledDir, "package.json"),
      JSON.stringify({
        name: "@godseye/whatsapp",
        godseye: {
          extensions: ["./index.js"],
          channel: {
            id: "whatsapp",
    ...
    fs.writeFileSync(
      path.join(bundledDir, "godseye.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        GODSEYE_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@godseye/whatsapp");
    expect(entry?.pluginId).toBe("whatsapp");
  });

  it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-official-catalog-"));
  ...
  it("lets external catalogs override shipped fallback channel metadata", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "godseye-fallback-catalog-"));
    ...
    const entry = listChannelPluginCatalogEntries({
      ...
      env: {
        ...process.env,
        GODSEYE_BUNDLED_PLUGINS_DIR: path.join(dir, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");
