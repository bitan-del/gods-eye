...
import {
  ...
  GODSEYE_NODE_EXTRA_CA_CERTS_READY,
  GODSEYE_NODE_OPTIONS_READY,
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(plan?.env[GODSEYE_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(plan?.env[GODSEYE_NODE_OPTIONS_READY]).toBe("1");
  });
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "godseye", "gateway", "run"],
        env: {
          [GODSEYE_NODE_EXTRA_CA_CERTS_READY]: "1",
          [GODSEYE_NODE_OPTIONS_READY]: "1",
        },
